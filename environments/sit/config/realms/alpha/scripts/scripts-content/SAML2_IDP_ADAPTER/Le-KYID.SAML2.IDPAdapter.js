...
function preSendResponse() {
    ...
    if(request.getHeader("Cookie") && request.getHeader("Cookie")!=null){
        ...
        //   if(cookie.localeCompare("ReturnURL")==0){
        //       displayUrl = cookieName[1];
        //   } 

            if(cookie.localeCompare("rp-realm")==0){
                wtrealmUrl = cookieName[1];
              } 

              if(cookie.localeCompare("isWsfedApp")==0){
    ...
    try {
         ...
         if (issuer.localeCompare(systemEnv.getProperty("esv.logout.app.url")) == 0) {
            return false;
        } else if (redirectIsNeeded()) {
            logger.error(logPrefix + "iterating redirection conditions");
            if (session.getProperty("visitedUrl").localeCompare("FALSE") == 0) {
                logger.error(logPrefix + "******************************************Redirecting to force auth****************************" + session.getProperty("visitedUrl"));
                logger.error(logPrefix + "SAML Final Redirect URL2 : "+finalUrl);
                ...
                logger.error(logPrefix + "*********finalUrl: "+finalUrl)
                ...
                logger.error(logPrefix + "*********defaultUrl: "+defaultUrl)
...
function setAppContextInSessionProps(realm,request,authnRequest,response,reqId,session,relayState){
    ...
    var finalUrl = null;
  	...
  	logger.error(logPrefix + "************issuer value in setAppContextInSessionProps method*************"+issuer);
    ...
    if (issuer.localeCompare(systemEnv.getProperty("esv.logout.app.url")) == 0 && !session) {
		...
		logger.error(logPrefix + "************authnRequest.getExtensions() value in setAppContextInSessionProps method*************"+authnRequest.getExtensions().getAny());
        //wsfed application coming from on-prem app
        var authnRequestExtensionList=authnRequest.getExtensions().getAny();
        //logger.error("SAML PreAuthentication AUTHNREQUEST-EXTLIST: "+authnRequestExtensionList);
        var rawDisplayUrl=authnRequestExtensionList.get(0).split("saml2:AttributeValue")[1];
        displayUrl=rawDisplayUrl.substring(1,rawDisplayUrl.length()-2);
        response.addHeader("Set-Cookie", "ReturnURL=" + decodeURIComponent(displayUrl) + "; Path=/; HttpOnly");
        logger.error(logPrefix + "************displayUrl in a ReturnURL cookie in setAppContextInSessionProps method*************"+displayUrl);
        //logger.error("Set displayUrl in a ReturnURL cookie: " + displayUrl);
        ...
        response.addHeader("Set-Cookie", "rp-realm=" + decodeURIComponent(wtrealmUrl) + "; Path=/; HttpOnly");
        logger.error(logPrefix + "************wtrealmUrl in a rp-realm cookie in setAppContextInSessionProps method*************"+wtrealmUrl);
        logger.error(logPrefix + "************WSFed PreAuthentication DISPLAY URL in setAppContextInSessionProps method*************"+displayUrl);
        //logger.error("Set wtrealmUrl in a rp-realm cookie: " + wtrealmUrl);
        //logger.error("WSFed PreAuthentication DISPLAY URL: "+displayUrl);
        isWsfedApp=true;
    } else if(issuer.localeCompare("kyidsp") == 0 ) {
            ...
            response.addHeader("Set-Cookie", "ReturnURL=" + decodeURIComponent(displayUrl) + "; Path=/; HttpOnly");
            response.addHeader("Set-Cookie", "rp-realm=" + decodeURIComponent(displayUrl) + "; Path=/; HttpOnly");
            logger.error(logPrefix + "************Default DISPLAY URL in setAppContextInSessionProps method*************"+displayUrl);
    } else if(relayState !== null && relayState.startsWith("https")) {
        //SAML Application with a full proper relayState URL
        displayUrl = relayState;
        response.addHeader("Set-Cookie", "ReturnURL=" + decodeURIComponent(displayUrl) + "; Path=/; HttpOnly");
    ...
    response.addHeader("Set-Cookie", "locale=" + lang + "; Path=/; HttpOnly; sameSite=Lax; domain="+tenantCookieDomain);
    response.addHeader("Set-Cookie", "clocale=" + lang + "; Path=/; HttpOnly; sameSite=Lax; domain="+tenantCookieDomain);
    logger.error(logPrefix + "Language value in a locale cookie in setAppContextInSessionProps method: " + lang);
...
function redirectIsNeeded() {
    if (!session) {
        logger.error(logPrefix + " redirection is needed");
        return true;
    }

    // Integrity check below. Do not enforce master journey only if the previous app is the same as current.
    var lastAppId = session.getProperty("lastAppId");
    var currentAppEntityId = getSpEntityId(request, authnRequest)
    logger.error(logPrefix + "lastAppId from session: {}, current: {}", lastAppId, currentAppEntityId);


    if (currentAppEntityId && lastAppId && (currentAppEntityId == lastAppId)) {
        logger.error(logPrefix + "no redirection is needed");
        return false;
    }
