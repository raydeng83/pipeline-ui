...
try {
    ...
    var appIDinWidget = nodeState.get("appIDinWidget").toString();
if(appIDinWidget.startsWith("\"")) {
    appIDinWidget = appIDinWidget.substring(1,appIDinWidget.length-1);
}
    var requestAcessurl = "https://" + url + "/am/XUI/?realm=alpha&authIndexType=service&authIndexValue=kyid_2B1_requestAccess" + "&appIDinWidget=" + appIDinWidget;
    //var requestAcessurl = "https://selfservice.dev.kyid.ky.gov:5173/authorization/request-access" + "?appId=" + appIDinWidget;
  //  var requestAcessurl = "/authorization/request-access" + "?appId=" + appIDinWidget;
    logger.error("requestAcessurl url"+requestAcessurl)
    config = {
        "successUrl": requestAcessurl
    }
}
catch (e) {
