...
try {
    logger.debug("Executing KYID.2B1.Journey.Login.Register.MFA.GetContext ")
            ...
            logger.debug("context: "+ context +"::"+"userPrereqId:"+userPrereqId)
                  ...
                  if(getUserPrereqDetails(userPrereqId)){
                        ...
                        if(userPrereqResponse.result[0].preRequisiteTypeId.typeName ==="MFA" || userPrereqResponse.result[0].preRequisiteTypeId.typeName ==="4"){
                              logger.debug("KYID.2B1.Journey.Login.Register.MFA.GetContext --> userPrereqResponse:"+userPrereqResponse)
                              logger.debug("KYID.2B1.Journey.Login.Register.MFA.GetContext --> User ID is --> "+userId)
                              if(userPrereqResponse.result[0].requestedUserAccountId === userId){
                              logger.debug("KYID.2B1.Journey.Login.Register.MFA.GetContext --> Inside Userid == requestedUserAccountId")
                                    if(getRequiredMFACode(userPrereqResponse.result[0].associatedRoleIds)){    
                                          ...
                                          logger.debug("KYID.2B1.Journey.Login.Register.MFA.GetContext --> getRequiredMFACode:"+requiredMFACode)
...
function getRequiredMFACode(roleId) {
    try {
       ...
       logger.debug("KYID.2B1.Journey.Login.Register.MFA.GetContext getRequiredMFACode is-->  "+response )
