...
var nodeConfig = {
    ...
    script: "Script",
    scriptName: "KYID.Journey.MFADynamicChoiceCollectorWithoutResetRemove.LoginMain",
    timestamp: dateTime,
...
function availableMFAOptions(){
    ...
    var newMFAOptions = []
    if (MFAContextCode === 3) {
        newMFAOptions = ["DEFAULT", "EMAIL", "SMSVOICE", "FRPUSH", "PUSH", "FRTOTP", "TOTP", "SYMANTEC"]
    }
    
    else if (MFAContextCode === 4) {
        newMFAOptions = ["DEFAULT", "SMSVOICE", "FRPUSH", "PUSH", "FRTOTP", "TOTP", "SYMANTEC"]
    }
    
    else if (MFAContextCode === 5) {
        newMFAOptions = ["DEFAULT", "FRPUSH", "PUSH", "FRTOTP", "TOTP", "SYMANTEC"]
    }
...
function buildMFAOptionsArray(common,usrMFAData) {
    ...
    try {
        ...
        for (var i = 0; i < common.length; i++) {                                                                                                                                                                                                                                                                     
            ...
            } else if (mfaMethod === "FRTOTP" || mfaMethod === "TOTP") {
                mfaMethod = localeMessages.FRTOTP;
                mfaOptionsArray.push(mfaMethod);
            } else if (mfaMethod === "FRPUSH" || mfaMethod === "PUSH") {
        ...
        if (callbacks.isEmpty()) {
            ...
            callbacksBuilder.choiceCallback(
               `${promptMessage}`,
                ...
                var isJourneyRecoveryMFA = nodeState.get("isJourneyRecoveryMFA");
               
                if (isJourneyRecoveryMFA !== null && isJourneyRecoveryMFA !== undefined) {
                    logger.error("printing isJourneyRecoveryMFA " + isJourneyRecoveryMFA);
                if (isJourneyRecoveryMFA === "true") {
                    callbacksBuilder.confirmationCallback(0, [localeMessages.verify], 0);
                } else {
                callbacksBuilder.confirmationCallback(0, [localeMessages.verify, localeMessages.recoverymfa], 0);
                }
                }                                                                    
