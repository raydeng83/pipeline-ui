    ...
    if (missingInputs.length > 0) {
        ...
        if (kogAPITokenResponse.status === 200) {
            ...
            if (res.status === 200) {
                ...
                nodeLogger.error(nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + ":: HTTP response ::" + res.status + "::Email::" + newPrimaryEmail);
                action.goTo(nodeOutcome.FAIL);
            }
        } else {
            logger.error("kogAPITokenResponse is not 200 ");
            nodeLogger.error(nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + ":: kogAPITokenResponse is not 200 :: Email ::" + newPrimaryEmail);
            action.goTo(nodeOutcome.FAIL);
        }
    }
} catch (error) {
