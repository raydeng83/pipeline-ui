...
function getPrereqType(txid,nodeLogger,ops,prereqname){
    ...
    try { 
        logger.error("prereqname is "+prereqname);
          ...
          logger.error("recordPrereqTypes.result[0].typeofprerequisite - "+recordPrereqTypes.result[0].typeofprerequisite)
...
function patchPrerequisiteCancelRequestStatus(ops,txid,nodeLogger,id,userID,arrPreReqTypes,status){
    ...
    try{
        for(var i=0;i<arrPreReqTypes.length;i++){
            ...
            logger.error("Successfully queried record in alpha_kyid_request managed object for patch operation:: "+JSON.stringify(recordRequest));
            recordRequestJSONObj = JSON.parse(JSON.stringify(recordRequest.result[0]));
            logger.error("Resource ID - "+recordRequestJSONObj["_id"]);
...
function main(){
    ...
    getPrereqType(txid,nodeLogger,ops,type.toLowerCase());
        ...
        logger.error("Type of Prereq is - "+typeofprerequisite)
        ...
        if(selectedOutcome === 0){
            ...
            if(arrPreReqTypes.length>0){
                if(nodeState.get("PreqRoleRequestId")){
                    logger.error("Request Id is ----"+nodeState.get("PreqRoleRequestId"))
                ...
                logger.error("isAllPrereqStatusCancelled return value - "+isAllPrereqStatusCancelled); 
