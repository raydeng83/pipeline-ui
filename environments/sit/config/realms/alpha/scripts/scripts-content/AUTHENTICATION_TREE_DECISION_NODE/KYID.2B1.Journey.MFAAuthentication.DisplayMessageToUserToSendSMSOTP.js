...
var nodeConfig = {
    ...
    node: "Node",
    nodeName: "DisplayMessageToUserSMSOTP",
    script: "Script",
    scriptName: "KYID.Journey.MFAAuthentication.DisplayMessageToUserToSendSMSOTP",
    timestamp: dateTime,
...
function getTelephoneNumber() {
    ...
    if (userId) {
        ...
        if (userData && userData.telephoneNumber) {
            ...
            nodeLogger.debug(nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + nodeConfig.begin + "User data is null or telephone number is not available for user ID: " + userId);
        }
    } else {
        nodeLogger.debug(nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + nodeConfig.begin + "User ID is null or undefined.");
    }
    return null;
}
...
function createArray(){
    ...
    var displayMessage = "Send a code via SMS to " + boldoutput.bold() + ". \n Carrier messaging charges may apply."
    ...
    try {
        if (callbacks.isEmpty()) {
            ...
            if (clocale === "en") {
                ...
                var displayMessage = "Send a code to " + boldoutput.bold() + ". <br><br> Carrier messaging charges may apply.";
              ...
              //  callbacksBuilder.textOutputCallback(0,displayMessage)
                callbacksBuilder.confirmationCallback(0, ["Receive a code via SMS", "Receive a code via Voice","Return to authenticator list"], 0);
                ...
                var boldoutput = nodeStateNumber[0] + nodeStateNumber.slice(1, 3) + " XXX-XXX-" + nodeStateNumber.slice(-4)
                var displayMessage ="Enviar un código  a " + boldoutput.bold() + ". \n Se pueden aplicar cargos por mensajes del operador."
                callbacksBuilder.textOutputCallback(0,displayMessage)
                callbacksBuilder.confirmationCallback(0, ["Recibir un código vía SMS", "Recibir un código por voz", "Verificar con otra cosa"], 0);
            }else {
