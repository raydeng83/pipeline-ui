...
// Node Config
 var nodeConfig = {
    ...
    node: "Node",
    nodeName: "SwitchToSpanish",
    script: "Script",
    scriptName: "KYID.Journey.SwitchToSpanish",
    timestamp: dateTime,
...
// try {
//      if(requestCookies.RequestURL && requestCookies.RequestURL!=null){
//          var url = requestCookies.RequestURL;
//          var link= decodeURIComponent(requestCookies.RequestURL)+"&clocale=es"+"&locale=es";
...
try {
    logger.error("Insdie Spanish Language Switch Router Tree")
    ...
    if(RequestURL === undefined || !RequestURL || RequestURL === null){
        ...
        var link= decodeURIComponent(vlink)+"&clocale=es"+"&locale=es";
        nodeLogger.debug(nodeConfig.timestamp+"::"+nodeConfig.node+"::"+nodeConfig.nodeName+"::"+nodeConfig.script
                +"::"+nodeConfig.scriptName+"::"+link);
        var redirect = callbacksBuilder.redirectCallback(link, null, "GET");
        action.goTo("redirect");
        ...
        var link= RequestURL+"&clocale=es"+"&locale=es";
        nodeLogger.debug(nodeConfig.timestamp+"::"+nodeConfig.node+"::"+nodeConfig.nodeName+"::"+nodeConfig.script
                +"::"+nodeConfig.scriptName+"::"+link);
        var redirect = callbacksBuilder.redirectCallback(link, null, "GET");
        action.goTo("redirect");
