...
try {
    ...
    if (missingInputs.length > 0) {
        logger.error("DEBUG::" + nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + nodeConfig.missingInputParams + "::" + missingInputs);
        ...
        logger.error("kogAPITokenResponse value => "+JSON.stringify(kogAPITokenResponse))
        logger.error("kogAPITokenResponse: " + JSON.stringify(kogAPITokenResponse) + "for KYID" + kogID);

        if (kogAPITokenResponse.status === 200) {
            logger.error("access token status is 200" + kogAPITokenResponse + "for KYID" + kogID)
            ...
            if (res.status === 200) {
                ...
                if (data.ResponseStatus === 0 && data.TransactionID) {
                    ...
                    if(kogAPIResponse!=null && kogAPIResponse.ResponseStatus==0){
                        ...
                        nodeLogger.error(nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::Terminated user primary email update in KOG fail::KYID::" + kogID);
                ...
                nodeLogger.error(nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + ":: HTTP response ::" + res.status + "::KYID::" + kogID);
                auditLog("ACM002", "Account Termination Failure");
                action.goTo(nodeOutcome.FAIL);
            }
        } else {
            logger.error("kogAPITokenResponse is not 200 ");
            nodeLogger.error(nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + ":: kogAPITokenResponse is not 200 :: KYID ::" + kogID);
            auditLog("ACM002", "Account Termination Failure");
            action.goTo(nodeOutcome.FAIL);
        }
    }
...
function invokeUsrEmailUpdateKOGAPI(kogID,updateUserPrimaryEmailAPI,sihcertforapi,bearerToken,txID){
    ...
    logger.error("requestOptions in invokeUsrEmailUpdateKOGAPI() =>"+JSON.stringify(requestOptions))
     
    if(updateUserPrimaryEmailAPI!=null && requestOptions!=null){
        try{
            res = httpClient.send(updateUserPrimaryEmailAPI, requestOptions).get();
            logger.error("Result of the updateUserPrimaryEmail API =>" + res.text());
