...
function main() {
    try {
        ...
        if (!userIdentity) {
            nodeLogger.error("AUDIT::" + nodeConfig.timestamp + "::userIdentity is null or undefined.");
            action.goTo(NodeOutcome.FAILED);
        }
        // var attributes = [
        ...
        var identity_attributes = ["suffix", "givenName", "middleName", "sn", "gender", "dob", "addressLine1", "addressLine2", "city", "stateCode", "zip", "countryCode", "countyCode", "lastVerificationDate", "custom_title", "zipExtension"];
        var identity_attributes2 = ["custom_suffix", "givenName", "custom_middleName", "sn", "custom_gender", "custom_dateofBirth", "postalAddress", "custom_postalAddress2", "city", "stateProvince", "postalCode", "country", "custom_county", "lastVerificationDate", "custom_title", "zipExtension"];
        ...
        var auditData = auditDetails.getAuditDetails("UPDATE", nodeState)
            ...
            logger.error("oldValue in alpha_user:: " + oldValue)
            logger.error("oldValue in alpha_user::  " + newValue)
            ...
            // });
             ...
             patchArray.push({
                ...
                value: nodeState.get("languagePreference")|| "1"
            });
     ...
     identity_patchArray.push({
        ...
        if (patchArray.length > 0) {
            ...
            nodeLogger.error("AUDIT::" + nodeConfig.timestamp + ":: Patched Attributes in alpha_user: " + JSON.stringify(patchArray));
        } else {
            nodeLogger.error("AUDIT::" + nodeConfig.timestamp + ":: No attribute changes detected, skipping patch.");
        }

        logger.error("identity_patchArray is :: " + JSON.stringify(identity_patchArray))
        logger.error("identity_patchArray is :: " + userIdentity)
        if (identity_patchArray.length > 0) {
            ...
            nodeLogger.error("AUDIT::" + nodeConfig.timestamp + ":: Patched Attributes in user_identity: " + JSON.stringify(response));
        } else {
            nodeLogger.error("AUDIT::" + nodeConfig.timestamp + ":: No attribute changes detected, skipping patch.");
        }
