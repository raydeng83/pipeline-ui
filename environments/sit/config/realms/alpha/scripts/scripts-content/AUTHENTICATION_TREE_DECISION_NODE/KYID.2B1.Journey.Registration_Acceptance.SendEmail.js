...
function sendMail(hotp, mail, givenName, sn, phoneContact, emailContact) {
    logger.error("invoking sendEmail method")
    ...
    try {
        ...
        params.to = mail;
        logger.error("email target is: " + mail)
        params._locale = lang;
        params.object = {
            "givenName": givenName,
            "sn": sn,
        ...
        var sendEmailResponse = openidm.action(nodeConfig.idmEndpoint, nodeConfig.idmAction, params);
        logger.error("send email response is: " + sendEmailResponse)
...
try {
    logger.error("Inside KYID.2B1.Journey.Registration_Acceptance.SendEmail script")
...
try {
    ...
    if (nodeState.get("primary_secondary_email") == true && nodeState.get("primary_secondary_email") != null) {
        ...
        if (nodeState.get("primaryEmailComplete") === true || (nodeState.get("resend_alternate_mail") === true && nodeState.get("resend_alternate_mail") != null)) {
            ...
            logger.error("sendemail response 1" + sendemail);
            if (sendemail == true) {
                outcome = "success";
            } else {
                outcome = "failure";
            ...
            logger.error("inside primaryEmailSent false")
            var email = nodeState.get("mail");
            logger.error("email 2 : " + email);
            var hotp = nodeState.get("oneTimePassword");
            logger.debug("hotp : " + hotp);
            nodeState.putShared("primaryemailhotp", hotp);
            var sendemail = sendMail(hotp, email, givenName, sn, phoneContact, emailContact);
            logger.error("sendemail response 2" + sendemail);
        ...
        logger.error("primary_secondary_email false in send primary mail")
