...
function getDynamicContentEndpointId(widgetId) {
    try {
        ...
        if (!widObj || !widObj.dynamicContentServiceEndpointId) {
            logger.debug("No dynamicContentServiceEndpointId linked with widget: " + widgetId);
            return null;
        }

        // Get the endpoint ID from the widget
        var dynamicSERespID = widObj.dynamicContentServiceEndpointId._refResourceId;
        logger.debug("Found dynamicSERespID: " + dynamicSERespID);
        ...
        logger.debug("dynamicSERespIDResp: " + JSON.stringify(dynamicSERespIDResp));

        if (dynamicSERespIDResp && dynamicSERespIDResp.name) {
            var dynamicSERespIDRespName = dynamicSERespIDResp.name;
            logger.debug("dynamicSERespIDRespName: " + dynamicSERespIDRespName);
            return dynamicSERespIDRespName;
        } else {
            logger.debug("Endpoint found but 'name' attribute is missing or null for ID: " + dynamicSERespID);
        }
 ...
 */
function formatJSON(businessAppName, businessAppLogo, matchedRoles, applicationData,businessAppDisplayName) {
    try {
    var roleList = [];
    ...
    for (var i = 0; i < matchedRoles.length; i++) {
        ...
        roleList.push({
            ...
            content: content,
            isDelegable: response.isDelegable,
            isForwardDelegable: response.isForwardDelegable,
            isOrgScopedRole: response.isOrgScopedRole
        });
    }
    ...
    if(businessAppDisplayName && businessAppDisplayName!==null){
        finalOutput["businessAppDisplayName"] = businessAppDisplayName || ""
    }
...
function getApphelpdesk(appId) {
    try {
        ...
        var result = {
            application: {
                helpDeskContactId: applicationHelpdeskContactID || null,
...
function filterRemovableRoles(matchedRoleIds, userKyAccountType) {
  ...
  for (var i = 0; i < matchedRoleIds.length; i++) {
        ...
        // Check if role is org-scoped, if so, NOT removable
        if (policyResp && Boolean(policyResp.isOrgScopedRole)) {
            logger.error("Role is org-scoped, NOT removable: " + roleId);
            rolesWithFlags.push({
                roleId: roleId,
