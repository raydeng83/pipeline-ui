...
(function () {
    ...
    logger.error("===REQUEST in enrollment_v2B=== " + JSON.stringify(request));
...
function isUserPrerequsitePresent(action, enrollmentRequestId) {
    try {
        ...
        logger.error("Error Occurred in isUserPrerequsitePresent - " + error)
        return null
    }
        ...
        if (getEnrollmentResponse !== null && getEnrollmentResponse.resultCount > 0) {
            ...
            if (userPrereCancelResponse) {
                ...
                if (cancelEnrollmetReqResponse && cancelEnrollmetReqResponse._id) {
                    // Get user details, app name and role name for email
                    ...
                    const roleName = getEnrollmentResponse.result[0].roleContext && getEnrollmentResponse.result[0].roleContext[0]
                        ? getEnrollmentResponse.result[0].roleContext[0].roleName : "";

                    // Cancel workflow with basic email details (user-initiated cancellation)
            ...
            enrollmentRequestIdArray.forEach(enrollmentRequestId => {
                ...
                if (getEnrollmentResponse !== null && getEnrollmentResponse.resultCount > 0) {
                    ...
                    if (userPrereCancelResponse) {
                      ...
                      roleName2 = getEnrollmentResponse.result[0].roleContext[0].roleName;
...
logger.error(" Enrollment cancel request email : app name "+appName2 +" Date :"+new Date().toISOString()+" reason :"+payload.confirmation.reason+" givenName :"+userDetails.givenName+" sn :"+userDetails.sn+" mail :"+userDetails.mail);
                      ...
                      let languageMap = identityServer.getProperty("esv.language.preference");
                       var locale = "en";
                        if(userDetails && userDetails.custom_languagePreference){
                          let langPref = userDetails.custom_languagePreference || "1"
                              if(languageMap && languageMap!=null){
                                locale = getLangCode(langPref,languageMap);
                              }
                        }
                        logger.error("Cancel Enrollment Enduser Locale is "+locale)

                        sendEmail(appName2, roleName2, easternTimeStamp, payload.confirmation.reason, userDetails.givenName, userDetails.sn, userDetails.mail,"kyid2B1EnrollmentCancelToEndUser",locale) ;
                        // Send Emaill to Requester
                        if(requesterUserId && requesterUserId !== userDetails._id){
                            ...
                            let languageMap = identityServer.getProperty("esv.language.preference");
                             var locale = "en";
                              if(RequesterUserInfo && RequesterUserInfo.custom_languagePreference){
                                let langPref = RequesterUserInfo.custom_languagePreference || "1"
                                    if(languageMap && languageMap!=null){
                                      locale = getLangCode(langPref,languageMap);
                                    }
                              }
                          logger.error("Cancel Enrollment Requester Locale is "+locale)
                          sendEmail(appName2, roleName2, easternTimeStamp, payload.confirmation.reason, RequesterUserInfo.givenName, RequesterUserInfo.sn, userDetails.mail,"kyidRequestCancelled",locale,userDetails.mail) ;
                        }
        ...
        for (let j = 0; j < response.result.length; j++) {
            ...
            if (businessApp._id != queryFilter.businessAppId) {
                ...
                for (let i = 0; i < userPrerequisiteList.length; i++) {
                    if(userPrerequisiteList[i].preRequisiteId){  //Fix for TFS# 211435
    ...
    try {
        ...
        for (let i = 0; i < currentActiveEnrollments.length; i++) {
            ...
            var userDetails = getUserDetails(currentEnrollment.requestedUserId);
              //Defect# 204746 | Added Filter query => recordState eq "ACTIVE" or recordState eq "0" | Rajat: 11/13
            var userPrerequisiteListResponse = openidm.query("managed/alpha_kyid_enrollment_user_prerequisites/", {
                    "_queryFilter": '/enrollmentRequestId/ eq "' + currentEnrollment._id + '"'+ ' AND (recordState eq "ACTIVE" or recordState eq "0" )'   
                }, ["*", "*_ref/*"]);

            // Ben TFS-209980: Skip enrollments with no prerequisites
            //if (!userPrerequisiteListResponse || userPrerequisiteListResponse.result.length === 0) {
            //    logger.error("Skipping enrollment " + currentEnrollment._id + " - No prerequisites found for role: " + roleIds[0]._id);
            ...
            var dataEntry = {
              ...
              "applicationDisplayName":{"en":"","es":""}
            }
            if(businessApp.content && businessApp.content[0].title && businessApp.content[0].title.en){
              dataEntry.applicationName = businessApp.content[0].title.en
              dataEntry.applicationDisplayName = businessApp.content[0].title
            }
          else{
            dataEntry.applicationName = businessApp.name
            dataEntry.applicationDisplayName = {"en":businessApp.name,"es":businessApp.name}
          }
...
function getDelegatedEnrollments(queryFilter) {
    logger.error("kyid enrollment getDelegatedEnrollments");
    try {
        var dataArray = [];
        ...
        logger.error("kyid enrollment getDelegatedEnrollments response: " + response);

        var currentActiveEnrollments = response.result;
        for (let i = 0; i < currentActiveEnrollments.length; i++) {
            var currentEnrollment = currentActiveEnrollments[i];
            logger.error("kyid enrollment getDelegatedEnrollments currentEnrollment: " + currentEnrollment);
            ...
            var associatedPrerequisites = [];
                if (userPrerequisiteListResponse && userPrerequisiteListResponse.result.length > 0) {
                    var userPrerequisiteList = userPrerequisiteListResponse.result;
                    logger.error("kyid custom endpoint enrollment getDelegatedEnrollments userPrerequisiteList: " + userPrerequisiteList);
            ...
            var businessApp = businessAppResponse.result[0];
            logger.error("DEBUG getTagNamesFromRole: businessApp name=" + businessApp.name + ", roleIds[0].tags=" + JSON.stringify(roleIds[0].tags));
            ...
            var dataEntry = {
                ...
                "tagName": roleIds[0].tags ? getTagNamesFromRole(roleIds[0].tags) : [],
                "creationDate": currentEnrollment.createDate,
                "expiryDate": currentEnrollment.expiryDate,
                "associatePrerequisites": associatedPrerequisites,
                "id": currentEnrollment._id
            ...
            logger.error("DEBUG getTagNamesFromRole: dataEntry.tagName = " + JSON.stringify(dataEntry.tagName));
            dataArray.push(dataEntry);



        }
        logger.error("DEBUG getTagNamesFromRole: final dataArray = " + JSON.stringify(dataArray));
        let returnPayload = {
            "responseCode": 0,
            "transactionId": request.content.transactionId,
            "message": {
        ...
        logger.error("DEBUG getTagNamesFromRole: returnPayload = " + JSON.stringify(returnPayload));
        return returnPayload;
    } catch (error) {
...
/**
 * @name getTagNamesFromRole
 * @description Method extracts tag names from a role object's tags array
 *
 * @param {Array} tags - The tags array from a role object (with _refResourceId)
 * @returns {Array} Array of tag names with displayName containing en/es localized values
 */
function getTagNamesFromRole(tags) {
    logger.error("DEBUG getTagNamesFromRole: input tags = " + JSON.stringify(tags));
    var tagNames = [];

    if (!tags || !Array.isArray(tags) || tags.length === 0) {
        logger.error("DEBUG getTagNamesFromRole: tags is empty or invalid");
        return tagNames;
    }

    // Build query for all tag IDs
    var tagIds = [];
    ...
    logger.error("DEBUG getTagNamesFromRole: tagIds = " + JSON.stringify(tagIds));

    if (tagIds.length === 0) {
        logger.error("DEBUG getTagNamesFromRole: no valid tagIds found");
        return tagNames;
    }
    ...
    if (tagIds.length === 1) {
        ...
        for (var j = 0; j < tagIds.length; j++) {
            orParts.push("_id eq " + tagIds[j]);
        }
        tagQuery = orParts.join(" or ");
    }
    logger.error("DEBUG getTagNamesFromRole: tagQuery = " + tagQuery);

    // Query all tags at once
    var tagResults = openidm.query("managed/alpha_kyid_tag", {
        "_queryFilter": tagQuery
    });
    logger.error("DEBUG getTagNamesFromRole: tagResults = " + JSON.stringify(tagResults));

    if (tagResults && tagResults.result) {
        for (var k = 0; k < tagResults.result.length; k++) {
            var tagObj = tagResults.result[k];
    ...
    logger.error("DEBUG getTagNamesFromRole: returning tagNames = " + JSON.stringify(tagNames));
    return tagNames;
}
                ...
                request: {
                    custom: {
                        requesterUser: {
                            ...
                            roleName: emailDetail.Role_name || ""
                        }
                    }
                },
                timeStamp: emailDetail.Timestamp || new Date().toISOString(),
                reason: emailDetail.Reason || "",
                cancelledBy: emailDetail.CancelledBy || "",
                comment: emailDetail.Comment || ""
            };
...
function cancelWorkflowRequest(enrollmentRequestId, requestorEmail, applicationName, roleName, cancellationDetails) {
  try {
      logger.info("Cancelling workflow request for enrollment: " + enrollmentRequestId + ", application: " + applicationName);

      // Handle both old-style string reason and new-style object
      var reason, cancelledBy, comment;
      if (typeof cancellationDetails === 'object' && cancellationDetails !== null) {
          reason = cancellationDetails.reason || "";
          cancelledBy = cancellationDetails.cancelledBy || "";
          comment = cancellationDetails.comment || "";
      } else {
          // Backward compatibility: if string is passed, use it as reason
...
function cancelWorkflowForHelpdesk(enrollmentRequestId, enrollmentRequest, helpdeskOperatorId, reason, comment) {
    try {
        ...
        if (enrollmentRequest.appSystemName && enrollmentRequest.appSystemName.length > 0) {
            ...
            roleName = enrollmentRequest.roleContext[0].roleName;
        }
        ...
        // Pass separate fields for email template: reason, cancelledBy, comment
        var cancellationDetails = {
            reason: reason || "",
            cancelledBy: "Helpdesk",
            comment: comment || ""
        };

        // Call cancelWorkflowRequest to cancel IGA workflow and send emails to approvers
        cancelWorkflowRequest(enrollmentRequestId, requestorEmail, applicationName, roleName, cancellationDetails);
