...
if (endpointExecution && endpointExecution === "true") {
    ...
    var moName = "alpha_kyid_enrollment_access_policy"
    ...
    if (enableAuditLogHistory && enableAuditLogHistory === "true") {
        try {
            ...
            //logger.error("Response from Audit History endpoint for create event on alpha_kyid_enrollment_access_policy object:::: => " + JSON.stringify(response));
        } catch (error) {
            logger.error("An error occurred while updating the CREATE event audit History events for alpha_kyid_enrollment_access_policy object");
        }
    }
}