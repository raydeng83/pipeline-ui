...
try {  
     var lang = nodeState.get("localeLoginUrl").toString();
     if(lang.startsWith("\"")) {
        lang = lang.substring(1,lang.length-1);
     }
     var finalLoginUrl = nodeState.get("finalLoginUrl").toString();
     if(finalLoginUrl.startsWith("\"")) {
        finalLoginUrl = finalLoginUrl.substring(1,finalLoginUrl.length-1);
     }
     var kogurl = finalLoginUrl+"&locale="+lang+"&clocale="+lang;
     logger.error("Final_Login_URL:"+kogurl)
     nodeLogger.error(nodeConfig.timestamp+"::"+nodeConfig.node+"::"+nodeConfig.nodeName+"::"+nodeConfig.script
                 +"::"+nodeConfig.scriptName+"::"+kogurl); 
      config = {
        "failureUrl": kogurl
      }
