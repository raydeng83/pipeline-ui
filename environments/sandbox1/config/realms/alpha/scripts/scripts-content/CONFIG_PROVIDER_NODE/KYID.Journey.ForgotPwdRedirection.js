...
// Node Config
 var nodeConfig = {
    ...
    node: "Node",
    nodeName: "Config Provider: Forgot Pwd",
    script: "Script",
    scriptName: "KYID.Journey.ForgotPwdRedirection",
    timestamp: dateTime,
...
var clocale = nodeState.get("clocale").toString();
logger.error("Printing the return value of getLocale function ::::::::: " + clocale);

if(clocale.startsWith("\"")) {
    clocale = clocale.substring(1,clocale.length-1);
}

try {
   
    /*var email = nodeState.get("email").toString();
    if(email.startsWith("\"")) {
        email=email.substring(1,email.length-1);                
    }*/
    var tokenizedEmail = nodeState.get("tokenizedEmail").toString();
    if(tokenizedEmail.startsWith("\"")) {
        tokenizedEmail=tokenizedEmail.substring(1,tokenizedEmail.length-1);                
    }
    var url = systemEnv.getProperty("esv.kyid.tenant.fqdn");
    var kogurl = url + "/am/XUI/?realm=alpha&authIndexType=service&authIndexValue=kyid_forgotPasswordMain" + "&clocale=" + clocale + "&locale=" + clocale + "&email=" + encodeURIComponent(tokenizedEmail);
    logger.error("kogurl:"+kogurl)
    //var kogurl = url + "/am/XUI/?realm=alpha&authIndexType=service&authIndexValue=kyid_forgotPasswordMain" + "&clocale=" + clocale + "&locale=" + clocale + "&email=" + encodeURIComponent(email);
    //var kogurl = "https://sso.pilot1.kyid.ky.gov/am/XUI/?realm=alpha&authIndexType=service&authIndexValue=test-LanguageKYID-ForgotPassword" + "&clocale=" + clocale + "&locale=" + clocale + "&email=" + email;
    //logger.error("Printing the return value of getLocale function ::::::::: " + clocale);
    config = {
        "failureUrl": kogurl
    }
