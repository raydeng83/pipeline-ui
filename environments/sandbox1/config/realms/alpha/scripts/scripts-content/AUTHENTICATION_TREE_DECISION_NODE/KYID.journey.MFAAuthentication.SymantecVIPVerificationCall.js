...
var nodeConfig = {
    ...
    script: "Script",
    scriptName: "KYID.journey.MFAAuthentication.SymantecVIPVerificationCall",
    timestamp: dateTime,
...
function getCredID() {
    ...
    if(nodeState.get("KOGID") && nodeState.get("KOGID")!=null){
        nodeLogger.error("Printing KOGID for enduser"+ nodeState.get("KOGID"));
    ...
    //to handle when user has only one symantec cred registered
     if (mfaValueArray.length == 1) {
        nodeLogger.error("Only one Symantec ID found, navigating to singlesymantec."+mfaValueArray[0]);
        ...
        logger.error("***********Symnatec Array Config: "+String(symantecIDs).replace(/,/g,''));
        ...
        if (selectedOutcome === 0) {
            if (securityCode) {
                if(getCredID().localeCompare("single")==0){
                    ...
                    nodeLogger.error("AUDIT::" + nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + nodeConfig.begin +"Symantec Transcation ID " + nodeState.get("Id"))
                    ...
                    nodeLogger.error("AUDIT::" + nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + nodeConfig.begin + "Inside single");
                    action.goTo(NodeOutcome.SINGLE_SYMANTEC);
                } else {
                    var SymantecTransId =generateRandomCode();
                    nodeState.putShared("Id", SymantecTransId);
                    nodeLogger.error("AUDIT::" + nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + nodeConfig.begin +"Symantec Transcation ID " + nodeState.get("Id"))
                    nodeLogger.error("AUDIT::" + nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + nodeConfig.begin + "Inside multiple");
                    nodeState.putShared("errorMessage", null);
                    nodeState.putShared("errorMessage_BlankOTP",null);
                    nodeState.putShared("anotherFactor",null);
                ...
                nodeLogger.error("AUDIT::" + nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + nodeConfig.begin + "Security Code is Empty");
                nodeState.putShared("errorMessage", null);
                nodeState.putShared("anotherFactor",null);
