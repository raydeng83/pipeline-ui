...
var nodeConfig = {
  ...
  accessTokenStateField: "idmAccessTokenOne",
  maxAttempts: 3,
  scope: "fr:idm:*",
  serviceAccountClientId: "testAccess",
  jwtValiditySeconds: 1800,
};
...
function getAccessToken(accountId, privateKey, tenantFqdn, maxAttempts) {
  ...
  nodeLogger.error("Got assertion JWT " + assertionJwt);
...
(function () {
  try {
    ...
    nodeLogger.error("Success - adding token to transient state");
    
    nodeState.putShared(nodeConfig.accessTokenStateField, accessToken);
    nodeLogger.error("*************** Get Access Token SCript New ****************" + accessToken);
