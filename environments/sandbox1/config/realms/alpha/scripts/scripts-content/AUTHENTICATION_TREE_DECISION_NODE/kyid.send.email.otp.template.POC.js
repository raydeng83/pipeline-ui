...
var config = {
  ...
  */
  ...
  function sendMail(accessToken, fqdn, hotp, mail, givenName,sn) {
    ...
    logger.debug(config.nodeName + ": Sending templated email via the IDM email service for user: "  + " with mail address: " + mail + " and HOTP: " + hotp + " and template: " + config.templateID + "GivenName" + givenName);
  ...
  try{
    ...
    if (response.status === 200) {
        logger.error(config.nodeName + ": Email sent for user: "  + " with email: " + mail);
        return NodeOutcome.PASS;
    }
    else if (response.status === 401) {
        logger.error(config.nodeName + ": Access token is invalid: " + response.status+ " for user: " );
        return NodeOutcome.ERROR;
    }
 ...
 var mail = userProfile.mail;
  ...
  outcome = NodeOutcome.FAIL;
    ...
    logger.error(config.nodeName + ": Unable to retrieve sn attribute from the idRepository");
  outcome = NodeOutcome.FAIL;
  }
  ...
  //Execute function to send mail via IDM EMail Service
    ...
    outcome = sendMail(accessToken, fqdn, hotp, mail, givenName,sn);
  }