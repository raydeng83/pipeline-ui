...
var nodeConfig = {
    ...
    script: "Script",
    scriptName: "KYID.Journey.MFAAuthentication.DisplayMessageToUserToSendEmailOTP",
    timestamp: dateTime,
...
function createArray(userEmail) {
    var mfaOptionsArray = []
     logger.error(" :::::::: Prinintg Email from node state ::::::::: "+ nodeState.get("mail"));
    ...
    try {
        if (callbacks.isEmpty()) {
            ...
            if (selectedOutcome === 0) {
                ...
                logger.error("Email display message to the user, selected outcome send me an email  ******************** " )
...
function main(){
    try {
        ...
        if(nodeState.get("mail") && nodeState.get("mail")!=null){
             userEmail = nodeState.get("mail");
        }
        logger.error("***********userEmail in main is: "+userEmail);
        var userId = getUserId();
        logger.error("Printing the user Id " + userId)
        if (userId) {
            var userData = fetchUserData(userId)
            nodeLogger.error(nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + nodeConfig.begin + "Printing the user data inside Main execution" + userData);
            var displayMessage = createArray(userEmail);
            nodeLogger.error(nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + nodeConfig.begin + "Printing the shared state of the user ::::: " + userEmail);
