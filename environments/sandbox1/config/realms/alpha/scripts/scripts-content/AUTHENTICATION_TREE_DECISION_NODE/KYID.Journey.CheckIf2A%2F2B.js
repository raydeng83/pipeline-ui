...
function fetchdescriptionFromUserStore(_id) {
  try {
    ...
    nodeLogger.error(nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" +nodeConfig.inFunction);
...
function fetchMailFromUserStore(_id) {
  try {
    ...
    if (userQueryResult.result && userQueryResult.result.length > 0) {
      ...
      logger.error("mail found" + userMail);
      return userMail;
    } else {
      logger.error("mail found" + null);
      return null;
    }
  } catch (error) {
...
function accountStatus(_id) {
  try {
    ...
    nodeLogger.error(nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" +nodeConfig.accountStatus);
...
if (accountStatus === "Inactive") {
  nodeLogger.error(nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + transactionId +"::"+ nodeConfig.userLocked+"::"+username);
