/**
* Function: SelfRegistrationCreateUserInAIC_v2.updated
...
var nodeConfig = {
    ...
    script: "Script",
    scriptName: "SelfRegistrationCreateUserInAIC_v2",
    timestamp: dateTime,
...
var nodeConfig = {
    ...
    node: "Node",
    nodeName: "Create User in AIC",
    scriptName: "SelfRegistrationCreateUserInAIC_v2.updated",
    script: "Script",
    timestamp: dateTime,
...
try {
    ...
    // Logging input data
    nodeLogger.debug("User Data: " + JSON.stringify(jsonObj));

    // Attempt user creation
    var createUserResponse;
    if (!phoneNumber) {
        nodeLogger.debug("Phone number not provided, creating user without phone.");
        createUserResponse = openidm.create("managed/alpha_user", null, jsonObj1);
    } else {
        nodeLogger.debug("Phone number provided, creating user with phone.");
        createUserResponse = openidm.create("managed/alpha_user", null, jsonObj);
    }

    nodeLogger.debug("User Created Successfully: " + JSON.stringify(createUserResponse));

    // MFA Registration
    if (createUserResponse) {
        nodeLogger.debug("Attempting to register MFA for user: " + usrEmailAddress);
        if (usrEmailAddress && !lookupInMFAObject(usrKOGID, usrEmailAddress)) {
            nodeLogger.debug("MFA Email Registration in Progress");
