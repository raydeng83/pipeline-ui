...
function getEnrolmentPrerequisiteTypes(id) {
    ...
    try { 
        logger.error("Started getEnrolmentPrerequisiteTypes");
        recordEnrolmentContext = crudOps.crudOps("query", "managed/Alpha_Kyid_EnrolmentContext", null, "contextID eq '"+id+ "'", ["contextID", "role"], null, null);
...
function getRoleDetails(id) {
   ...
   var policyID = null;
   try { 
        recordRole = crudOps.crudOps("query", "managed/alpha_role", null,  "_id eq '"+id+ "'", ["_id", "name", "_policy"], null, null);
...
function getPolicyDetails(id) {
   ...
   try { 
        ...
        recordPolicy = crudOps.crudOps("query", "managed/Alpha_Kyid_Policy", null,   "_id eq '"+id+ "'",  ["_id", "policyName", "_prereqtypes"], null, null);
...
function getListOfPrereqTypes(arrPreReqTypeIDs){
  ...
  try { 
        for(var j=0;j<arrPreReqTypeIDs.length;j++){
          ...
          recordPrereqTypes = crudOps.crudOps("query", "managed/Alpha_Kyid_PrerequisiteType", null,   "_id eq '"+id+ "'",  ["_id", "name", "isSave"], null, null);
