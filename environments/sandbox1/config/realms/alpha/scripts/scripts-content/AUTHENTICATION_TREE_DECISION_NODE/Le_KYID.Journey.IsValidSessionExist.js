    ...
    if (cookieHeader && cookieHeader.get(0)) {
        ...
        for (var i = 0; i < rawCookieParams.length; i++) {
            ...
            if (cookie.localeCompare(appCookieName) == 0) {
                logger.error("Found matching cookie: " + cookieName[0]);
                ...
                if (cookieName.length > 1) {
                    ...
                    logger.error("Matched cookie value: " + matchedCookieValue);
                } else {
                    logger.error("Cookie value for " + targetCookieName + " is empty or malformed.");
                }
            }
         }
      }  else {
            logger.error("No cookies found in request headers");
    }
   //} 
...
function main(){
    ...
    logger.error("DEL-Test:: Error Logs from KYID.Journey.IsValidSessionExist");
        logger.debug("DEL-Test:: Debug Logs from KYID.Journey.IsValidSessionExist");
        logger.info("DEL-Test:: INFO Logs from KYID.Journey.IsValidSessionExist");
    try {
        createRequestURL();
