...
var nodeConfig = {
    ...
    missingwindowsAccName:"Missing windowsaccountname",
...
if (nodeState.get("windowsAccName") != null) {
...
/*if (nodeState.get("windowsaccountname") != null) {
     windowsaccountname = nodeState.get("windowsaccountname");
     var jsonObj = {};
     jsonObj["operation"] = "replace";
     jsonObj["field"] = "custom_logon";
     jsonObj["value"] = windowsaccountname;
     jsonArray.push(jsonObj);
}  else {
     missingInputs.push(nodeConfig.missinglogonValue);
}
*/
...
if(missingInputs.length>0){
    ...
    try{ 
        ...
        if (usrEmailAddress != null && typeof usrEmailAddress != "undefined") {
            // logger.error("Naren Logs :: MFA_ID is "+ MFA_ID)
            logger.error("Naren Logs Inside If :: Line 308 ")
...
function updateMFAObject(usrKOGID, method, usrMfaValue, status,MFA_ID) {
    ...
    logger.error("Values are :: "+usrKOGID+method+ usrMfaValue+status+MFA_ID)
    // var mfajsonObj = {};
    ...
    openidm.patch("managed/alpha_kyid_mfa_methods/" + MFA_ID, null, [{"operation": "replace","field": "MFAValue","value": usrMfaValue }]);
}
