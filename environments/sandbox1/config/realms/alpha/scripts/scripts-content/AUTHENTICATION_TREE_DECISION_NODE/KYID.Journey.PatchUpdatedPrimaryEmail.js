var dateTime = new Date().toISOString();

// Node Config
var nodeConfig = {
    begin: "Begining Node Execution",
    node: "Node",
    nodeName: "Patch NewEmail in UserProfile",
    script: "Script",
    scriptName: "KYID.Journey.PatchUpdatedPrimaryEmail",
    timestamp: dateTime,
    end: "Node Execution Completed"
};

var NodeOutcome = {
    SUCCESS: "true",
    FAILED: "false"
  };

var nodeLogger = {
    // Logs detailed debug messages for troubleshooting  
    debug: function (message) {
        logger.debug(message);
    },
    // Logs Error that can impact Application functionality
    error: function (message) {
        logger.error(message);
    }
  };

try {
    // Retrieve user ID and check if it's valid
    var userId = nodeState.get("_id");
    //var userId = "ace845e3-d921-44cc-8b4e-13ba26a39065";
    if (!userId) {
        nodeLogger.error("AUDIT::" + nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + nodeConfig.begin + " Error: User ID is null or undefined.");
        action.goTo(NodeOutcome.FAILED);
    }

    // Retrieve telephone number and check if it's valid
    var newemail1 = nodeState.get("newemail1");
    if (newemail1 === null || newemail1 === undefined) {
        nodeLogger.error("AUDIT::" + nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + nodeConfig.begin + " Error: NewEmail is null or undefined.");
        action.goTo(NodeOutcome.FAILED);
    }



    // Perform the patch operation to update the user's telephone number
    var result = openidm.patch("managed/alpha_user/" + userId, null, [{"operation": "add","field": "mail","value": newemail1 }]);

    // Check if the patch operation was successful
    if (result) {
        nodeState.putShared("MFAMethod","EMAIL")
        action.goTo(NodeOutcome.SUCCESS);
    } else {
        nodeLogger.error("AUDIT::" + nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + nodeConfig.begin + " Error: Failed to patch newemail for user ID: " + userId);
        action.goTo(NodeOutcome.FAILED);
    }
} catch (error) {
    nodeLogger.error("AUDIT::" + nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + nodeConfig.begin + " Error in main execution: " + error.message);
    action.goTo(NodeOutcome.FAILED);
}
