...
var nodeConfig = {
    ...
    script: "Script",
    scriptName: "KYID.Journey.MFARegistration.DisplayNotRegisteredMFAMethods.Self",
    timestamp: dateTime,
...
function availableMFAOptions(){
    ...
    //The context will be 3 for Self Journey -- This is for Self and Recovery Flow.
...
function processCallbacks(mfa) {
    ...
    try {
        ...
        var setupLabel = clocale === "es" ? "Verificar la identidad con un método de seguridad" : "Set Up";
        if (callbacks.isEmpty()) {
            ...
            if (selectedOutcome === 0) {
                ...
                if (selectedMFAOption.includes(defaultMsgen) || selectedMFAOption.includes(defaultMsges)) {
                    ...
                    nodeState.putShared("setupAdditionalSecurityMethods", null);
                    nodeState.putShared("blankmsg",null);
    ...
    try {
        ...
        if (userId) {
            ...
            if (userData) {
                ...
                var setMFAContext = nodeState.get("setMFAContext")
                //The context will be 3 for Self Journey
                var MFAContextCode = 3;
