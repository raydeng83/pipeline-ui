...
logger.error("ExternalDomain:"+extDomain)
logger.error("DomainValue:"+domain)
...
if(systemEnv.getProperty("esv.kyid.ext.connector") && systemEnv.getProperty("esv.kyid.ext.connector")!=null) {
        ...
        logger.error("ExtConnectorName:"+ConnectorName)
    ...
    if(systemEnv.getProperty("esv.kyid.int.connector") && systemEnv.getProperty("esv.kyid.int.connector")!=null) {
        ConnectorName = systemEnv.getProperty("esv.kyid.int.connector").toLowerCase(); 
        logger.error("IntConnectorName:"+ConnectorName)
        nodeLogger.error(nodeConfig.timestamp+"::"+nodeConfig.node+"::"+nodeConfig.nodeName+"::"+nodeConfig.script
                         +"::"+nodeConfig.scriptName+"::"+nodeConfig.ConnectorName+"::"+ConnectorName);
    }  else {
     missingInputs.push(nodeConfig.missingConnectorInfo);
...
if(missingInputs.length>0){
    ...
    if(result.records>0) {
        ...
        nodeLogger.error("Value of LockoutTime is: "+ldapUser.lockoutTime);
