 ...
 */
  ...
  var query = null;
  //logger.error("*queryLDAP***"+ConnectorName+UserNameAttribute+UserNameAttributeValue);
  ...
  function queryLDAP(ConnectorName, UserNameAttribute, UserNameAttributeValue) {
    ...
    //logger.error("*****values*****"+UserNameAttribute+UserNameAttributeValue+ConnectorName);
    try {
      // Query to check if user exists in AD
      nodeLogger.error(nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" +nodeConfig.queryLDAP);
      var ldapUserQuery = openidm.query(
        `system/` + ConnectorName + `/User`,
        query
      );
      records = ldapUserQuery.result.length;
      //logger.error("****records*****"+records)
