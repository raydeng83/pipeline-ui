...
(function () {
    // SETUP
logger.error("OIDC Claim Script");
 // logger.error("OIDC Session isInternalUser "+typeof session.getProperty('isinternaluser'))
  logger.error("OIDC Identity "+identity)
    /**
    ...
    utils.setScopeClaimsMap({
        profile: [
            ...
            'family_name',
            'given_name',
            'CustomSAMAccountName',
            'CustomUPN',
            'CustomADAbbreviation',
            'WindowsAccountName',
            'amr',
            'CustomIsInternal',
            'idpauthtime',
    ...
    utils.setClaimResolvers({
        ...
        */
        ...
        CustomSAMAccountName: utils.getUserProfileClaimResolver('fr-attr-istr3'),
        CustomUPN: utils.getUserProfileClaimResolver('fr-attr-istr1'),

        uniqueid: utils.getUserProfileClaimResolver('fr-attr-istr1'),
        email1: utils.getUserProfileClaimResolver('mail'),
        firstname: utils.getUserProfileClaimResolver('givenname'),
        lastname: utils.getUserProfileClaimResolver('sn'),
        
        CustomADAbbreviation: function () {
            if(session.getProperty('isinternaluser').localeCompare("true")==0)
            	return "CHFS";
            else 
                return "CIT";
        }, 
        WindowsAccountName: function () {
           return session.getProperty('windowsaccountname');
        },
        amr: function () {
           logger.error(" Session Property idpauthtype " + session.getProperty('idpauthtype'));
    
            return session.getProperty('idpauthtype').split(",");
          
        },
        CustomIsInternal: function () {
          logger.error(" Session Property isinternaluser " + session.getProperty("isinternaluser"));
 		    if(session.getProperty('isinternaluser').localeCompare("true")==0)
            	return true;
            else 
                return false;
        },
        idpauthtime: function () {
            return session.getProperty('AuthenticationInstant');
        }
