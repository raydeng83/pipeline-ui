...
function getCustomAttributes() {
    ...
    try {
        if (remoteEntityId.localeCompare(systemEnv.getProperty("esv.logout.app.url")) == 0) {
            ...
            //var givenname = "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/given_name";
            //var familyname = "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/family_name";
            var email = "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/email";

            logger.error("logger in samlidpattributemapper193 " +session.getProperty("mail"));
            customSet = new java.util.TreeSet();
            customSet.add(session.getProperty("mail"));
                attributes.add(
                idpAttributeMapperScriptHelper.createSAMLAttribute(
                    email, null, customSet));
            ...
            if (JSON.stringify(session.getProperty("appClaims")).length > 2) {
                ...
                logger.error("logger in samlidpattributemapper261 " +session.getProperty("EmailAddress"));
                logger.error("logger in samlidpattributemapper262 " +session.getProperty("FirstName"));
                logger.error("logger in samlidpattributemapper263 " +session.getProperty("LastName"));
                
                if(session.getProperty("EmailAddress") && session.getProperty("EmailAddress")!=null){
                  	var mail = session.getProperty('EmailAddress').split(",");
                    logger.error("mail: "+mail.toString());
                    var customSet = new java.util.TreeSet();
                    for (var i = 0; i < mail.length; i++) {
                      var type = mail[i];
                      customSet.add(type.toString());
                    }
                    attributes.add(idpAttributeMapperScriptHelper.createSAMLAttribute("http://schemas.chfs.ky.gov/kog/v1/identity/claims/mail", null, customSet))
                }

                if(session.getProperty("FirstName") && session.getProperty("FirstName")!=null){
                  	var FirstName = session.getProperty('FirstName').split(",");
                    logger.error("FirstName: "+FirstName.toString());
                    var customSet = new java.util.TreeSet();
                    for (var i = 0; i < FirstName.length; i++) {
                      var type = FirstName[i];
                      customSet.add(type.toString());
                    }
                    attributes.add(idpAttributeMapperScriptHelper.createSAMLAttribute("http://schemas.chfs.ky.gov/kog/v1/identity/claims/FirstName", null, customSet))
                }

                if(session.getProperty("LastName") && session.getProperty("LastName")!=null){
                  	var LastName = session.getProperty('LastName').split(",");
                    logger.error("LastName: "+LastName.toString());
                    var customSet = new java.util.TreeSet();
                    for (var i = 0; i < LastName.length; i++) {
                      var type = LastName[i];
                      customSet.add(type.toString());
                    }
                    attributes.add(idpAttributeMapperScriptHelper.createSAMLAttribute("http://schemas.chfs.ky.gov/kog/v1/identity/claims/LastName", null, customSet))
                }
