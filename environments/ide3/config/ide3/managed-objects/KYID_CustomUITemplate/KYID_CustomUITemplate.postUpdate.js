...
if (endpointExecution && endpointExecution === "true") {
    ...
    if (enableAuditLogHistory && enableAuditLogHistory === "true") {
        ...
        //logger.error("Response from Audit History endpoint for UPDATE event on KYID_CustomUITemplate object:::: => " + JSON.stringify(response));
        } catch (error) {
            logger.error("An error occurred while updating the UPDATE event audit History events for KYID_CustomUITemplate object");
        }
    }   
}