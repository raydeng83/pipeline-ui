...
function updatePrerequisite(requestObj) {
    ...
    logger.error("Print out user prerequisite" + userPrerequisite); // check if retrieved userPrerequisite
...
try {
    ...
    logger.info("kyid-workflow requestObj: " + JSON.stringify(requestObj));
} catch (e) {
    failureReason = "kyid-workflow error: " + e;
    logger.error("Failed to read request " + requestId + " for rejection flow: " + e);
}
...
if (!requestObj || !requestObj.decision || !requestObj.decision.phases) {
    ...
    for (var i = 0; i < phases.length; i++) {
        ...
        var isApprovalRejectPhase =
            p.displayName === "Level 1 Approval" ||
            p.displayName === "Level 1 Backup Approval" ||
            p.displayName === "Level 2 Approval" ||
            p.displayName === "Level 2 Backup Approval";


    if (isApprovalRejectPhase &&
        p.status === "complete" &&
