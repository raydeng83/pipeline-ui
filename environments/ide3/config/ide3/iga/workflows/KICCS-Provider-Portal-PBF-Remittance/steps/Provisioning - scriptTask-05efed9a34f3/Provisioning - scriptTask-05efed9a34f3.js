...
function saveUserPrerequisiteValues(requestObj) {
    ...
    var prereqTypeId = userPrerequisite.preRequisiteTypeId._refResourceId;
    ...
    if (shouldPatchUserPrerequisite) {
        var pendingPrereqResponse = openidm.query("managed/alpha_kyid_enrollment_user_prerequisites/",
            {
                "_queryFilter":  '/preRequisiteTypeId/_refResourceId eq "' + prereqTypeId + '"'
                + ' AND requestedUserAccountId eq "' + requestedUserAccountId + '"'
                    + ' AND (recordState eq "ACTIVE" OR recordState eq "0")'
            },
            ["*"]
        );
        logger.error("pending for approval prereq in getPrerequisites --> " + JSON.stringify(pendingPrereqResponse))
        // var pendingApprovalRecords = pendingPrereqResponse.result.filter(                                    
        //                                 record => record.status === "PENDING_APPROVAL"
        //                              ).map(record => record._id);

        var pendingApprovalList = pendingPrereqResponse.result
            .filter(item => item.status === "PENDING_APPROVAL")
            .map(item => ({
                enrollmentRequestId: item.enrollmentRequestId,
                _id: item._id
            }));                       

    //     pendingApprovalRecords.forEach(userPrerequisiteId => {
    //     openidm.patch("managed/alpha_kyid_enrollment_user_prerequisites/" + userPrerequisiteId, null, [{
    //             "operation": "replace",
    //             "field": "/status",
    //             "value": "COMPLETED"
    //         },
    ...
    // });
    // }
        pendingApprovalList.forEach(record => {
            openidm.patch(
                "managed/alpha_kyid_enrollment_user_prerequisites/" + record._id,
                null,
                [
                    {
                        "operation": "replace",
    ...
    var apiRequestPayloads = pendingApprovalList.map(record => ({
        requestedUserAccountId: requestedUserAccountId,
        enrollmentRequestId: record.enrollmentRequestId,
        preReqId: null
    }));
    logger.error("apiRequestPayload for getPrerequisites --> " + JSON.stringify(apiRequestPayloads))
    ...
    apiRequestPayloads.forEach(apiRequestPayload => {
        ...
        if (isPendingPrereq == false) {
            let payload = {
                requestedUserAccountId: apiRequestPayload.requestedUserAccountId,
                enrollmentRequestId: apiRequestPayload.enrollmentRequestId
            }
            logger.error("paylod before calling access endpoint --> " + JSON.stringify(payload))
