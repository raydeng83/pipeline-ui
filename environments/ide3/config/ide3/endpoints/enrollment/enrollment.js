...
(function () {
    ...
    logger.error("===REQUEST in enrollment=== " + JSON.stringify(request));
            ...
            if (getActiveEnrollmentRequest(enrollmentContextId, requestedUserAccountId)) {
                ...
                if (createEnrollmentResponse) {
                    ...
                    if (createEnrollmentResponse.message) {
                        ...
                        return {
                            "responseCode": createEnrollmentResponse.code,
                            "transactionId": context.transactionId && context.transactionId.transactionId && context.transactionId.transactionId.value ? context.transactionId.transactionId.value : "",
                            "message": {
                                "code": String(createEnrollmentResponse.code),
                                "content": createEnrollmentResponse.message,
                            }
                        }
    ...
    try {
        if (requestedUserAccountId !== null && getUserDetails(requestedUserAccountId)) {
            ...
            if (action == 1) {
                ...
                if (enrollmentContextResponse !== null && enrollmentContextResponse && enrollmentContextResponse.code !== 1 && enrollmentContextResponse.code !== 500 && enrollmentContextResponse.code !== 2 && enrollmentContextResponse.status === "0") {
                    if (enrollmentContextResponse.requestedUserAccountAttibutes && enrollmentContextResponse.requestedUserAccountAttibutes.length > 0) {
                        for (let i = 0; i < enrollmentContextResponse.requestedUserAccountAttibutes.length; i++) {
                            if (enrollmentContextResponse.requestedUserAccountAttibutes[i].attributeName.toLowerCase() === "mail" || enrollmentContextResponse.requestedUserAccountAttibutes[i].attributeName.toLowerCase() === "primaryemailaddress") {
                                requestedUserEmail = enrollmentContextResponse.requestedUserAccountAttibutes[i].attributeValue
                    ...
                    var loggedInUserInfo = JSON.parse(requestedUserDetails)
                    if (loggedInUserInfo && loggedInUserInfo.user && loggedInUserInfo.user.result && loggedInUserInfo.user.result.length > 0) {
                        loggedInUserEmail = loggedInUserInfo.user.result[0].mail
                    }
                    logger.error("requestedUserEmail from Context is --> " + requestedUserEmail)
                    logger.error("logedin User Email ID--> " + loggedInUserInfo.user.result[0].mail)
                    if (enrollmentContextResponse.requestedUserAccountId !== null && enrollmentContextResponse.requestedUserAccountId
                        ...
                        if (requestedUserAccountId === enrollmentContextResponse.requestedUserAccountId) {// validating if requested user is same as loggedin User
                            ...
                            logger.error("Dummy log6 Invalid User --> ");
                            return { code: 400, message: 'Invalid User' }
                        }


                    } else if (enrollmentContextResponse.requestedUserAccountId !== null && enrollmentContextResponse.requestedUserAccountId && requestedUserAccountId !== enrollmentContextResponse.requestedUserAccountId) {
                        logger.error("Logged in user mismatching with enrollmentContextResponse RequestedUserAccountId");
                        return { code: 400, message: 'Invalid User' }
                    }
                    else if ((enrollmentContextResponse.requestedUserAccountId == null || enrollmentContextResponse.requestedUserAccountId == "") && requestedUserEmail && loggedInUserEmail && requestedUserEmail.toLowerCase() !== loggedInUserEmail.toLowerCase()) {
                        logger.error("Logged in user mismatching with enrollmentContextResponse RequestedUserAccountId");
                        return { code: 400, message: 'Invalid User' }
                    }
                    else {
                ...
                } else if (enrollmentContextResponse.status !== "0" || enrollmentContextResponse.code === 1) {
                    logger.error("Enrollment Request is Expired Status Value is --> " + enrollmentContextResponse.status)
                    // return { code: 2, message: "The invitation link you used has expired and is no longer valid. To receive a new invitation, please contact the Helpdesk or request that a new invitation be sent." }
                    return { code: 400, message: 'Context Id is expired' }
                }
                else {
                ...
                if ((outcome && ((outcome.activeEnrollmentsWithPreReq && outcome.activeEnrollmentsWithPreReq.length > 0) || (outcome.activeEnrollmentsWithoutPreReq && (Object.keys(outcome.activeEnrollmentsWithoutPreReq).length > 0))) && (enrollmentContextResponse && enrollmentContextResponse.expiryDate && enrollmentContextResponse.expiryDate !== null && enrollmentContextResponse.expiryDate !== ""))) {
                    let requestBody = [{
                        "operation": "replace",
                        "field": "status",
                        "value": "1",
                    ...
                    patchMORecord(enrollmentContextResponse._id, "alpha_kyid_enrollment_contextId", requestBody)
                }
                const helpTopic = getFaqTopidId("appEnrollment", "enrollmentSummary")
                outcome["helpTopics"] = helpTopic
                return outcome;
            }
    ...
    try {
        ...
        if (response && response.resultCount > 0) {
            ...
            if (response.result[0].expiryDateEpoch && response.result[0].expiryDateEpoch > currentTimeinEpoch) {
                ...
                //     "message": {
                //         "code": "2",
                //         "content": "The invitation link you used has expired and is no longer valid. To receive a new invitation, please contact the Helpdesk or request that a new invitation be sent.",
                //     }
                // }
...
// function getRoleDetailsLocalMethod(action, roleId, userId, appId) {
//     try {
//         let roleMembers = [];
//         let policyResponse = null;
//         if (action === "4") {
//             if (roleId !== null) {
//                 // Read role information WITHOUT members 
//                 const role = openidm.read("managed/alpha_role/" + roleId, null, ["_id", "name", "description", "recordState", "requiredMFAMethod", "content", "businessAppId", "accessPolicy"]);

//                 logger.error("Get Role Details -- endpoint/LIB-RoleAPI Role read: " + JSON.stringify(role))

//                 if (role && role.recordState === "ACTIVE") {
//                     // Query members separately from the relationship endpoint
//                     var members = [];
//                     try {
//                         const membersResponse = openidm.query("managed/alpha_role/" + roleId + "/members", {
//                             "_queryFilter": "true"
//                         });

//                         logger.error("Get Role Details -- endpoint/LIB-RoleAPI Members query: " + JSON.stringify(membersResponse))

//                         if (membersResponse && membersResponse.result) {
//                             members = membersResponse.result;
//                         }
//                     } catch (memberError) {
//                         logger.error("Get Role Details -- endpoint/LIB-RoleAPI Members query failed: " + memberError)
//                     }


//                     role.members = members;

//                     // Wrap response in query format for compatibility with existing endpoints
//                     const formattedResponse = {
//                         resultCount: 1,
//                         result: [role]
//                     };

//                     logger.error("Get Role Details -- endpoint/LIB-RoleAPI Final response: " + JSON.stringify(formattedResponse))

//                     return formattedResponse;
//                 }
//                 else {
//                     logger.error("Get Role Details -- endpoint/LIB-RoleAPI Record not found or not active with Role ID")
//                     return { code: 404, message: 'Record not found or not active with Role ID' };
//                 }
//             }
//             else {
//                 logger.error("Get Role Details -- endpoint/LIB-RoleAPI Role ID is null")
...
// Le Deng: copied from /LIB-RoleAPI but removed '/member' to avoid returning all members
function getRoleDetailsLocalMethod(action, roleId, userId, appId) {
    try {
        let roleMembers = [];
        let policyResponse = null;
        if (action === "4") {
            if (roleId !== null) {
                const response = openidm.query("managed/alpha_role/", { "_queryFilter": '/_id/ eq "' + roleId + '"' + ' AND recordState eq "' + "ACTIVE" + '"' }, ["*"]);
...
function getRoleDetails(action, roleId, userId, appId) {
    try {
        ...
        var response = getRoleDetailsLocalMethod(action, roleId, userId, appId)
        logger.error("getRoleDetails is --> " + JSON.stringify(response))
        var endTime = new Date();
        var duration = endTime - startTime;
        var durationInSeconds = duration / 1000;
        logger.error("Performance time enrollment: " + durationInSeconds);
...
function isUserPrerequsitePresent(action, enrollmentRequestId) {
    try {
        ...
        logger.error("Error In isUserPrerequsitePresent Function " + error)
        return null
    }
            ...
            for (let i = 0; i < prereqDetails.length; i++) {
                ...
                if (prereqDetails[i].enrollmentActionSettings.triggerAlways === true) {
                    ...
                    if (prerequisiteTypeResponse) {
                        if ((prerequisiteTypeResponse.result[0].prereqTypeId.typeName === "MFA" || prerequisiteTypeResponse.result[0].prereqTypeId.typeName === "4") && (requiredMFA === "0")) {
...
function evaluateUserPrereq(requestedUserDetails, enrollmentRequestId, userId, requesterId, roleId, prereqDetails, requiredMFACode, appRoleContext, requestForEvaluation) {
    try {
        ...
        if (prereqDetails !== null && prereqDetails.length > 0) {
            if (requestForEvaluation && requestForEvaluation.length > 0) {
                requestForEvaluation.forEach(val => {
                    ...
                    roleContext.applicationId = val.applicationId || null
                    roleContext.roleId = val.roleId || null
                    roleContext.originalDelegatorUserAccountId = val.originalDelegatorUserAccountId || null
                    roleContext.currentDelegatorUserAccountId = val.currentDelegatorUserAccountId || null
                    roleContext.orgId = val.orgId || null
                    roleContext.orgType = val.orgType || null
                    roleContext.orgName = val.orgName || null
                    roleContext.orgSourceUniqueId = val.orgSourceUniqueId || null
                    roleContext.businessKeyTypeName = val.businessKeyTypeName || null
                    roleContext.businessKeyId = val.businessKeyId || null
                    roleContext.businessKeyName = val.businessKeyName || null
                    roleContext.businessKeyDescription = val.businessKeyDescription || null
                    roleContext.kogOrgBusinessKeyId = val.kogOrgBusinessKeyId || null
                    roleContext.isForwardDelegable = val.isForwardDelegable || false
                    roleContext.delegationEndDate = val.delegationEndDate || null
                    roleContext.delegationEndDateEpoch = val.delegationEndDateEpoch || null
            ...
            for (let i = 0; i < prereqDetails.length; i++) {
                ...
                // orgType matching logic: skip prereq if orgType doesn't match
                // "" and null are both treated as "no orgType"
                var contextOrgType = appRoleContext && appRoleContext.orgType ? appRoleContext.orgType.trim() || null : null;
                var prereqOrgType = prereqDetails[i].orgType ? prereqDetails[i].orgType.trim() || null : null;
                logger.error("orgType check - contextOrgType: " + contextOrgType + ", prereqOrgType: " + prereqOrgType);

                // Skip this prereq if:
                // - contextId has orgType AND
                // - prereq has orgType AND
                // - they don't match
                if (contextOrgType && prereqOrgType && contextOrgType !== prereqOrgType) {
                    logger.error("Skipping prereq due to orgType mismatch - contextOrgType: " + contextOrgType + ", prereqOrgType: " + prereqOrgType);
                    isSkipPrereq++;
                    continue;
                }
                ...
                if (prereqDetails[i].enrollmentActionSettings.triggerAlways === true) {
                    ...
                    if ((prerequisiteTypeResponse.result[0].prereqTypeId.typeName === "MFA" || prerequisiteTypeResponse.result[0].prereqTypeId.typeName === "4") && requiredMFACode === "0") { // Adding this condition if MFA is not required (MFA code as 0)
                        ...
                        if (isSkipPrereq > 0) {
                            userPrereqPayload.displayOrder = i + 1 - isSkipPrereq
                        }
                        else {
                        ...
                        if (prerequisiteTypeResponse && prerequisiteTypeResponse.result[0]._id && prerequisiteTypeResponse.result[0].name) {
                            ...
                            if (prereqDetails[i].enrollmentActionSettings.allowReuse === false) {
                                ...
                                completedPrereqResponse = getCompletedPrereq(userId, prereqDetails[i]._id, prerequisiteTypeResponse.result[0].prereqTypeId._refResourceId)
                                if (completedPrereqResponse) {
                                    ...
                                    logger.error("completedPrereqResponse.result[0].completionDateEpoch -->" + completedPrereqResponse.completionDateEpoch)
                                    let allowReuseIfDaysOld = prereqDetails[i].enrollmentActionSettings.allowReuseIfDaysOld || "0"
                                    let completionDateEpoch = completedPrereqResponse.completionDateEpoch || null
                                    let completionDate = completedPrereqResponse.completionDate || null
                                    let completedExpiryDate = completedPrereqResponse.expiryDate || null
                                    let completedExpiryDateEpoch = completedPrereqResponse.expiryDateEpoch || null
                                    if (isPrereqReusable(allowReuseIfDaysOld, completionDateEpoch) === true) {
                                        logger.error("Inside Already Completed condition" + isPrereqReusable(allowReuseIfDaysOld, completionDateEpoch))

                                        if (prerequisiteTypeResponse.result[0].prereqTypeId.typeName === "MFA" || prerequisiteTypeResponse.result[0].prereqTypeId.typeName === "4") {
                                            if (foundMFA === true) {
                                                userPrereqPayload.completionDate = completionDate
                                                userPrereqPayload.completionDateEpoch = completionDateEpoch
                                                userPrereqPayload.expiryDate = completedExpiryDateEpoch
                                                userPrereqPayload.completionDateEpoch = completionDateEpoch
                                            ...
                                            userPrereqPayload.completionDate = completionDate
                                            userPrereqPayload.expiryDate = completedExpiryDate
                                            userPrereqPayload.expiryDate = completedExpiryDateEpoch
                                            userPrereqPayload.completionDateEpoch = completionDateEpoch
                                            userPrereqPayload.status = "ALREADY_COMPLETED"
                                        }
                                    ...
                                    // else if(prerequisiteTypeResponse.result[0].prereqTypeId.prerequisiteSourceName === "Credential Prerequisite"){

                                    //     var requestedAccountId = requestedUserDetails.user.result[0]._id
                                    //     logger.error("requesterId" + requestedAccountId)
                                    //      var prereqTypeId = prerequisiteTypeResponse.result[0].prereqTypeId._refResourceId
                                    //      var pendingPrereqResponse = openidm.query(
                                    //           "managed/alpha_kyid_enrollment_user_prerequisites/",
                                    //           {
                                    //               "_queryFilter":
                                    //               '/preRequisiteTypeId/_refResourceId eq "' + prereqTypeId + '"'
                                    //               + ' AND requestedUserAccountId eq "' + requestedAccountId + '"'
                                    //               + ' AND (recordState eq "ACTIVE" OR recordState eq "0")'
                                    //           },
                                    //           ["*"]
                                    //       );
                                    //     logger.error("Credential Prerequisites response is --> " + JSON.stringify(pendingPrereqResponse))
                                    //     var pendingApprovalRecords = pendingPrereqResponse.result.filter(
                                    //         record => record.status === "PENDING_APPROVAL"
                                    //           );

                                    //     logger.error("pending approval response is --> " + JSON.stringify(pendingApprovalRecords));


                                    //     if(pendingApprovalRecords.length>0)
                                    //     {
                                    //         userPrereqPayload.status = "PENDING_APPROVAL"
                                    //     }
                                    //     else{
                                    //         userPrereqPayload.status = "NOT_STARTED"
                                    //     }
            ...
            // If no user_prerequisites were actually created (orgType skip, triggerAlways=false, or combination),
            // treat as no prerequisites needed
            if (!isUserPrereq) {
                logger.error("No user_prerequisites created (isSkipPrereq=" + isSkipPrereq + ", prereqDetails.length=" + prereqDetails.length + "), treating as no prerequisites needed")
                return "skipped_all"
            }
...
function isPrereqReusable(allowReuseIfDaysOld, completionDateEpoch) {
    try {
        if (allowReuseIfDaysOld && Number(allowReuseIfDaysOld) > 0 && completionDateEpoch) {
        ...
        else if (allowReuseIfDaysOld === "0" || allowReuseIfDaysOld === "-1") {
            return true
        }
        else {
            return false
    ...
    try {
        ...
        if (prereqId) {
            ...
            if (response && response !== null && response.resultCount > 0) {
                logger.error("getCompletedPrereq -- response  is" + JSON.stringify(response))
                // Date 11/12/2025- Narendra :- added logic to get the latest Comepleted Record
        ...
        if (roleId !== null && roleId) {
            ...
            if (roleId) {
                ...
                if (getRoleDetails("4", roleId, requestedUser, null)) {
                    ...
                    logger.error("roleResponse.result is --> " + JSON.stringify(roleResponse.result))
                    if (!roleResponse.result || roleResponse.result.length === 0) {
                        logger.error("Role not found with ID: " + roleId + ", returning error to user. roleResponse: " + JSON.stringify(roleResponse))
                        return {
                            code: 2,
                            message: "The invitation link you used has expired and is no longer valid. To receive a new invitation, please contact the Helpdesk or request that a new invitation be sent."
                        }
                    }
                    ...
                    // Le Deng: updated to query just user and role instead of checking all members of role
                    var userRoleResponse = openidm.query("managed/alpha_user/" + requestedUser + "/roles", { "_queryFilter": '_refResourceId eq "' + roleId + '"' }, ["*"]);

                    if (userRoleResponse.result.length > 0) {
                        logger.error("Role is already assigned to user with id " + requestedUser)
                        return { code: 400, message: "Role is already provisioned" }
                            ...
                            if (policyResponse) {
                                ...
                                if (policyResponse.result[0].mutuallyExclusiveRole !== null && policyResponse.result[0].mutuallyExclusiveRole.length > 0) {
                                    ...
                                    if (userRoles !== null && userRoles.length > 0) {
                                        ...
                                        if (commonRoles.length > 0) { // evaluating mutually exclusive condtion
                                            return { code: 400, message: 'ERR_BUSINESS_LOGIC-Mutually Exclusive Role Present' }
                                        }
                                ...
                                if (policyResponse.result[0].preRequisites !== null && policyResponse.result[0].preRequisites.length > 0 && isPrereqRequired(policyResponse.result[0].preRequisites, roleResponse.result[0].requiredMFAMethod) === true) {
                                    ...
                                    if (enrollmentReqId) {
                                        ...
                                        if (createUserPrereqRespons === "skipped_all") {
...
function enrollmentReqLogic(enrollmentContextId, requesterUser, requesterUserDetails, enrollmentContextResponse, requestedUserDetails, requestedUser) {
    try {
        ...
        let proccessedRoleIdWithPrereq = []
        let activeEnrollmentsWithoutPreReq = {}
        let activeEnrollmentsWithPreReq = [];
        let expiryDateEpoch = currentTimeinEpoch + defaultPrereqExpiryInDays * 24 * 60 * 60 * 1000
        let expiryDate = new Date(expiryDateEpoch).toISOString();
            ...
            if (isRoleAppValid.code === 3) {
                ...
                for (let i = 0; i < applicationRoles.length; i++) {
                    ...
                    if ((applicationRoles[i].roleId && applicationRoles[i].roleId !== null) && (applicationRoles[i].applicationId && applicationRoles[i].applicationId !== null)) {
                        if (proccessedRoleIdWithPrereq.includes(applicationRoles[i].roleId)) {
                        ...
                        if (roleResponse.result[0]) {
                            ...
                            if (applicationRoles[i].kogOrgBusinessKeyId) {
                                isRoleProvisionedRequestBody.payload.kogOrgBusinessKeyId = applicationRoles[i].kogOrgBusinessKeyId
                            }
                            ...
                            // }
                            // if (!(isRoleProvisionedRequestBody.payload.orgId || isRoleProvisionedRequestBody.payload.kogOrgBusinessKeyId)) {
                            //     if (roleMembers.includes(requestedUser)) {
                            //         logger.error("Role is already assigned")
                            //         isRoleAssigned = true
                            ...
                            //     }
                            // }
                            if (isRoleProvisionedRequestBody.payload.orgId === null && isRoleProvisionedRequestBody.payload.kogOrgBusinessKeyId === null) {
                                logger.error("OrgID OR kogOrgBusinessKeyId are null ")
                                // Check if this is a delegation context
                                if (isRoleProvisionedRequestBody.payload.currentDelegatorId || isRoleProvisionedRequestBody.payload.originalDelegatorId) {
                                    // Has delegation info - use isOrgLevelRoleProvisioned to check with delegator context (includes delegator in query)
                                    logger.error("DEBUG_ENROLL: Has delegation info, checking with delegator context")
                                    logger.error("isRoleProvisionedRequestBody ---> " + JSON.stringify(isRoleProvisionedRequestBody))
                                    isOrgLevelRoleProvisionedResponse = isOrgLevelRoleProvisioned(isRoleProvisionedRequestBody)
                                    logger.error("DEBUG_ENROLL: isOrgLevelRoleProvisionedResponse=" + JSON.stringify(isOrgLevelRoleProvisionedResponse))
                                } else {
                                    // No delegation info - just check roleMembers
                                    // if (roleMembers.includes(requestedUser)) {
                                    //     logger.error("Role is already assigned")
                                    //     isRoleAssigned = true
                                    // }
                                    ...
                                    var userRoleResponse = openidm.query("managed/alpha_user/" + requestedUser + "/roles", { "_queryFilter": '_refResourceId eq "' + applicationRoles[i].roleId + '"' }, ["*"]);
                        ...
                        if (isRoleAssigned === false && isOrgLevelRoleProvisionedResponse === false) {
                            ...
                            if (roleResponse.result[0].accessPolicy._refResourceId !== null && roleResponse.result[0].accessPolicy._refResourceId) {
                                ...
                                if (policyResponse && policyResponse.isPolicyPresent !== false) {
                                    ...
                                    if (policyResponse.result[0].preRequisites !== null && policyResponse.result[0].preRequisites.length > 0 && isPrereqRequired(policyResponse.result[0].preRequisites, roleResponse.result[0].requiredMFAMethod) === true) {
                                        roleWithPrereq.push(applicationRoles[i])
                                        // getting required attributes to create enrollment request ID
                                        let requestForEvaluation = []
                                        applicationRoles.forEach(val => {
                                            if (val.roleId == applicationRoles[i].roleId) {
                                                requestForEvaluation.push(val)
                                            }
                                        })
                                        ...
                                        requestForEvaluation.forEach(value => {
                                            ...
                                            if (!proccessedRoleIdWithPrereq.includes(applicationRoles[i].roleId)) {
                                                ...
                                                proccessedRoleIdWithPrereq.push(applicationRoles[i].roleId)
                                            }
                                            roleContextTemplate = {
                                                roleId: null,
                                                applicationId: null,
                                            ...
                                            businessKeysTemplate = {
                                                businessKeyName: null,
                                                businessKeyValue: null
                                            };
                                            if (value.businessKeyId) {
                                                businessKeysTemplate.businessKeyName = value.businessKeyId
                                            }
                                            if (value.businessKeyName) {
                                                businessKeysTemplate.businessKeyValue = value.businessKeyName
                                            }
                                            roleContextTemplate.businessKeys.push(businessKeysTemplate)
                                            if (value.currentDelegatorUserAccountId) {
                                                roleContextTemplate.currentDelegatorId = value.currentDelegatorUserAccountId
                                            }
                                            if (value.originalDelegatorUserAccountId) {
                                                roleContextTemplate.orginalDelegatorId = value.originalDelegatorUserAccountId
                                            }
                                            if (value.roleName) {
                                                roleContextTemplate.roleName = value.roleName
                                            }
                                            if (value.applicationName) {
                                                roleContextTemplate.appName = value.applicationName
                                            }
                                            if (value.orgId) {
                                                roleContextTemplate.orgId = value.orgId
                                            }
                                            if (value.orgType) {
                                                roleContextTemplate.orgType = value.orgType
                                            }
                                            if (value.orgName) {
                                                roleContextTemplate.orgName = value.orgName
                                            }
                                            if (value.applicationId) {
                                                roleContextTemplate.applicationId = value.applicationId
                                            }
                                            if (value.roleId) {
                                                roleContextTemplate.roleId = value.roleId
                                            }
                                            if (value.orgSourceUniqueId) {
                                                roleContextTemplate.orgSourceUniqueId = value.orgSourceUniqueId
                                            }
                                            if (value.businessKeyTypeName) {
                                                roleContextTemplate.businessKeyTypeName = value.businessKeyTypeName
                                            }
                                            if (value.businessKeyId) {
                                                roleContextTemplate.businessKeyId = value.businessKeyId
                                            }
                                            if (value.businessKeyName) {
                                                roleContextTemplate.businessKeyName = value.businessKeyName
                                            }
                                            if (value.businessKeyDescription) {
                                                roleContextTemplate.businessKeyDescription = value.businessKeyDescription
                                            }
                                            if (value.kogOrgBusinessKeyId) {
                                                roleContextTemplate.kogOrgBusinessKeyId = value.kogOrgBusinessKeyId
                                            }
                                            if (value.isForwardDelegable) {
                                                roleContextTemplate.isForwardDelegable = value.isForwardDelegable
                                            }
                                            if (value.delegationEndDateEpoch) {
                                                roleContextTemplate.delegationEndDateEpoch = value.delegationEndDateEpoch
                                            }
                                            if (value.delegationEndDate) {
                                                roleContextTemplate.delegationEndDate = value.delegationEndDate
                                            }
                                        ...
                                        if (enrollmentReqId !== null && enrollmentReqId) {
                                            ...
                                            logger.error("prereqRecord JOSN is ==> " + JSON.stringify(prereqRecord));
                                            createUserPrereqRespons = evaluateUserPrereq(requestedUserDetails, enrollmentReqId._id, requestedUser, requesterUser, roleResponse.result[0]._id, policyResponse.result[0].preRequisites, roleResponse.result[0].requiredMFAMethod, enrollmentContextResponse.applicationRoles[i], requestForEvaluation)
                                            logger.error("createUserPrereqRespons ---> " + JSON.stringify(createUserPrereqRespons))
                                            if (createUserPrereqRespons === "skipped_all") {
                                                activeEnrollmentsWithoutPreReq["associatedAppInfo"] = prereqRecord
                                                activeEnrollmentsWithoutPreReq["id"] = enrollmentReqId._id
                        ...
                        // }
                            ...
                            logger.error("Role Is Already Provisioned")
                            // return { code: 400, message: "Roleis Already Provisioned "}
                        }
                ...
                if (appRolesContextwithoutPrereq.length > 0) {
                    ...
                    for (let l = 0; l < appRolesContextwithoutPrereq.length; l++) {
                        ...
                        if (!(proccessedRoleId.includes(appRolesContextwithoutPrereq[l].roleId))) {
                            proccessedRoleId.push(appRolesContextwithoutPrereq[l].roleId)
        ...
        if (applicationRoles !== null && applicationRoles && applicationRoles.length > 0) {
            for (let i = 0; i < applicationRoles.length; i++) {
                ...
                if ((applicationRoles[i].roleId !== null && applicationRoles[i].roleId) && (applicationRoles[i].applicationId !== null && applicationRoles[i].applicationId)) {
                    logger.error("Role ID --> " + applicationRoles[i].roleId)
                    logger.error("App ID -- " + applicationRoles[i].applicationId)
                    if ((applicationRoles[i].currentDelegatorUserAccountId && applicationRoles[i].currentDelegatorUserAccountId !== null) && (applicationRoles[i].originalDelegatorUserAccountId === null || applicationRoles[i].originalDelegatorUserAccountId === "")) {
                        return { code: 1, message: "Original Delegator Not Present" }
                    }
                    else if ((applicationRoles[i].originalDelegatorUserAccountId && applicationRoles[i].originalDelegatorUserAccountId !== null) && (applicationRoles[i].currentDelegatorUserAccountId === null || applicationRoles[i].currentDelegatorUserAccountId === "")) {
                        return { code: 1, message: "Current Delegator Not Present" }
                    }
                    else if (getRoleDetails("4", applicationRoles[i].roleId, requestedUser, null) && getBusinessAppDetails(0, applicationRoles[i].roleId)) {
                        roleResponse = getRoleDetails("4", applicationRoles[i].roleId, requestedUser, null)
                        logger.error("Inside checkRoleAndApplication roleResponse are --> " + roleResponse)
                        businessAppResponse = getBusinessAppDetails(0, roleResponse.result[0].businessAppId._refResourceId)
                        logger.error("BusinessApp Response is --> " + businessAppResponse)
                        ...
                        logger.error("Inside checkRoleAndApplication businessAppResponse are --> " + businessAppResponse)
                        logger.error("long if condition: " + !(roleResponse.result[0].accessPolicy && roleResponse.result[0].accessPolicy._refResourceId !== null && roleResponse.result[0].accessPolicy._refResourceId));
                        if (!(roleResponse.result[0].accessPolicy && roleResponse.result[0].accessPolicy._refResourceId !== null && roleResponse.result[0].accessPolicy._refResourceId)) {
                            return {
                                code: 1,
                                message: "policy not present"
                            }
...
function isOrgLevelRoleProvisioned(payload) {
    try {
        ...
        logger.error("Error Occurred While isOrgLevelRoleProvisioned " + error)
...
function isMFARegistered(requiredMFACode, requestedUserAccountId) {
    try {
        let requestedUserDetails = getUserDetails(requestedUserAccountId)
        let foundMFA = false
        var UserMFAResponse = null
        if (requestedUserDetails && requestedUserDetails.authenticators !== null) {
            UserMFAResponse = requestedUserDetails.authenticators
        }
        else {
            return false
        }
        ...
        logger.error("Error Occurred WHile Checking MFA")
        return false
    }
...
function patchUserPrerequisites(id, status, completionDateEpoch, completionDate, expiryDate, expiryDateEpoch) {
    ...
    try {
        logger.error("Executing patchUserPrerequisites Function")
        ...
        if (expiryDateEpoch) {
            jsonArray.push({ "operation": "replace", "field": "expiryDateEpoch", "value": expiryDateEpoch })
        }
        var startTime = new Date();
        const response = openidm.patch("managed/" + MO_NAME + "/" + id, null, jsonArray)
        var endTime = new Date();
        ...
        logger.error("Error Occurred While Patching User Prerequisite" + error)
        return null
...
function getPrerequisites(apiRequestPayload) {
    try {
        ...
        const helpTopic = getFaqTopidId("appEnrollment", "prerequisiteSummary")
        if (helpTopic) {
            userPrereqSummary.helpTopics = helpTopic
        }
        var startTime = new Date();
        const response = openidm.query("managed/alpha_kyid_enrollment_user_prerequisites/", {
            ...
            ["status", "displayOrder", "preRequisiteType", "preRequisiteTypeId/*", "preRequisiteTypeId/typeName", "preRequisiteId/*", "preRequisiteId/displayName", "preRequisiteId/displayDescription"])
        ...
        if (response !== null && response.resultCount > 0) {
            ...
            const enrollmentResponse = getMORecord(apiRequestPayload.enrollmentRequestId, ["*"], "alpha_kyid_enrollment_request")
            if (
                enrollmentResponse &&
                enrollmentResponse.roleIds &&
                enrollmentResponse.roleIds.length > 0
            ) {
                const roleResponse = getMORecord(
                    enrollmentResponse.roleIds[0]._refResourceId,
                    ["*","accessPolicy/*"],
                    "alpha_role"
                );
                if (roleResponse && roleResponse !== null) {
                    userPrereqSummary["roleId"] = roleResponse._id || "";
                    if (
                        roleResponse.content &&
                        roleResponse.content.length > 0 &&
                        roleResponse.content[0].name
                    ) {
                        userPrereqSummary["roleDisplayName"] = roleResponse.content[0].name || "";
                    } else {
                        userPrereqSummary["roleDisplayName"] = "";
                    }
                    if(roleResponse.accessPolicy && roleResponse.accessPolicy._refResourceId ){
                      const accessPolicyResponse =  getMORecord(
                    roleResponse.accessPolicy._refResourceId,
                    ["prereqSequence"],
                    "alpha_kyid_enrollment_access_policy"
                );
                      if(accessPolicyResponse && accessPolicyResponse.prereqSequence && accessPolicyResponse.prereqSequence.length>0){
                        userPrereqSummary["prereqSequence"] = accessPolicyResponse.prereqSequence
                      }
                    }

                    if (roleResponse.businessAppId) {
                        const businessAppRolespone = getMORecord(
                            roleResponse.businessAppId._refResourceId,
                            ["*"],
                            "alpha_kyid_businessapplication"
                        );
                        if (businessAppRolespone && businessAppRolespone !== null) {
                            userPrereqSummary["appId"] = businessAppRolespone._id || "";
                            if (
                                businessAppRolespone.content &&
                                businessAppRolespone.content.length > 0 &&
                                businessAppRolespone.content[0].title
                            ) {
                                userPrereqSummary["appDisplayName"] =
                                    businessAppRolespone.content[0].title;
                                userPrereqSummary["appLogo"] = businessAppRolespone.logoURL || ""
                            } else {
                                userPrereqSummary["appDisplayName"] = "";
                            }
                        }
                ...
                return { code: 400, message: "Enrollment Request is Not Valid" }
            }
            ...
            for (let i = 0; i < response.resultCount; i++) {
                ...
                if (
                    !response.result[i] ||
                    !response.result[i].preRequisiteId ||
                    !response.result[i].preRequisiteTypeId
                ) {
                    continue; // Skip this iteration if either is missing
                }
                userPrereqTemplate = {
                    id: null,
                    userPrereqId: null,
                ...
                userPrereqTemplate.prereqName = response.result[i].preRequisiteTypeId.name
                if (response.result[i].status === "COMPLETED" || response.result[i].status === "ALREADY_COMPLETED") {
                    completedCounter++
                }
                else if (userPrereqTemplate.typeName == "MFA" || userPrereqTemplate.typeName == "4") {
                    logger.error("Inside MFA Check Condtion")
                    let MFACompletionCheck = isMFARegistered(roleResponse.requiredMFAMethod, apiRequestPayload.requestedUserAccountId)
                    logger.error("MFACompletionCheck Resut is" + MFACompletionCheck)
                    if (MFACompletionCheck == true) {
                        userPrereqTemplate.status = "COMPLETED"
                        patchUserPrerequisites(response.result[i]._id, "COMPLETED", null, null)
                        completedCounter++

                    }
                }
                // Narendra 11/12/2025 - Added This to Mark User Prereq As Completed for Reuse Condtion
                else if (response.result[i].preRequisiteId && response.result[i].preRequisiteId.enrollmentActionSettings && response.result[i].preRequisiteId.enrollmentActionSettings.allowReuse === true) {
                    logger.error("Inside Else Contion of get Prereq Summary Allow Reuse True")
                    let completedUserPrereq = getCompletedPrereq(apiRequestPayload.requestedUserAccountId, response.result[i].preRequisiteId._id, response.result[i].preRequisiteTypeId._id)
                    if (completedUserPrereq) {
                        let completionDateEpoch = completedUserPrereq.completionDateEpoch || null
                        let completionDate = completedUserPrereq.completionDate || null
                        let expiryDate = completedUserPrereq.expiryDate || null
                        let expiryDateEpoch = completedUserPrereq.expiryDateEpoch || null
                        let allowReuseIfDaysOld = response.result[i].preRequisiteId.enrollmentActionSettings.allowReuseIfDaysOld || "0"

                        let prereqReuse = isPrereqReusable(allowReuseIfDaysOld, completionDateEpoch)
                        if (prereqReuse == true) {
                            logger.error("Inside Else Contion of get Prereq Summary Allow prereqReuse True")
                            patchUserPrerequisites(response.result[i]._id, "ALREADY_COMPLETED", completionDateEpoch, completionDate, expiryDate, expiryDateEpoch)
                            completedCounter++
                        }
        ...
        if (getEnrollmentResponse !== null && getEnrollmentResponse.resultCount > 0) {
            ...
            if (userPrereCancelResponse) {
                ...
                if (cancelEnrollmetReqResponse && cancelEnrollmetReqResponse._id) {
                    userDetailsWithMail = getUserDetails(userId)

                    cancelWorkflowRequest(enrollmentRequestId, userDetailsWithMail); // cancel AIC IGA workflow request
                    ...
                    var langPref = "1"
                    var locale = "en"
                    if (getEnrollmentResponse.result[0].roleContext && getEnrollmentResponse.result[0].roleContext.length > 0) {
                        if (getEnrollmentResponse.result[0].roleContext[0].appName) {
                            appName = getEnrollmentResponse.result[0].roleContext[0].appName
                        }
                        if (getEnrollmentResponse.result[0].roleContext[0].roleName) {
                            roleName = getEnrollmentResponse.result[0].roleContext[0].roleName
                        }
                    }

                    if (getEnrollmentResponse.result[0].requestedUserId) {
                        var requestedUserInfo = openidm.read("managed/alpha_user/" + getEnrollmentResponse.result[0].requestedUserId);
                        if (requestedUserInfo && requestedUserInfo.custom_languagePreference) {
                            langPref = requestedUserInfo.custom_languagePreference || "1"
                    ...
                    sendEmail(appName, roleName, easternTimeStamp, null, requestedUserInfo.givenName, requestedUserInfo.sn, requestedUserInfo.mail, "kyid2B1EnrollmentCancelToEndUser", locale);

                    if (getEnrollmentResponse.result[0].requestedUserId && getEnrollmentResponse.result[0].requesterId && getEnrollmentResponse.result[0].requestedUserId !== getEnrollmentResponse.result[0].requesterId) {
                        locale = "en"
                        langPref = "1"
                        var requesterUserInfo = openidm.read("managed/alpha_user/" + getEnrollmentResponse.result[0].requesterId);
                        ...
                        sendEmail(appName, roleName, easternTimeStamp, null, requesterUserInfo.givenName, requesterUserInfo.sn, requesterUserInfo.mail, "kyidRequestCancelled", locale, requestedUserInfo.mail);
                    }
...
function CancelEnrollmetRequest(enrollmentRequestId, userId) {
    try {
        ...
        logger.error("Performance time enrollment: " + durationInSeconds);
        if (response) {
            logger.error("CancelEnrollmetRequest Response is --> " + JSON.stringify(response))
...
function getInProgressApproval(workflowRequest) {
    ...
    logger.error("custom-endpoint enrollment getInProgressApproval requestPhases: " + requestPhases);
    ...
    if (actors && actors.length > 0) {
        for (let i = 0; i < actors.length; i++) {
            if (actors[i].phase == approvalName) {
                logger.error("custom-endpoint enrollment getApproverMailsForInProgressApproval actor: " + actors[i]);
                approverMails.push(actors[i].mail);
            }
        }
    }

    logger.error("custom-endpoint enrollment getApproverMailsForInProgressApproval approverMails: " + approverMails);


    return approverMails;
}

// Get full approver objects (including givenName, sn, mail) for in-progress approval
        ...
        if (actors && actors.length > 0) {
            for (let i = 0; i < actors.length; i++) {
                ...
                if (actors[i].phase == approvalName) {
                    ...
                    // Check for duplicate email to avoid sending multiple emails to the same approver
                    var isDuplicate = false;
                    for (var k = 0; k < approvers.length; k++) {
                        if (approvers[k].mail.toLowerCase() === mail.toLowerCase()) {
                            isDuplicate = true;
                            logger.info("Skipping duplicate approver: " + mail);
                            break;
                        }
                    }
                    if (isDuplicate) {
...
/*function sendCancellationEmail(approverMails,userDetailsWithMail,enrollmentRequestId, cancellationReason) {
    logger.error("response of userDetailsWithMail:"+ JSON.stringify(userDetailsWithMail))
   ...
   let requesterUserObj = {
    ...
    var userId = userDetailsWithMail.user.result[0].custom_updatedByID
    logger.error("Value of enrollmentRequestId is - " + enrollmentRequestId)
    logger.error("Value of requestedUserAccountId is - " + userId)
    apipayload.enrollmentRequestId = enrollmentRequestId
    apipayload.requestedUserAccountId = userId
  
    let requestBody = {
        "payload": apipayload,
        "action": 8 //Get
    }
    logger.error("Value of request before calling prereq endpoint is - " + JSON.stringify(requestBody))

  try{
        const roleApplicationResponse = openidm.create("endpoint/prerequisite", null, requestBody)
    }
    catch(e){
    ...
    var userDetails = userDetailsWithMail.user.result[0]
    logger.error("userdetails response is - " + JSON.stringify(userDetails))
    requesterUserObj.givenName = userDetails.givenName
    requesterUserObj.userId = userDetails._id
    requesterUserObj.userEmail = userDetails.mail
    requesterUserObj.sn = userDetails.sn
    requesterUserObj.username = userDetails.userName
    ...
    requesterUserObj.timeStamp = timestamp

    requesterUserObj.reason = (typeof cancellationReason === 'object' && cancellationReason !== null) ? cancellationReason.reason : cancellationReason
    ...
    for (let i=0; i<approverMails.length; i++) {
        try {
            var body = {
                ...
                object: requesterUserObj
            };
       
            openidm.action("external/email", "sendTemplate", body);
        } catch (e) {
            logger.info("custom-endpoint enrollment sendCancellationEmail unable to send cancellation notification email: " + e);
        }
    }
}*/


function sendCancellationEmail(approvers, userDetailsWithMail, enrollmentRequestId, cancellationReason) {
    logger.error("response of userDetailsWithMail:" + JSON.stringify(userDetailsWithMail))

    // Get requested user details (B - the person being enrolled)
    var requestedUserMail = userDetailsWithMail.user.result[0].mail;
    var requestedUserDetails = userDetailsWithMail.user.result[0];

    // Get application name and role name
    var applicationName = "";
    var roleName = "";
    var timestamp = isoToEastern();

    // Handle cancellationReason as object or string for backward compatibility
    var reason, cancelledBy, comment;
    if (typeof cancellationReason === 'object' && cancellationReason !== null) {
        reason = cancellationReason.reason || "User cancelled the request";
        cancelledBy = cancellationReason.cancelledBy || "";
        comment = cancellationReason.comment || "";
    } else {
        reason = cancellationReason || "User cancelled the request";
        cancelledBy = "";
        comment = "";
    }

    // Try to get application name from enrollment request
    try {
        var startTime = new Date();
        var enrollmentReq = openidm.query("managed/alpha_kyid_enrollment_request/", {
        ...
        var durationInSeconds = duration / 1000;
        logger.error("Performance time enrollment: " + durationInSeconds);
        if (enrollmentReq && enrollmentReq.result && enrollmentReq.result.length > 0 &&
            enrollmentReq.result[0].appSystemName && enrollmentReq.result[0].appSystemName.length > 0) {
            applicationName = enrollmentReq.result[0].appSystemName[0];
            if (enrollmentReq.result[0].roleContext && enrollmentReq.result[0].roleContext.length > 0) {
                roleName = enrollmentReq.result[0].roleContext[0].roleName || "";
            }
        }
    } catch (e) {
        logger.error("Error fetching application name: " + e);
    }
        ...
        try {
            ...
            var emailObject = {
                ...
                request: {
                    custom: {
                        requesterUser: {
                            requesterUserMail: requestedUserMail,
            ...
            logger.error("custom-endpoint enrollment sendCancellationEmail unable to send cancellation notification email: " + e);
        }
    }
}

// function sendCancellationEmail(approverMails,userDetailsWithMail) {
//     logger.error("SendCanellationEmail: response of approverMails:"+ JSON.stringify(approverMails))
//     logger.error("SendCanellationEmail: response of userDetailsWithMail:"+ JSON.stringify(userDetailsWithMail))
//     for (let i=0; i<approverMails.length; i++) {
//         try {
//             var body = {
//                 subject: "Request Cancelled" ,
...
/*function sendCancellationEmailToRequester(requestermail) {
    try {
        var body = {
            subject: "Request Cancelled",
            to: requestermail,
        ...
        logger.info("custom-endpoint enrollment sendCancellationEmail unable to send cancellation notification email for requester: " + e);
    }
}*/

function sendCancellationEmailToRequester(requestermail, userDetailsWithMail, enrollmentRequestId, cancellationReason) { //Rishitha:02/10
    var requesterUserObject = mapUserDetailsAndRoles(userDetailsWithMail, enrollmentRequestId, cancellationReason)
    try {
        var body = {
            subject: "KYID Account Notification: Cancel Enrollment",
            to: requestermail,
            templateName: "kyid2B1EnrollmentCancelToEndUser",
            object: requesterUserObject
        };
        var startTime = new Date();
        openidm.action("external/email", "sendTemplate", body);
        var endTime = new Date();
...
function mapUserDetailsAndRoles(userDetailsWithMail, enrollmentRequestId, cancellationReason) {  //Rishitha:02/10
    let roleApplicationResponse = null
    let requesterUserObj = {
        givenName: null,
        userId: null,
        userEmail: null,
    ...
    try {
        var startTime = new Date();
        roleApplicationResponse = openidm.create("endpoint/prerequisite", null, requestBody)
        var endTime = new Date();
        var duration = endTime - startTime;
        var durationInSeconds = duration / 1000;
        logger.error("Performance time enrollment: " + durationInSeconds);
    ...
    logger.debug("Values for mail: " + JSON.stringify(requesterUserObj));

    var now = new Date();
    // var timestamp = convertTime.isoToEastern();
    var easternTimeStamp = isoToEastern();
    logger.error("PRINT TS = " + easternTimeStamp + " ")

    logger.error("Values for mail: " + JSON.stringify(requesterUserObj))
    requesterUserObj.updatedTime = easternTimeStamp

    requesterUserObj.reason = (typeof cancellationReason === 'object' && cancellationReason !== null) ? cancellationReason.reason : cancellationReason

    // Get phoneContact from helpdesk contact table
    var phoneContact = null;
    try {
        if (requesterUserObj.applicationName) {
            ...
            var durationInSeconds = duration / 1000;
            logger.error("Performance time enrollment: " + durationInSeconds);
            if (userQueryResult2 && userQueryResult2.result && userQueryResult2.result.length > 0 &&
                userQueryResult2.result[0].phoneContact && userQueryResult2.result[0].phoneContact.length > 0) {
                phoneContact = userQueryResult2.result[0].phoneContact[0].phoneNumber;
            }
        }
    } catch (error) {
        logger.error("custom-endpoint enrollment mapUserDetailsAndRoles error retrieving phoneContact: " + error);
    }
    requesterUserObj.phoneContact = phoneContact;

    return requesterUserObj
...
function cancelWorkflowRequest(enrollmentRequestId, userDetailsWithMail) {
    try {
        // Build cancellation details object for email template
        ...
        logger.error("custom-endpoint enrollment cancelWorkflowRequest enrollmentRequestId: " + enrollmentRequestId);
        var startTime = new Date();
        ...
        var durationInSeconds = duration / 1000;
        logger.error("Performance time enrollment: " + durationInSeconds);
        logger.error("Response from enrollment_user_prerequisites " + JSON.stringify(userPrerequisite));
        var requestId = null;
        var requesterMail = null;

        if (userPrerequisite && userPrerequisite != null) {
            requestId = userPrerequisite.result[0].pingApprovalWorkflowId;
            logger.error("requestId from userPrerequisite is:" + requestId);
            ...
            if (requestId && requestId != null) {
                ...
                var durationInSeconds = duration / 1000;
                logger.error("Performance time enrollment: " + durationInSeconds);
                logger.error("custom-endpoint enrollment cancelWorkflowRequest workflowRequest: " + workflowRequest);
                ...
                if (workflowRequest && workflowRequest != null) {
                    if (workflowRequest.decision.status.toLowerCase() != "completed") {
                        ...
                        logger.error("custom-endpoint enrollment cancelWorkflowRequest inProgressApproval: " + JSON.stringify(inProgressApproval));


                        if (inProgressApproval && inProgressApproval != null) {
                            var startTime = new Date();
                            var cancelResponse = openidm.action("/iga/governance/requests/" + requestId, "POST", { "comments": cancellationReason.reason }, { _action: "cancel", phaseName: inProgressApproval.name });

                            var endTime = new Date();
                            var duration = endTime - startTime;
                            var durationInSeconds = duration / 1000;
                            logger.error("Performance time enrollment: " + durationInSeconds);
                            logger.error("custom-endpoint enrollment cancelWorkflowRequest cancelResponse:" + JSON.stringify(cancelResponse));

                            if (cancelResponse.id != null) {
                                var approvers = getApproversForInProgressApproval(inProgressApproval.name, workflowRequest);
                                // if(requestedUserDetails && requestedUserDetails!=null)
                                // {
                                requesterMail = userDetailsWithMail.user.result[0].mail
                                logger.error("UserDetailsWithMail:" + requesterMail);
                                //}
                                logger.error("custom-endpoint enrollment cancelWorkflowRequest approvers:" + JSON.stringify(approvers));

                                if (approvers && approvers.length > 0) {
                                    sendCancellationEmail(approvers, userDetailsWithMail, enrollmentRequestId, cancellationReason)
                                }
                                // if (requesterMail && requesterMail.length > 0) {
                                //     //sendCancellationEmailToRequester(requesterMail)
                                //     sendCancellationEmailToRequester(requesterMail, userDetailsWithMail, enrollmentRequestId, cancellationReason)   //Rishitha: 02/10
                                // }
                            }
...
function auditLogger(eventCode, eventName, eventDetails, requesterUserId, requestedUserId, transactionId) {
    try {
      ...
      //Fetch the requestedEmail ID
        var requestedemailID ="";
        if(requestedUserId && requestedUserId !== ""){
          var userQueryFilter = '(_id eq "' + requestedUserId + '")';
          var requestedUserObj = openidm.query('managed/alpha_user', { _queryFilter: userQueryFilter}, ["mail"]); 
          if(requestedUserObj && requestedUserObj.result && requestedUserObj.result.length > 0 && requestedUserObj.result[0].mail){
            requestedemailID = requestedUserObj.result[0].mail;
          }
        }
         ...
         //Defect Fix# 211192 (Unknown Location) - 03/12  ----BEGIN
          sessionRefId = context.oauth2 && context.oauth2.rawInfo && context.oauth2.rawInfo.sessionRefId
           ...
           logger.error("In endpoint/enrollment:: Typeof sessionRefId - "+typeof sessionRefId +" and value is - "+JSON.stringify(sessionRefId))
           ...
           var city = sessionRefId.city || "";
            ...
            logger.error("***placeParts in endpoint/enrollment => "+placeParts)
            var place = "";
             if(!city){
        ...
        let logPayload = {
            ...
            requestedUseremailID: requestedemailID,
...
function getFaqTopidId(pageHeader, Process) {
    try {
        ...
        logger.error("Error Occurred while Excuting getFaqTopidId -> " + error)
        return null
    }
...
// Send Email : NR
function sendEmail(applicationName, roleName, updatedTime, reason, givenName, sn, mail, templateName, locale, requestedUserEmail) {
    try {
