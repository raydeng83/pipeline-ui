...
function getAccess(paramJSON) {
    try {
        if (!paramJSON.userId || paramJSON.userId === null) {
            throw {
                code: 500,
                message: "userId is required for search request."
            };
        }
        let queryFilter = "/userIdentifier eq \"" + paramJSON.userId + "\"";
        ...
        var result = openidm.query("managed/alpha_kyid_access", {
            "_queryFilter": queryFilter
        }, ["*", "app/*", "role/*", "user/*", "currentDelegator/*", "originalDelegator/*"]).result;
...
function validateRequest(paramJSON) {
    ...
    try {
        if (paramJSON.requestedUserAccountId && paramJSON.requestedUserAccountId !== null) {
            ...
            throw {
                code: 500,
                message: "requestedUserAccountId not present."
            };
        }
        if (paramJSON.enrollmentRequestId && paramJSON.enrollmentRequestId !== null) {
            ...
            if (requestResponse.expiryDateEpoch < Date.now()) {
                ...
                throw {
                    code: 500,
                    message: "Enrollment Request is expired."
                };
            }
            ...
            throw {
                code: 500,
                message: "enrollmentRequestId not present."
            };
        }
...
function removeRolesFromUser(roles, user, requesterUser) {
    try {
        ...
        throw {
            code: 500,
            message: "Role deprovisioning failed: " + error
        };
    }
    return true;
}
...
function disablePreReqs(roles, user, request) {
    try {
        ...
        throw {
            code: 500,
            message: "Role deprovisioning failed: " + getException(error)
        };
    }
    return true;
}
    ...
    try {
        ...
        if (request && request.roleContext) {
            ...
            if (request.roleIds && request.roleIds.length > 0 && request.requestedUserId) {
                ...
                let queryFilter = {
                    "_queryFilter": `status eq 'IN_PROGRESS' AND requestedUserId eq '${request.requestedUserId}' AND (recordState eq 'ACTIVE' OR recordState eq '0') AND (${roleFilter})`
                };
                ...
                if (response) {
                    response.forEach(val => {
                        if (val.roleContext && val.roleContext.length > 0 && compareJsonArrays(val.roleContext, request.roleContext) === true) {
                            ...
                            let userPrereqQuery = {
                                "_queryFilter": `enrollmentRequestId eq '${val._id}' AND !(status eq 'COMPLETED' OR status eq '2' OR status eq 'ALREADY_COMPLETED' OR status eq '2') `
                            }
    ...
    try {
        ...
        if (roleContext && userId) {
            if (roleContext.roleId) {
                let queryFilter = ` roleContext/[roleId eq '${roleContext.roleId}]' AND status eq 'IN_PROGRESS' AND requestedUserId '${userId}' eq AND (recordState eq 'ACTIVE' OR recordState eq '0')`;
                let response = searchMO("alpha_kyid_enrollment_request", queryFilter, ["*"])
                if (response) {
                    response.forEach(val => {
    ...
    try {
        ...
        if (objects.userResponse && objects.requestResponse) {
            ...
            if (!requestResponse.roleIds) {
                throw {
                    code: 500,
                    message: "No application roles to provision access to."
                };
            }
            ...
            requestResponse.roleContext.forEach(function (roleContext) {
                ...
                if (tempRole && tempRole.recordState && tempRole.recordState.toLowerCase() !== "active") {
                    throw {
                        code: 500,
                        message: "Requested role is inactive: " + tempRole.name
                    };
                }
                ...
                if (roleContext && roleContext !== null) {
                    ...
                    if (!entry.originalDelegator && entry.currentDelegator) {
                        ...
                        throw {
                            code: 500,
                            message: "Delegator Id not present for delegable role"
                        };
                    }
                    ...
                    throw {
                        code: 500,
                        message: "Role context not present for role: " + tempRole.name
                    };
                    // add to accessArray
                ...
                if (tempRole.businessAppId && enrolledApps.find(item => item.kogAppId === tempRole.businessAppId.kogAppId) === undefined) {
                    ...
                    if (contextReturnURL && tempRole.businessAppId.kogParentAppName && tempRole.businessAppId.kogParentAppName.toString().trim() !== "") {
                        ...
                        var childBusinessAppRecords = openidm.query("managed/alpha_kyid_businessapplication", {
                            "_queryFilter": queryFilter
                        }, ["*"]);
                ...
                if (tempRole.accessPolicy && tempRole.accessPolicy._ref) {
                    ...
                    policy.dependentRole.forEach(function (depRole) {
                        ...
                        if (tempDepRole.recordState && tempDepRole.recordState.toLowerCase() !== "active") {
                            throw {
                                code: 500,
                                message: "Requested role is inactive: " + tempDepRole.name
                            };
                        }
                        if (tempDepRole) {
            ...
            try {
                ...
                accessArray.forEach(function (entry) {
                    ...
                    var existingAccess = openidm.query("managed/alpha_kyid_access", {
                        "_queryFilter": queryFilter
                    }, ["_id"]);
            ...
            let patchResponse = openidm.patch("managed/alpha_kyid_enrollment_request/" + requestResponse._id, null, [{
                "operation": "replace",
                "field": "status",
                "value": "COMPLETED"
            },
            ...
            if (pendingEnrollmentRequestIds.length > 0) {
                pendingEnrollmentRequestIds.forEach(val => {
                    patchResponse = openidm.patch("managed/alpha_kyid_enrollment_request/" + val, null, [{
                        "operation": "replace",
                        "field": "status",
                        "value": "COMPLETED"
                    }, {
                    ...
                    policy.dependentRole.forEach(function (depRole) {
                        ...
                        if (tempDepRole.isDelegable) {
                            ...
                            if (roleContext && roleContext !== null) {
                                ...
                                throw {
                                    code: 500,
                                    message: "Role context not present for role: " + tempDepRole.name
                                };
                            }
                        }
