...
(function () {
    ...
    const transactionIdauditLogger = context.current.parent.parent.parent.parent.parent.parent.transactionId ? context.current.parent.parent.parent.parent.parent.parent.transactionId.value : ""
    const sessionRefIDauditLogger = context.current.parent.parent.parent.parent.parent.rawInfo ? context.current.parent.parent.parent.parent.parent.rawInfo.sessionRefId : ""
    const sessionDetailsauditLogger = context.current.parent.parent.parent.parent.parent.rawInfo && context.current.parent.parent.parent.parent.parent.rawInfo.sessionRefId
        ? {"sessionRefId": context.current.parent.parent.parent.parent.parent.rawInfo.sessionRefId}
        : {}

    // Create logger object to pass to functions
...
function accountSearch(input) {
    try {
        ...
        if (input.payload) {
            ...
            const queryFilter = input.payload.queryFilter
            let filterCondition = ""
            // TESTING: Hardcode a user ID for testing the account type filter
            var testUserId = "00489fb6-8ea7-4268-93b3-8067bc44885d"; // Replace queryFilter.userIdFromContext for testing
            logger.error("AARON-DEBUG-1: Using testUserId = " + testUserId);
            var alphaUserType = openidm.read("managed/alpha_user/" + testUserId, null, ["*"]);
            logger.error("AARON-DEBUG-2: alphaUserType.custom_kyidAccountType = " + (alphaUserType ? alphaUserType.custom_kyidAccountType : "null/not found"));
            if (queryFilter) {
                ...
                if (input.isAdvanced) {
                    ...
                    logger.error("AARON-DEBUG-3: Before account type check - businessAppId = " + input.businessAppId + ", alphaUserType exists = " + (alphaUserType != null));
                    if (!input.businessAppId && alphaUserType && alphaUserType != null){
                    logger.error("AARON-DEBUG-4: Checking account type, current type = " + alphaUserType.custom_kyidAccountType);
                    if ( alphaUserType.custom_kyidAccountType && alphaUserType.custom_kyidAccountType === 'P') {
                       filterCondition += ' AND '
                       filterCondition += '/custom_kyidAccountType eq "P"'
                       logger.error("AARON-DEBUG-5: Added Personal filter - will ONLY show Personal accounts");
                    } else {
                       logger.error("AARON-DEBUG-6: User is NOT Personal type, no additional filter added - will show ALL account types");
                    }
                    }
                    logger.error("AARON-DEBUG-7: Final filterCondition (before closing paren) = " + filterCondition);
                ...
                logger.error("AARON-DEBUG-8: Complete final filterCondition = " + filterCondition);
    ...
    try {
        ...
        if (status == "Terminated" && payload.reuseEmail) {
            ...
            if (KOGEmailUpdateAPIResponse && KOGEmailUpdateAPIResponse.ResponseStatus !== 0) {
                logger.error("Update KOG Email Update API Response is --> " + JSON.stringify(KOGEmailUpdateAPIResponse))
            } else {
                logger.error("Update KOG Email Update API Response is --> " + JSON.stringify(KOGEmailUpdateAPIResponse))
            }
        }
        ...
        if (kogAPIResponse && kogAPIResponse.ResponseStatus !== 0) {
            logger.error("Update KOG API Response is --> " + JSON.stringify(kogAPIResponse))
        } else {
            logger.error("Update KOG API Response is --> " + JSON.stringify(kogAPIResponse))
        }
...
function invokeKOGAPI(payload) {
  logger.error("Inside invokeKOGAPI()")
    try {
        const requestBody = {
        ...
        logger.error("requestBody in invokeKOGAPI =>"+JSON.stringify(requestBody))
        const response = openidm.create("endpoint/invokeCertAPI", null, requestBody);
        logger.error("response in invokeKOGAPI =>"+JSON.stringify(response))
        ...
        logger.error("Exception caught in invokeKOGAPI =>"+error)
