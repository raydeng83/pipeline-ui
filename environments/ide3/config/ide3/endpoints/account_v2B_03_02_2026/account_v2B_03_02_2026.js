                ...
                if (input.isAdvanced) {
                    ...
                    if (queryFilter.conditions) {
                        queryFilter.conditions.forEach(condition => {
                            if (condition.name === "mobileNumber") {
                                ...
                                //Ben TFS-214635 Use an OR logic to search for both with the IA_ prefix and without
                    ...
                    if (queryFilter.conditions[0].name === "basic" || true) {
                        ...
                        searchParam.forEach(param => {
                            ...
                            //Ben TFS-214635
                            filterCondition += `/givenName/ co "${param}" OR /sn/ co "${param}" OR /mail eq "${param}" OR (/mail sw "IA_" AND /mail co "${param}")`
...
function updateAccountStatus(input) {
    ...
    try {
        ...
        //Ben-Start: Email update logic with timestamp prefix (IA_YYMMDDHHMM_originalemail)
       ...
       // var kogAPIResponse = false;   // added to test
        ...
        //Ben - Start: Remove roles first so notification goes to ORIGINAL email
        //This will send email notification to the ORIGINAL email address
        //Previously when I added INACTIVE_ it would send the emails to the inactive email address
...
function invokeKOGEmailUpdateAPI(payload,emailNew) {
    try {
        ...
        //Ben - Start: Return full response object to check error codes
        if (response && response.status === "200") {
            return response.response
        } else {
            return null
