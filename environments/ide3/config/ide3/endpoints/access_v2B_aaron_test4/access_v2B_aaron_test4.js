...
(function () {
    ...
    // AUTH CHECK REMOVED FOR TESTING (aaron_test3)
    logger.error("access_v2B_aaron_test4: Auth check bypassed for testing")

    logger.error("===REQUEST in access_v2B_aaron_test4=== " + JSON.stringify(request));
    ...
    const ENDPOINT_NAME = "endpoint/access_v2B_aaron_test4"
 ...
 * Consistent with businessapplication_v2B_aaron_test3 pattern.
 *
...
function getRoleBasedFilters(objectName) {
    ...
    try {
        ...
        logger.info("access_v2B_aaron_test4: getRoleBasedFilters authRequest: " + JSON.stringify(authRequest));

        var authResponse = openidm.create("endpoint/dataAuthorizationCheck_aaron_test1", null, authRequest);

        logger.info("access_v2B_aaron_test4: getRoleBasedFilters authResponse: " + JSON.stringify(authResponse));
        ...
        if (authResponse && authResponse.filter && authResponse.filter.length > 0) {
            ...
            if (filterStrings.length > 0) {
                ...
                logger.info("access_v2B_aaron_test4: getRoleBasedFilters result: " + finalFilter);
                return finalFilter;
            }
        }

        logger.info("access_v2B_aaron_test4: getRoleBasedFilters no filters, returning 'false'");
        return "false";
    } catch (e) {
        logger.error("access_v2B_aaron_test4: getRoleBasedFilters error: " + e.message);
        return "false";
    }
}
