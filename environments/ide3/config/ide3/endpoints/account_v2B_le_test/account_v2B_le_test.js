    ...
    if (request.method === 'create') {
        ...
        if (action === 4) {
            if (view && view != null) {
                ...
                } else if (view == "UserAuthorizationInformation") {
                    return getUserAuthorizationInformation(queryFilter);
                }
            }
        } else if (action === 3) {
            ...
            if (queryFilter) {
                ...
                if (input.businessAppId) {
                    ...
                    if (input.view === "InvitationSearch") {
                        ...
                        contextResponse = openidm.query("managed/alpha_kyid_enrollment_contextId", {
                            "_queryFilter": contextQuery,
                            "_sortKeys": "-createDate",
                            "_fields": "requestedUserAccountId"
                        }).result;
                        ...
                        contextResponse = openidm.query("managed/alpha_kyid_enrollment_request", {
                            "_queryFilter": contextQuery,
                            "_sortKeys": "-createDate",
                            "_fields": ["requestedUserId", "createDate", "expiryDate", "roleIds/*"]
                        }).result;
            ...
            if (queryFilter) {
                if (input.isAdvanced) {
                    ...
                    if (telephoneSearchParam !== "") {
                        let mfaResponse = openidm.query("managed/alpha_kyid_mfa_methods", {
                            "_queryFilter": "/MFAMethod eq \"SMSVOICE\" and MFAValue eq \"" + telephoneSearchParam + "\""
                        }).result;
...
function getUserAuthorizationInformation(userAccountId) {
    logger.error("Inside getUserAuthorizationInformation")
    let alphaUser = openidm.read("managed/alpha_user/" + userAccountId, null, ["*", "roles/*"]);
    logger.error("alphaUser is " + alphaUser)
    let roleList = alphaUser.roles;
    let roleAppList = [];

    if (roleList && roleList.length > 0) {
        for (let i = 0; i < roleList.length; i++) {
            if (roleList[i].name && roleList[i].name.toLowerCase().startsWith("kyid-app-level1-helpdesk")) {
                roleAppList.push({
                    "roleName": roleList[i].name,
                    // "authorizedAppList": roleList[i].authorizedAppList
                    "authorizedAppList": [{
                        "appName": "test123",
                        "appId": "test123"
                    }]
    ...
    logger.error("roleAppList is " + roleAppList)
