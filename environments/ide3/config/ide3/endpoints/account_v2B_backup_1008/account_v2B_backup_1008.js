...
(function () {
    ...
    // Debug: Print context if any auditLogger value is empty
    try {
        if (!transactionIdauditLogger || !sessionRefIDauditLogger || Object.keys(sessionDetailsauditLogger).length === 0) {
            logger.error("account_v2B :: One or more auditLogger values are empty");
            logger.error("account_v2B :: transactionIdauditLogger = " + transactionIdauditLogger);
            logger.error("account_v2B :: sessionRefIDauditLogger = " + sessionRefIDauditLogger);
            logger.error("account_v2B :: sessionDetailsauditLogger = " + JSON.stringify(sessionDetailsauditLogger));
            logger.error("account_v2B :: Printing full context:");
            logger.error("account_v2B :: context = " + JSON.stringify(context));
        } else {
            logger.error("account_v2B :: auditLoggerObj = " + JSON.stringify(auditLoggerObj));
        }
    } catch (e) {
        logger.error("account_v2B :: Error logging auditLoggerObj/context: " + e);
    }
