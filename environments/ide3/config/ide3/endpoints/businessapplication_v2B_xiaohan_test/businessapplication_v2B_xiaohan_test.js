...
(function () {
    ...
    const relationshipAttributes = ["application", "tag", "userName", "roleAppId", "applicationHelpdeskContact"]
    ...
    try {
        ...
        if(viewName){
            return generateResponse(RESPONSE_CODE_SUCCESS, input.transactionId, SUCCESS_MESSAGE,{
                result: getView(viewName,result)    
 ...
 */
 function searchBusinessApplication(input){
    ...
    try{
        ...
        /* Check if  */
        if(input.payload){
            const returnProperties = input.payload.returnProperties?input.payload.returnProperties:["*"]
            const queryFilter = input.payload.queryFilter
            
            if(input._MO_OBJECT_NAME && queryFilter){
                logDebug(input.transactionId,input.endPoint,"searchBusinessApplication",`Search filter: ${queryFilter}`)
 ...
 */
function getView(viewName, result){
            ...
            case "BusinessAdminApplications":
                ...
                if(result){
                    ...
                    result.forEach(app=>{
                        ...
                        logger.error("xiaohan debug point tag 0")
                        let appName = ''
                        let appDescription = ''
                        let logoURL = ''
                        let tagResults = []
                        //let tagQueriedResults
                        //let tagQueriedResults
                        ...
                        if(app.tags){
                            ...
                            app.tags.forEach(tag => {
                                ...
                                //tagQueriedResults.push(openidm.read(tag._ref, null, ["localizedContent"]))
                                let tagQueriedResults = openidm.read(tag._ref, null, ["localizedContent"]).localizedContent
                                
                                if(tagQueriedResults){

                                    tagResults.push(tagQueriedResults.map(tagQueriedResult => ({name: tagQueriedResult.displayTitle, description: tagQueriedResult.displayDescription})))
                                }
