    ...
    try {
        ...
        if (input.payload) {
            ...
            if (queryFilter) {
                ...
                logger.error("Aaron-Debug-1: Before calling getFinalFilter");
                const authFilter = getFinalFilter("alpha_user");
                logger.error("Aaron-Debug-2: After getFinalFilter, authFilter = " + authFilter);
                const finalFilterCondition = "(" + filterCondition + ") AND (" + authFilter + ")";
                logger.error("Aaron-Debug-3: finalFilterCondition = " + finalFilterCondition);

                logger.error("AccountSearch filter (before auth): " + filterCondition);
                logger.error("AccountSearch auth filter: " + authFilter);
                logger.error("AccountSearch final filter: " + finalFilterCondition);

                const searchResponse = openidm.query("managed/alpha_user",
                    {
                        "_queryFilter": finalFilterCondition,
                        "_pageSize": 200
                    },
                    returnProperties
                )
...
function getViewUserPersonalInformation(queryFilter) {
    try {
        ...
        if (userIdentity && userIdentity != null) {
            ...
            //benpark TFS-208614 - Adding check for string "yes" and "true" in addition to boolean true
...
/**
 * Get role-based authorization filters for a user
 * @description Gets role-based authorization filters for the user
 * @param {String} objectName - The managed object name to check authorization for
 * @returns {Array} Array of filter strings
 */
function getRoleBasedFilters(objectName) {
    try {
        logger.error("Aaron-Debug-5: Inside getRoleBasedFilters, objectName = " + objectName);
        let authRequest = {
            action: "getFilter",
            // userId: context.oauth2.rawInfo.user_id,
            userId: "e8748417-bdc4-48b1-b0c8-ea5f30596978",
            objectName: objectName
        };

        logger.error("Aaron-Debug-6: authRequest = " + JSON.stringify(authRequest));

        let authResponse = openidm.create("endpoint/dataAuthorizationCheck", null, authRequest);

        logger.error("Aaron-Debug-7: authResponse = " + JSON.stringify(authResponse));

        if (authResponse && authResponse.filter) {
            let filterStrings = [];
            authResponse.filter.forEach(filterObj => {
                if (filterObj.objectFilter) {
                    filterStrings.push(filterObj.objectFilter);
                }
            });
            logger.error("Aaron-Debug-8: filterStrings = " + JSON.stringify(filterStrings));
            return filterStrings;
        } else {
            logger.error("Aaron-Debug-9: No filters found, returning empty array");
            return [];
        }
    } catch (e) {
        logger.error("Aaron-Debug-10: getRoleBasedFilters error: " + e.message);
        return [];
    }
}

/**
 * Get final filter combining role-based authorization
 * @param {String} objectName - The managed object name
 * @returns {String} Role-based filter string
 */
function getFinalFilter(objectName) {
    try {
        logger.error("Aaron-Debug-4: Inside getFinalFilter, objectName = " + objectName);

        // Get authorization filters
        let roleBasedFilters = getRoleBasedFilters(objectName);

        logger.error("Aaron-Debug-11: roleBasedFilters = " + JSON.stringify(roleBasedFilters));

        let finalFilter = "";

        if (roleBasedFilters.length > 0) {
            finalFilter = "(" + roleBasedFilters.join(") AND (") + ")";
            logger.error("Aaron-Debug-12: Combined filter = " + finalFilter);
        } else {
            finalFilter = "true";
            logger.error("Aaron-Debug-13: No filters, returning 'true'");
        }

        return finalFilter;
    } catch (e) {
        logger.error("Aaron-Debug-14: getFinalFilter error: " + e.message);
        return "true";
    }
}
