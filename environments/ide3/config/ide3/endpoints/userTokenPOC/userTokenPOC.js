...
(function() {
    // Validate userToken from request header using Ping AIC
    try {
        // In ForgeRock/Ping AIC custom endpoints, headers are accessed via context.http.headers
        // Each header value is returned as an array, so we take the first element
        var httpHeaders = context.http.headers;
        var userTokenHeader = httpHeaders['userToken'] || httpHeaders['usertoken'] || httpHeaders['UserToken'] || httpHeaders['USERTOKEN'];
        var userToken = (userTokenHeader && userTokenHeader.length > 0) ? userTokenHeader : null;

        logger.error("Inside UserToken: " + userToken);
        
        if (!userToken || userToken.trim() === '') {
            logger.error("Create Enrollment context API: userToken header is missing or empty");
            throw {
                code: 401,
                message: "Unauthorized - userToken header is missing or empty"
            };
        }
       
        logger.info("Create Enrollment context API: userToken validated successfully");
        
    } catch (ex) {
        logger.error("Create Enrollment context API: Token validation error - " + ex);
        throw {
            code: 401,
            message: "Unauthorized - " + (ex.message || "Token validation failed")
        };
    }
    ...
    if (request.method === 'create') {
        ...
        try {
            ...
            if (request.content != null) {
				...
				if(request.content.userToken !=null){
				}
...
/**
 * Validates the userToken by calling the Ping AIC (ForgeRock Identity Cloud)
 * OAuth2 tokeninfo endpoint.
 * Returns true if the token is valid and active, false otherwise.
 */
function validateUserToken(userToken) {
    try {
        logger.info("Create Enrollment context API: Validating userToken via Ping AIC tokeninfo");

        // var amBaseUrl = identityServer.getProperty("openidm.idpconfig.identityprovider.url")
        //     || identityServer.getProperty("esv.aic.am.url");


        // if (!amBaseUrl || amBaseUrl.trim() === '') {
        //     logger.error("Create Enrollment context API: AM base URL is not configured");
        //     return false;
        // }

        // Call the Ping AIC OAuth2 tokeninfo endpoint
        var tokenInfoUrl = "https://openam-commkentsb3-use1-sandbox.id.forgerock.io" + "/am/oauth2/realms/root/realms/alpha/access_token";
        logger.info("Create Enrollment context API: Calling tokeninfo endpoint: " + tokenInfoUrl);

        var startTime = new Date();
        var tokenInfoResponse = openidm.action("external/rest", "call", {
            "url": tokenInfoUrl,
            "method": "GET",
            "headers": {
        ...
        logger.info("Create Enrollment context API: tokeninfo response time: " + durationInSeconds + "s");

        if (tokenInfoResponse === null || tokenInfoResponse === undefined) {
            logger.error("Create Enrollment context API: tokeninfo returned null response");
            return false;
        }

        // Check HTTP status code if available
        if (tokenInfoResponse.status && tokenInfoResponse.status !== 200) {
            logger.error("Create Enrollment context API: tokeninfo returned status " + tokenInfoResponse.status);
            return false;
        }

        // tokeninfo returns token details if valid; check for access_token or scope
        if (tokenInfoResponse.access_token || tokenInfoResponse.scope) {
            logger.info("Create Enrollment context API: Token is valid. Scope: " + tokenInfoResponse.scope);
            return true;
        }

        // Fallback: if the response has an error_description, token is invalid
        if (tokenInfoResponse.error || tokenInfoResponse.error_description) {
            logger.error("Create Enrollment context API: Token validation failed - " + tokenInfoResponse.error_description);
            return false;
        }

        logger.error("Create Enrollment context API: Unexpected tokeninfo response");
        return false;

    } catch (error) {
        logger.error("Create Enrollment context API: Error calling Ping AIC tokeninfo - " + error);
        return false;
    }
}