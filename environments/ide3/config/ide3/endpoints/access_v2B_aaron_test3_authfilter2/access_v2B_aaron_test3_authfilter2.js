...
var DATA_AUTH_ENABLED = identityServer.getProperty("esv.data.authorization.filter.enabled");
(function () {
    ...
    try {
        ...
        if (input.payload) {
            ...
            if (input._MO_OBJECT_NAME && queryFilter) {
                ...
                if (DATA_AUTH_ENABLED === "true" || DATA_AUTH_ENABLED === true) {
                    var authFilter = getRoleBasedFilters(input._AUTH_OBJECT_NAME)
                    if (authFilter && authFilter !== "false" && authFilter !== "true") {
    ...
    try {
        if (context.oauth2 && context.oauth2.rawInfo && context.oauth2.rawInfo.sessionRefId) {
            sessionRefIDauditLogger = context.oauth2 && context.oauth2.rawInfo && context.oauth2.rawInfo.sessionRefId
                ...
                : "";
        }  else if (request && request.additionalParameters && request.additionalParameters.sessionDetail) {
           
        sessionRefIDauditLogger = (request && request.additionalParameters && request.additionalParameters.sessionDetail)
            ? request.additionalParameters.sessionDetail
            : "";
          logger.error("****DEBUG: sessionRefIDauditLogger value from context else if condition => " + JSON.stringify(sessionRefIDauditLogger)) //Remove Later 
    } else {
...
function auditLogger(eventCode, eventName, sessionDetails, eventDetails, requesterUserId, requestedUserId, transactionId, emailId, applicationName, sessionRefId) {
    try {
      ...
      //Fetch the requesterEmail ID
        ...
        logger.error("***typeof sessionRefId in endpoint/access_v2B => "+typeof sessionRefId)
        if(sessionRefId && typeof sessionRefId == "string"){
          sessionRefId = JSON.parse(sessionRefId)
           var city = sessionRefId.city || "";
            ...
            logger.error("***placeParts in endpoint/access_v2B => "+placeParts)
            var place = "";
             if(!city){
...
/**
 * @name getRoleBasedFilters
 * @description Get role-based authorization filter string
 *
...
function getRoleBasedFilters(objectName) {
    ...
    try {
        ...
        var authResponse = openidm.create("endpoint/dataAuthorizationCheck", null, authRequest);
