...
(function () {
  ...
  if(isEndpointExecutionAllowed === "true"){
    if (request.method === 'create') {
      ...
      const userIds = request.content.userIds
      ...
      if(apiRequestAction === 1){
        ...
        try{ 
          return processPageDetails(apiRequestPayload)
        } catch(error){
        ...
        try{
          ...
          logger.error("Exception in searchCustomPrereqTypes function is - "+getException(error))
          return error
        }
        ...
        try{
          return getPrereqTypeInfo(apiRequestPayload)
        } catch(error){
          logger.error("Exception in searchCustomPrereqTypes function is - "+getException(error))
          return error
        }
        ...
        try{
          return getUserDetailsFromAccount(userIds)
        } catch(error){
          logger.error("Exception in getUserDetailsFromAccount function is - "+getException(error))
          return error
        }
...
function getAllCustomPrereqTypes(){
  ...
  try{
      prereqTypeResponse = openidm.query("managed/alpha_kyid_enrollment_prerequisite_type/", { "_queryFilter": '(recordState eq "ACTIVE" OR recordState eq "0") AND (typeName eq "Custom" OR typeName eq "6")'}, ["*"])
      ...
      if(prereqTypeResponse && prereqTypeResponse !== null  && prereqTypeResponse.resultCount>0){
         prereqTypeResponse.result.forEach( prereqType => {
           ...
           pagesArray.forEach( page => {
             if((page.onLoadServiceEndpoint.endpointName && page.onLoadServiceEndpoint.endpointName==="KOG_Agreement_Endpoint") || 
               (page.onSubmitServiceEndpoint.endpointName && page.onSubmitServiceEndpoint.endpointName==="KOG_Agreement_Endpoint") ||
               (page.onLoadServiceEndpoint.endpointName && page.onLoadServiceEndpoint.endpointName==="KOG_Training_Endpoint") || 
               (page.onSubmitServiceEndpoint.endpointName && page.onSubmitServiceEndpoint.endpointName==="KOG_Training_Endpoint")){
...
/**
* @name <getPrereqTypeInfo>
* @description <It returns all the page fields of custom prerequisite type>

* @param (JSON} paramJSON
* @returns {JSON} Response JSON.
*/
function getPrereqTypeInfo(id){
...
function getPageWithPopulatedData(apiRequestPayload,prereqTypeResponse,userProfileResponse,
  ...
  let agreementEndptResponse = undefined
  let trainingEndptResponse = undefined
        ...
        if(serviceEndptRequestObj.endpoint.isWorkflow){
            ...
            if(workflowResponse.code==201){
              for(let l=0;l<fieldsArray.length;l++){ 
                if(fieldsArray[l].name=="Message"){
                  prereqTypeResponse.fields[l].value = "You're request has been submitted for approval and you'll be notified by email. Please click continue to proceed with the request."
                }
              }
            } 
         ...
         //Check whether configured endpoint is of Agreement Type 
            ...
            while(shouldRetryForAgreement && retryCountForAgreement < 3) {
                ...
                if(agreementEndptResponse == undefined){
                    retryCountForAgreement++;
                    logger.error("Retry count of invokeAgreementAPI is: " + retryCountForAgreement);
                    //Setting the response to 200 to patch user prerequisite when invoking Agreement API fails 
                    agreementEndptResponse={}
                    agreementEndptResponse.ResponseStatus = 0  
         ...
         //Check whether configured endpoint is of Training Type 
            ...
            while(shouldRetryForTraining && retryCountForTraining < 3) {
                ...
                if(trainingEndptResponse == undefined){
                    retryCountForTraining++;
                    logger.error("Retry count of invokeTrainingAPI is: " + retryCountForTraining);
                    //Setting the response to 200 to patch user prerequisite when invoking Agreement API fails 
                    trainingEndptResponse={}
                    trainingEndptResponse.ResponseStatus = 0  
...
function processPageDetails(apiRequestPayload){
  ...
  let responseKOGCredentialsAPI = undefined
  let responseUserOnboardingAPI = undefined
                                                    ...
                                                    if(payloadKOGCredentialsAPI != null && payloadKOGCredentialsAPI.code!="ERR-BUS-SER-VAL-001"){
                                                      ...
                                                      while(shouldRetryForKOG && retryCountForKOG < 3) {
                                                          ...
                                                          if(responseKOGCredentialsAPI == undefined){
                                                              ...
                                                              responseKOGCredentialsAPI.code = 200  
                                                       ...
                                                       {
                                                          logger.error("ResponseKOGCredentialsAPI: " + responseKOGCredentialsAPI);
                                                       }
                                                       //responseKOGCredentialsAPI = invokeKOGCredentialsAPI(payloadKOGCredentialsAPI)                               
                                                     ...
                                                     if(payloadUserOnboardingAPI != null){
                                                        while(shouldRetryForOnboarding && retryCountForOnboarding < 3) {
                                                            ...
                                                            if(responseUserOnboardingAPI == undefined){    
                                                                ...
                                                                responseUserOnboardingAPI.code = 200  
                                                        ...
                                                        {
                                                            logger.error("responseUserOnboardingAPI: " + responseUserOnboardingAPI);
                                                        }   
    ...
    if(serviceEndptRequestObj.endpoint.protocol==1 || serviceEndptRequestObj.endpoint.protocol==3){
      ...
      if(payload!=null && endptSuccessRespParams!=null && payload && endptSuccessRespParams){
          ...
          while(shouldRetryForSOAPWCFService && retryCountForSOAPWCFService < 3) {
                ...
                if(responseServiceEndpointAPI == undefined){
                    retryCountForSOAPWCFService++;
                    logger.error("Retry count of invokeSOAPWCFServiceEndpoint is: " + retryCountForSOAPWCFService);
                    //Setting the response to 200 to patch user prerequisite when invoking SOAP WCF Service Endpoint fails 
                    responseServiceEndpointAPI={}
                    responseServiceEndpointAPI.serviceOutcome = true  
          ...
          if(payload!=null && payload && payload.code != "ERR-BUS-SER-VAL-001") {
              ...
              while(shouldRetryForOrgTypeService && retryCountForOrgTypeService < 3) {
                  ...
                  if(responseServiceEndpointAPI == undefined){
                      ...
                      //Setting the response to 200 to patch user prerequisite when invoking Org Type Service Endpoint fails 
                      responseServiceEndpointAPI={}
                      responseServiceEndpointAPI.serviceOutcome = true            
                  } 
                  else {
          ...
          if(payload!=null && payload){
              ...
              while(shouldRetryForRESTService && retryCountForRESTService < 3) {
                  ...
                  if(responseServiceEndpointAPI == undefined){
                      ...
                      //Setting the response to 200 to patch user prerequisite when invoking REST Service Endpoint fails 
                      responseServiceEndpointAPI={}
                      responseServiceEndpointAPI.serviceOutcome = true            
                  } 
                  else {
      ...
      for(let i=0;i<endptSuccessRespParams.length;i++){
          ...
          if(validateResponseArray.includes(paramName)){
              ...
              if(paramValue == respParamValue){
                ...
                logger.error("Value in response from API doesn't match with config dharjani - "+paramValue)
...
function getEnrollmentRequest(enrollmentRequestId){
   ...
   try {
        var response = openidm.query("managed/alpha_kyid_enrollment_request/", { "_queryFilter": '/_id/ eq "' + enrollmentRequestId + '"' 
...
* @returns {JSON} Response JSON.
*/
function getUserDetailsFromAccount(userAccountIds){
    ...
    try{
        ...
        var response = openidm.query("managed/alpha_user", {
            "_queryFilter": '/_id eq "' + userAccountId + '"'
        }, ["givenName", "sn", "mail", "frIndexedString2", "frIndexedString1", "custom_userType","accountStatus","custom_gender"]);
    ...
    return finalResponse;
        //  if(userPrerequisiteResponse.requestedUserAccountId!=null){
        //    //logger.error("userPrerequisiteResponse.requestedUserAccountId is - "+userPrerequisiteResponse.requestedUserAccountId)
        //    userPayload.userAccountId = userPrerequisiteResponse.requestedUserAccountId
        //  }
        //  requestPayload.queryFilter = userPayload
        //  requestPayload.view = "UserPersonalInformation"
        //  let requestBody = {
        //     "payload": requestPayload,
        //     "action": 4 //Get
        //  }
         
        //  userDetailsResponse = openidm.create("endpoint/account_v2B", null, requestBody)
    
        // if(userIdentityResponses!=null && userIdentityResponses){
        //     if(userDetailsResponse.responseCode==0){
        //         logger.error("userIndormation response from endpoint is - "+JSON.stringify(userDetailsResponse))
        //         userDetails = userDetailsResponse.payload.data
        //         if(userDetails!=null && userDetails)
        //         {
        //             return userDetails
        //         }
        //         else{
        //             logger.error("User Identity Not Found")
        //             //return logDebug(funcName,null,'Account_InActive_In_System',"REF-003")
        //         }
                
      
        //     } else {
        //         logger.error("Respomse from API is" + userDetailsResponse.payload.message.content)
        //         //return logDebug(funcName,null,'Account_InActive_In_System',"REF-003")
        //     }
        // }
        // else{
        ...
        logger.error("Exception in "+funcName+" is - " + getException(error))
        throw logDebug(funcName,null,error,"REF-066")
    }
