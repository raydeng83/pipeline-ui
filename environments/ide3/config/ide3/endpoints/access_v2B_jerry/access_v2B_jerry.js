...
function deleteAccess(input) {
    ...
    try {
        ...
        if (input.payload) {
                            ...
                            if (roleAccessPolicy) {
                                ...
                                for (let dependepentRole of dependentRoles) {
                                    ...
                                    let searchResult = openidm.query(input._MO_OBJECT_NAME, {
                                      "_queryFilter":searchFilter}, ["*"]).result
            ...
            let userExistingRoles = openidm.query("managed/alpha_user/" + requestedAccountId + "/roles", {
                "_queryFilter": "true"
            }, ["*"]).result
            ...
            roleIds.forEach(roleId => {
                let userAccessListForSpecificRoleResult = openidm.query("managed/alpha_kyid_access", {
                    "_queryFilter": '/roleIdentifier eq "' + roleId + '" and userIdentifier eq "' + requestedAccountId + '"'
                }, ["*", "role/*", "app/*"])
           ...
           if(queryRole && queryRole.result && queryRole.result.length>0){
            ...
            if (roleRemovalOperations.length > 0) {
                let removeRoleResponse = openidm.patch("managed/alpha_user/" + requestedAccountId, null, roleRemovalOperations, null, ["*", "*/roles"])
                logger.error("role remove response: " + removeRoleResponse)
                logger.error("appRolearray values are: " + JSON.stringify(appRolearray))
                sendEmailfn(requestedAccountId, appRolearray, "kyid2B1RoleRemoved", "KYID-SYSTEM")
            }
...
var applicationNamesStr = applicationNames.join(", ")
    ...
    sendMail(mail, givenName, sn, roleNamesHtml, phoneContact, emailContact, templateId, requesterUser,applicationNamesStr)
}


function sendMail(mail, givenName, sn, roleNamesHtml, phoneContact, emailContact, templateId, requesterUser,applicationNames) {
    try {
    ...
    if(userAccessResult){
        for (let userAccess of userAccessResult){
            if(userAccess.role){
                ...
                if(role && role.isForwardDelegable){
                    let roleAccessPolicy = role.accessPolicy
                    if(roleAccessPolicy){
                        /* Read access policy */
...
function getExistingRolesForDelegatee(delegateeAccessQuery, input) {
    ...
    const _MO_ACCESS_POLICY = "managed/alpha_kyid_enrollment_access_policy/"
    const _MO_ALPHA_ROLE = "managed/alpha_role/"
    if (delegateeAccessQuery && delegateeAccessQuery.length > 0) {
        let delegateeAccessSearch = input
        delegateeAccessSearch.payload = {
            queryFilter: delegateeAccessQuery + ' AND (recordState eq "0" or recordState eq "ACTIVE")',
            returnProperties: input.payload.returnProperties
        }
                    ...
                    result.forEach(access => {
                        ...
                        if (access) {
                            ...
                            if (!isDelegated) {
                                ...
                                canForwardDelegate = roleIsForwardDelegable;
                            } else {
                                ...
                                canForwardDelegate = roleIsForwardDelegable && accessIsForwardDelegable;
                            }
