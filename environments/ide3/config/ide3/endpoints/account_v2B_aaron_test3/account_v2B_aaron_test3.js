...
function accountSearch(input) {
    try {
        ...
        const ROLE_BASED_ACCESS_MO = "alpha_kyid_access";
        ...
        if (input.payload) {
            ...
            if (queryFilter) {
                ...
                // POC: Get authorized user IDs and post-filter results
                var authorizedUserIds = getAuthorizedUserIds(ROLE_BASED_ACCESS_MO);

                var filteredResults;
                if (authorizedUserIds === true) {
                    // Master role - all access, skip post-filter
                    logger.error("AccountSearch POC: master role, skipping post-filter");
                    filteredResults = searchResponse.result;
                } else {
                    // Apply post-filter (handles both [] and [ids...])
                    logger.error("AccountSearch POC: authorizedUserIds count = " + authorizedUserIds.length);
                    filteredResults = searchResponse.result.filter(function(user) {
                        return authorizedUserIds.indexOf(user._id) !== -1;
                    });
                    logger.error("AccountSearch POC: filtered from " + searchResponse.result.length + " to " + filteredResults.length);
                }
...
function getRoleBasedFilters(objectName) {
    ...
    try {
        ...
        logger.info("account_v2B_aaron_test3: getRoleBasedFilters authRequest: " + JSON.stringify(authRequest));

        var authResponse = openidm.create("endpoint/dataAuthorizationCheck_aaron_test1", null, authRequest);

        logger.info("account_v2B_aaron_test3: getRoleBasedFilters authResponse: " + JSON.stringify(authResponse));
        ...
        if (authResponse && authResponse.filter && authResponse.filter.length > 0) {
            ...
            if (filterStrings.length > 0) {
                ...
                logger.info("account_v2B_aaron_test3: getRoleBasedFilters result: " + finalFilter);
                return finalFilter;
            }
        }

        // No filters found (404 cases: user not found, no filter, no internal role)
        logger.info("account_v2B_aaron_test3: getRoleBasedFilters no filters, returning 'false'");
        return "false";
    } catch (e) {
        // Error cases (including 404 responses) - return false to deny access
        logger.error("account_v2B_aaron_test3: getRoleBasedFilters error: " + e.message);
        return "false";
    }
}

/**
 * POC: Get authorized user IDs array from kyid_access
 * @param {String} objectName - The managed object name (e.g. "alpha_kyid_access")
 */
function getAuthorizedUserIds(objectName) {
    var filter = getRoleBasedFilters(objectName);

    try {
        logger.info("account_v2B_aaron_test3: getAuthorizedUserIds with filter: " + filter);

        // If filter is "false", skip query and return empty array (no access)
        if (filter === "false") {
            logger.info("account_v2B_aaron_test3: getAuthorizedUserIds filter is 'false', returning empty array");
            return [];
        }

        // If filter is/contains "true" (master role), return true to skip post-filter
        if (filter === "true" || filter.indexOf("true") !== -1) {
            logger.info("account_v2B_aaron_test3: getAuthorizedUserIds filter contains 'true' (master role), returning true");
            return true;
        }

        var queryResult = openidm.query("managed/" + objectName, {
            "_queryFilter": filter,
            "_fields": "userIdentifier"
        });

        logger.info("account_v2B_aaron_test3: getAuthorizedUserIds result count: " + queryResult.result.length);
        ...
        logger.info("account_v2B_aaron_test3: getAuthorizedUserIds unique users: " + userIds.length);
        return userIds;
    } catch (e) {
        logger.error("account_v2B_aaron_test3: getAuthorizedUserIds error: " + e.message);
        return [];
    }
}
