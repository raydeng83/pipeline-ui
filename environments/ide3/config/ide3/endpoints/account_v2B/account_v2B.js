...
(function () {
    ...
    if (request.method === 'create') {
        ...
        if (action === 4) {
            ...
            return overrideIdentity(input)
        } else if (action == 7) {
            let input = {
              payload: payload,
              auditLogger: auditLoggerObj
              };
            return overrideMFA(input)
        }
                    ...
                    if (lexIdSearchParam !== "") {
                        ...
                        if (userIdentityResponse && userIdentityResponse.result && userIdentityResponse.result.length > 0) {
                            ...
                            filterCondition += ")";
                             ...
                             // LexID provided but no matching records found - return empty results
                            logger.error("LexID Search - No results found for uuid: " + lexIdSearchParam);
                            let returnPayload = {
                                "transactionId": input.transactionId,
                                "message": {
                                    "code": "0",
                                    "content": "No users found with the provided LexID"
                                },
                                "payload": {
                                    "data": []
                                }
...
function updateAccountStatus(input) {
    ...
    try {
        ...
        var isAccountHighRisk = false
        if (status && status.toLowerCase() == "active") {
            //   var userKOGID = getKOGIDForUser(payload.userAccountId)
            var userResponse = isIdentityHighRisk(payload.userAccountId)
            var userKOGID = userResponse.kogid;
            isAccountHighRisk = userResponse.isHighRisk;
            isMFAHighRisk = isHighRiskMFAMethodPresent(userKOGID)
        }
        if (isMFAHighRisk == false && isAccountHighRisk == false) {
            ...
            return {
                ...
                "transactionId": input.auditLogger.transactionIdauditLogger,
                "message": {
                    "content": "Unable to Activate the User Due to High Risk .REF-009",
                    "code": "2"
                }
            };
        }
...
/**
 * Create user identity record (alpha_kyid_user_identity) for a JIT-provisioned user.
 *
 * Ported from AM Journey: KYID.2B1.Journey.CreateUserIdentity.JIT.script.js
 * The AM Journey creates this as a separate node after user creation.
 * This record stores personal information (name, address, etc.) and is linked
 * to alpha_user via the "account" relationship field.
 *
 * @param {string} userId - PingAIC alpha_user _id
 * @param {object} kogUser - KOG user profile object from GetUserDetails API
 */
function createUserIdentityForJIT(userId, kogUser) {
    logger.debug("[KOG-JIT-IDENTITY] Creating user identity for userId: " + userId + ", KOGID: " + kogUser.KOGID);
    ...
    // Map KOG fields to user_identity fields (same as AM Journey CreateUserIdentity.JIT)
    // KOGID: AM JIT doesn't set at creation, patches later during RIDP. We set it now since we already have it.
    if (kogUser.KOGID) userData["KOGID"] = kogUser.KOGID;
    if (kogUser.FirstName) userData["givenName"] = kogUser.FirstName;
    if (kogUser.LastName) userData["sn"] = kogUser.LastName;
    if (kogUser.MiddleName) userData["middleName"] = kogUser.MiddleName;
    if (kogUser.Address1) userData["addressLine1"] = kogUser.Address1;
    if (kogUser.Address2) userData["addressLine2"] = kogUser.Address2;
    if (kogUser.City) userData["city"] = kogUser.City;
    if (kogUser.State) userData["stateCode"] = kogUser.State;
    if (kogUser.ZipCode) {
        userData["zip"] = kogUser.ZipCode;
        userData["postalExtension"] = kogUser.ZipCode;
    }
    if (kogUser.CountyName) userData["countyCode"] = kogUser.CountyName;
    if (kogUser.CountryCode) userData["countryCode"] = kogUser.CountryCode;
    userData["isHomeless"] = false;

    var response = openidm.create("managed/alpha_kyid_user_identity", null, userData);
    logger.debug("[KOG-JIT-IDENTITY] User identity created successfully - _id: " + response._id);
    return response;
}
...
function isIdentityHighRisk(userAccountId) {
    var alphaUser = openidm.read("managed/alpha_user/" + userAccountId, null, ["*", "custom_userIdentity/*"]);
    var kogid = alphaUser && alphaUser != null ? alphaUser.userName : null;
    if (alphaUser && alphaUser.custom_userIdentity && alphaUser.custom_userIdentity.riskIndicator && alphaUser.custom_userIdentity.riskIndicator.toLowerCase() === "high") {
        return { isHighRisk: true, kogid: kogid };
    }
    else if (alphaUser && alphaUser.custom_userIdentity && alphaUser.custom_userIdentity.riskIndicator && alphaUser.custom_userIdentity.riskIndicator.toLowerCase() === "override") {
        let currentDateISO = new Date().toISOString()
        let currentDate = currentDateISO.split('T')[0]
        let highRiskOverrideDate = alphaUser.custom_userIdentity.highRiskOverrideDate ? alphaUser.custom_userIdentity.highRiskOverrideDate.split('T')[0] : null
        if (highRiskOverrideDate && highRiskOverrideDate >= currentDate) {
            return { isHighRisk: false, kogid: kogid };
        }
        else {
            return { isHighRisk: true, kogid: kogid };
        }
...
function jitProvisionFromKOG(kogUser, returnProperties) {
    ...
    // Step 3.5: Create user identity record (alpha_kyid_user_identity)
    try {
        createUserIdentityForJIT(newUser._id, kogUser);
        logger.debug("[KOG-JIT] User identity creation complete");
    } catch (e) {
        logger.warn("[KOG-JIT] User identity creation failed (non-fatal): " + (e.message || e.detail || e));
    }
...
function getViewAccountInformation(queryFilter) {
    try {
                    ...
                    if (!(mfaMethods[i].MFAMethod.toUpperCase() == "EMAIL") && mfaMethods[i].MFAStatus.toLowerCase() == "active") {
                        ...
                        if (mfaMethods[i].risk && (mfaMethods[i].risk.toLowerCase() == "high" || mfaMethods[i].risk.toLowerCase() == "highrisk") ) {
                            ...
                            highRiskMFATemplate._id = mfaMethods[i]._id,
                                ...
                                highRiskMFATemplate.methodDetails = mfaMethods[i].MFAValue,
                                // highRiskMFATemplate.UsageContexts = methodUsage,
                                highRiskMFATemplate.risk = "High Risk",
                        ...
                        } else if (mfaMethods[i].risk && mfaMethods[i].risk.toLowerCase() == "override") {
                            accountRecoveryAndMFAItem.risk = "High Risk With Overridden"
                            accountRecoveryAndMFAItem.UsageContexts.push("High Risk With Overridden")
                            highRiskMFATemplate._id = mfaMethods[i]._id,
                                highRiskMFATemplate.methodType = {
                                    "es": "es_" + mfaMethodName,
                                    "en": mfaMethodName
                                ...
                                highRiskMFATemplate.methodDetails = mfaMethods[i].MFAValue,
                                // highRiskMFATemplate.UsageContexts = methodUsage,
                                highRiskMFATemplate.risk = "High Risk With Overridden",
                                highRiskMFATemplate.transactionDate = mfaMethods[i].updateDate ? mfaMethods[i].updateDate : "",
                                // highRiskMFATemplate.status = mfaMethods[i].MFAStatus ? mfaMethods[i].MFAStatus : "",
                                highRiskMFATemplate.riskIndicators = mfaMethods[i].riskIndicator ? mfaMethods[i].riskIndicator : [],
                                highRiskMFATemplate.transactionId = queryFilter.transactionId ? queryFilter.transactionId : "",
      ...
      logger.error("Error Occurred While SHowing Data"+error)
        let returnPayload = {
            "responseCode": 2,
            "transactionId": request.content.transactionId,
            "message": {
    ...
    try {
        ...
        if (userIdentity && userIdentity != null) {
            ...
            switch (userIdentity.proofingMethod) {
                ...
                case "1":
                    // verificationStatus = "Verified"
                    verificationSource = "Lexis Nexis";
                    verificationMethod = "Lexis Nexis Identity Verification";
                    break;
                case "4":
                    // verificationStatus = "Verified"
                    verificationSource = "Lexis Nexis";
                    verificationMethod = "Lexis Nexis Identity Proofing";
                    break;
                case "2":
                    // verificationStatus = "Verified"
                    verificationSource = "CMS Experian";
                    verificationMethod = "CMS Experian Identity Proofing";
                    break;
                case "3":
                    // verificationStatus = "Verified"
                    verificationSource = "In-Person";
                    verificationMethod = "In-Person";
            }
            ...
            var assuranceLevel = ""
            if (userIdentity.assuranceLevel) {
                if (userIdentity.assuranceLevel == "0") {
                    assuranceLevel = "AL-0"
                } else if (userIdentity.assuranceLevel == "1") {
                    assuranceLevel = "AL-1"
                } else if (userIdentity.assuranceLevel == "2") {
                    assuranceLevel = "AL-2"
                }
            }
            ...
            var identityVerification = [
                ...
                {
                    "label": {
                        "es": "es_Verification Source(s)",
                        "en": "Verification Source(s)"
                    },
            ...
            var highRiskTransaction = []

            if (userIdentity.riskIndicator && (userIdentity.riskIndicator.toLowerCase() == "high" || userIdentity.riskIndicator.toLowerCase() == "override")) {
                highRiskTransaction = [
                    {
                        "label": {
                            "es": "es_Use Case",
                        ...
                        "value": userIdentity.lastVerificationMethod ? userIdentity.lastVerificationMethod : ""
                    },
                    {
                        "label": {
                            "es": "es_Transaction Date",
                            "en": "Transaction Date"
                        },
                        "value": userIdentity.lastVerificationDate ? userIdentity.lastVerificationDate : ""
                    },
                    {
                        ...
                        "value": userIdentity.riskIndicator ? (userIdentity.riskIndicator === "override" ? "High Risk With Overridden" : (userIdentity.riskIndicator === "high" ? "High Risk" :userIdentity.riskIndicator)) : ""
                    },
                    {
                        "label": {
                            "es": "es_High risk indicators ",
                            "en": "High risk indicators"
                        },
                        "value": ""
                    },
                    {
                        ...
                        "value": userIdentity.highRiskOverrideDate ? userIdentity.highRiskOverrideDate : ""
                    },
                    {
                        "label": {
                            "es": "es_Override Reason ",
                            "en": "Override Reason"
                        },
                        "value": userIdentity.overrideReason ? userIdentity.overrideReason : ""
                    },
                    {
                        "label": {
                            "es": "es_Override Comments ",
                            "en": "Override Comments"
                        },
                        "value": userIdentity.overrideComment ? userIdentity.overrideComment : ""
                    },
            ...
            try {
                ...
                if (ridpConfigResponse && ridpConfigResponse.resultCount > 0) {
                    ...
                    maxOverrideDays = ridpConfigResponse.result[0].ridp_userIdentity_maxOverrideDays
                }
            ...
            let returnPayload = {
                ...
                "payload": {
                    "data": {
                        ...
                        "MaxOverrideDays": maxOverrideDays,
                        "riskIndicators":userIdentity.riskIndicatorDetails ? userIdentity.riskIndicatorDetails : []
                    }
                }
            }
...
/* This method updates user status along with audit data
*/
function overrideIdentity(input) {
  logger.error("overrideIdentity Input is --> "+ JSON.stringify(input))
    var payload = input.payload;
    var eventCode = "";
    var eventName = "";
    var eventDetails = "";
    ...
    try {
        ...
        let currentTimeinEpoch = Date.now()
        let currentDate = new Date().toISOString()
        let currentDateWithoutISO = currentDate.split('T')[0]
        var MaxOverideDate = ""
        ...
        if (payload && payload._id && payload.overrideDate) {
            overrideDate = payload.overrideDate
            overrideDatewithoutISO = overrideDate.split('T')[0]
            if (payload.overrideDate) {
                jsonArray.push({
                    "operation": "replace",
                    "field": "highRiskOverrideDate",
            ...
            eventCode = "OVR004";
            eventName = "Error While Overriding Identity";


            eventDetails = {
                "Error": "Error in updating override date for user identity",
                "Exception": String(error),
                "IP": input.auditLogger.clientIP,
                "Browser": input.auditLogger.clientBrowser
            };
        ...
        try {
            ...
            logger.error("response from query :: " + JSON.stringify(ridpConfigResponse))
            if (ridpConfigResponse && ridpConfigResponse.resultCount > 0) {
                ridpConfigResponse = ridpConfigResponse.result[0]
        ...
        var maxOverrideDays = ridpConfigResponse.ridp_userIdentity_maxOverrideDays ? ridpConfigResponse.ridp_userIdentity_maxOverrideDays : "0"

        maxOverrideDate = addDays(currentDateWithoutISO, maxOverrideDays)

        if (overrideDatewithoutISO > maxOverrideDate) {

            return {
                "responseCode": 2,
                "transactionId": request.content.transactionId,
                "message": {
                    "content": "error in updating Override Date should be less than Max Override Date:" + payload.userAccountId,
                    "code": "2"
                }
            }
        }
        ...
        returnPayload = {
            ...
            "transactionId": request.content.transactionId,
            "message": {
                "content": "Identity Override Successfully " + updateIdentityResponse._id,
                "code": "0"
            }
        }
        ...
        auditLogger("OVR003", "Identity Override Successfully", input.auditLogger.sessionDetailsauditLogger, eventDetails, payload.requesterId, payload.userAccountId, input.auditLogger.transactionIdauditLogger, "", "", input.auditLogger.sessionRefIDauditLogger);
        return returnPayload;
    } catch (error) {
        logger.error("Error in updating the user identity " + error);
        let returnPayload = {
            ...
            "transactionId": request.content.transactionId,
            "message": {
                "content": "error in updating :" + payload.userAccountId,
                "code": "2"
            }
        }

            eventCode = "OVR005";
            eventName = "Error in updating override";
        

        eventDetails = {
            "Error": "Error in updating override",
            "Exception": String(error),
            "IP": input.auditLogger.clientIP,
            "Browser": input.auditLogger.clientBrowser
        };
...
function overrideMFA(input) {
    try {
        logger.error("overrideMFA Input is --> "+ JSON.stringify(input))
        var payload = input.payload;
        let returnPayload = [];
        let currentTimeinEpoch = Date.now()
        let currentDate = new Date().toISOString()
        ...
        if (payload && payload._id) {
            ...
            if (payload.reason) {
                jsonArray.push({
                    "operation": "replace",
                    "field": "overrideReason",
                    "value": payload.reason ? payload.reason : ""
                });
            }
            if (payload.comment) {
                jsonArray.push({
            ...
            returnPayload = {
                ...
                "transactionId": request.content.transactionId,
                "message": {
                    "content": "MFA is Overriden Successfully  " + updateMFAMethod._id,
                    "code": "0"
                }
            }
            eventDetails = {
            ...
            var eventCode = "OVR001"
            var eventName = "MFA Override Success"
        ...
        logger.error("Error While Overriding MFA " + error);
        let returnPayload = {
            "responseCode": 2,
            "transactionId": request.content.transactionId,
            "message": {
                "content": "error while updating MFA overriden  :" + payload.userAccountId,
                "code": "2"
            }
        }

        eventCode = "OVR002";
        eventName = "MFA Override Failure";


        eventDetails = {
            "Error": "Error while overriding MFA",
            "Exception": String(error),
            "IP": input.auditLogger.clientIP,
            "Browser": input.auditLogger.clientBrowser
        };
...
function addDays(dateStr, daysStr) {
