...
(function () {
    ...
    const UPDATE_USER_PROFILE_ENDPOINT_NAME = 'endpoint/account_v2B'
    // Generate transactionServiceCallID for logging
    let transactionServiceCallID = null

    try {
        logger.error("Receiving sync request")
        const requestContent = getRequestContent(context, request, ENDPOINT_NAME)

        // Set transactionServiceCallID from requestContent or generate UUID
        transactionServiceCallID = (requestContent && requestContent.ServiceCallID)
            ? requestContent.ServiceCallID
            : java.util.UUID.randomUUID().toString()

        logger.error("[" + transactionServiceCallID + "] Sync request content: " + JSON.stringify(requestContent))
        switch (request.method) {
            ...
            case REQUEST_POST:
                ...
                if (requestContent.ServiceCallID &&
                    ...
                    let checkConditionResponse = checkConditions(requestContent, requestedUser, transactionServiceCallID)
        ...
        logger.error("[" + (transactionServiceCallID || "NO_TXN_ID") + "] syncuserprofileandauthorizations error: " + JSON.stringify(error));
...
function handleProfileChanged(requestContent, requestedUser, UPDATE_USER_PROFILE_ENDPOINT_NAME) {
    ...
    let userProfileArray = {
        ...
        },
        {
            ...
            "propertyValue": userProfileInfo.UserStatus == 1 ? "active" : "inactive"
        },
        // {
        ...
        //Ben
        //{
        //    "propertyName": "frUnindexedString1",
        //    "propertyValue": userProfileInfo.IsInternal ? "Internal" : "External"
        //},
        //{
        //    "propertyName": "frUnindexedString2",
        //    "propertyValue": userProfileInfo.HomeActiveDirectoryName
        //},
        {
        ...
        //       "propertyValue": userProfileInfo.MobileNumber.startsWith("+")? userProfileInfo.MobileNumber : "+" + userProfileInfo.MobileNumber
        //   }]
    }

    if (userProfileInfo.MobileNumber && userProfileInfo.MobileNumber != null) {
        userProfileArray.alpha_kyid_mfa_methods = [{
            "propertyName": "MFAValue",
            "propertyValue": userProfileInfo.MobileNumber.startsWith("+") ? userProfileInfo.MobileNumber : "+" + userProfileInfo.MobileNumber
        }]
    } else {
...
function checkConditions(requestContent, requestedUser, transactionServiceCallID) {
    ...
    if (kogID) {
        ...
        if (alphaUserResult && alphaUserResult.length == 0) { // if KOGID doesn't exist in Ping
            logger.error("[" + transactionServiceCallID + "] SYNCERR001 - KOGID " + kogID + " not found during sync profile and auth ")
            return {
                "noAction": {
                    "ServiceCallID": requestContent.ServiceCallID,
                    "ResponseStatus": 0,
                    "ErrorCode": "001",
                    "ErrorMessage": "KOGID " + kogID + " not found during sync profile and auth "
                }
            }
        } else {
        ...
        if (userResponse.length > 0) {
            ...
            logger.error("[" + transactionServiceCallID + "] SYNCERR002 - User with  " + requestedUser.userIdAttributeName + ":" + requestedUser.userIdAttributeValue + " not found during sync profile and auth ")
            return {
                "noAction": {
                    "ServiceCallID": requestContent.ServiceCallID,
                    "ResponseStatus": 0,
                    "ErrorCode": "002",
                    "ErrorMessage": "User with  " + requestedUser.userIdAttributeName + ":" + requestedUser.userIdAttributeValue + " not found during sync profile and auth "
                }
            }
        }
    }
        ...
        for (let i = userAuthorizations.length - 1; i >= 0; i--) {
            ...
            if (applicationName) {
                ...
                if (businessAppResult && businessAppResult.length == 0) { // app not found in KYID
                    logger.error("[" + transactionServiceCallID + "] SYNCERR003 - Application with name " + applicationName + " not found during sync profile and auth ")
                ...
                logger.error("[" + transactionServiceCallID + "] SYNCERR004 - Application is required for UserAuth during sync profile and auth")
                return {
                    "error": {
                        "ServiceCallID": requestContent.ServiceCallID,
                        "ResponseStatus": 1,
            ...
            if (roleName) {
                roleResult = openidm.query("managed/alpha_role", {
                    "_queryFilter": `/name eq "${roleName}" and /businessAppId/_refResourceId eq "${businessApp._id}"`
                }, ["*"]).result

                if (roleResult && roleResult.length == 0) { // role not found in KYID
                    logger.error("[" + transactionServiceCallID + "] SYNCERR005 - Role with name " + roleName + " not found during sync profile and auth ")
                    ...
                    logger.error("[" + transactionServiceCallID + "] SYNCERR006 - Role with name " + roleName + " and app name " + applicationName + " returns more than one entry during sync profile and auth ")
                    ...
                    return {
                        "error": {
                            ...
                            "MessageResponses": [{
                                "MessageCocde": "-108",
                                "MessageDescription": "Role with name " + roleName + "and app name " + applicationName + " returns more than one entry during sync profile and auth "
                            }]
                        }
                    }
                    ...
                    if (roleState && roleState.toLowerCase() != "active" && roleState != "0") {
                        logger.error("[" + transactionServiceCallID + "] SYNCERR007 - Role with name " + roleName + " is not active during sync profile and auth ")
                        userAuthorizations.splice(i, 1); // remove current authorization
                        continue
                    }
                }
            } else {
                logger.error("[" + transactionServiceCallID + "] SYNCERR008 - Role name is required for UserAuth during sync profile and auth")
                return {
                    "error": {
                        "ServiceCallID": requestContent.ServiceCallID,
                        "ResponseStatus": 1,
            ...
            if (currentDelegatorKOGID && currentDelegatorKOGID != null && (!originalDelegatorKOGID || originalDelegatorKOGID == null)) {
                logger.error("[" + transactionServiceCallID + "] SYNCERR009 - OriginalDelegator is missing, while currentDelegatorKOGID is present. CurrentDelegator: " + currentDelegatorKOGID + ", OriginalDelegator: " + originalDelegatorKOGID)
            ...
            if ((!currentDelegatorKOGID || currentDelegatorKOGID == null) && originalDelegatorKOGID && originalDelegatorKOGID != null) {
                logger.error("[" + transactionServiceCallID + "] SYNCERR010 - currentDelegatorKOGID is missing, while originalDelegatorKOGID is present. CurrentDelegator: " + currentDelegatorKOGID + ", OriginalDelegator: " + originalDelegatorKOGID)
            ...
            if (BusinessKeyID &&
                !KOGOrgID) {
                logger.error("[" + transactionServiceCallID + "] SYNCERR011 - Business key information is present but org information is not for sync profile and auth")
            ...
            if (action.toLowerCase() == "profilechanged" && (!userProfileInfo || userProfileInfo == null)) {
                logger.error("[" + transactionServiceCallID + "] SYNCERR012 - Action is ProfileChange but no user profile info is present for sync profile and auth")
            ...
            if (action.toLowerCase() == "authchanged" && (!userAuthorizations || userAuthorizations == null)) {
                logger.error("[" + transactionServiceCallID + "] SYNCERR013 - Action is AuthChanged but no user auth info is present for sync profile and auth")
            ...
            if (action.toLowerCase() == "profileandauthchanged" && ((!userAuthorizations || userAuthorizations == null) || (!userProfileInfo || userProfileInfo == null))) {
                logger.error("[" + transactionServiceCallID + "] SYNCERR014 - Action is ProfileAndAuthChanged but user profile or auth info is missing for sync profile and auth")
            ...
            if (originalDelegatorKOGID) {
                ...
                if (originalDelegatorResult && originalDelegatorResult.length == 0) { // if KOGID doesn't exist in Ping
                    logger.error("[" + transactionServiceCallID + "] Original Delegator KOGID " + originalDelegatorKOGID + " not found during sync profile and auth - skipping authorization")
                    userAuthorizations.splice(i, 1); // remove current authorization
                    continue
                } else {
            ...
            if (currentDelegatorKOGID) {
                ...
                if (currentDelegatorResult && currentDelegatorResult.length == 0) { // if KOGID doesn't exist in Ping
                    logger.error("[" + transactionServiceCallID + "] Current Delegator KOGID " + currentDelegatorKOGID + " not found during sync profile and auth - skipping authorization")
                    userAuthorizations.splice(i, 1); // remove current authorization
                    continue
                } else {
            ...
            if (BusinessKeyID) {
                searchFilter = searchFilter.concat(` AND businessKeyName eq '${BusinessKeyID}'`)
            }

            logger.error("[" + transactionServiceCallID + "] _querFilter is: " + searchFilter)
            ...
            if (userAuthorizations[i].AuthorizationAction && userAuthorizations[i].AuthorizationAction.toLowerCase() == "add") {
                if (searchResponse.length > 0) {
                    logger.error("[" + transactionServiceCallID + "] The access entry already exists. Ignore it for add operation.")
                    userAuthorizations.splice(i, 1); // remove current authorization
                    continue
                }
            }
                ...
                if (searchResponse.length == 0) {
                    logger.error("[" + transactionServiceCallID + "] The access entry can not be found. Ignore it for remove operation.")
                    userAuthorizations.splice(i, 1); // remove current authorization
                    continue
                }
            }
