...
function evaluateUserPrereqV2(requestedUserDetails, enrollmentRequestId, userId, requesterId,appRoleContext) {
    try {
        logger.error("Inside evaluateUserPrereq ")
        let currentTimeinEpoch = Date.now()
        let currentDate = new Date().toISOString()
        ...
        let userPrereqPayload = {
            preRequisiteType: null,
            enrollmentRequestId: enrollmentRequestId,
            associatedRoleIds: appRoleContext[0].roleId,
            createDate: currentDate,
            completionDate: null,
            expiryDate: null,
            expiryDateEpoch: null,
        ...
        if (prereqDetails !== null && prereqDetails.length > 0) {
            if (appRoleContext && appRoleContext !== null && appRoleContext.length>0 ) {
                appRoleContext.forEach(val=>{
                roleContext.applicationId = val.applicationId || null
                roleContext.roleId = val.roleId || null
                roleContext.originalDelegatorUserAccountId = val.originalDelegatorUserAccountId || null
                roleContext.currentDelegatorUserAccountId = val.currentDelegatorUserAccountId || null
                ...
                roleContext.businessKeyId = val.businessKeyId || null
                roleContext.businessKeyName = val.businessKeyName || null
                roleContext.businessKeyDescription = valval.businessKeyDescription || null
                roleContext.kogOrgBusinessKeyId = val.kogOrgBusinessKeyId || null
                roleContext.isForwardDelegable = val.isForwardDelegable || false
                roleContext.delegationEndDate = val.delegationEndDate || null
                roleContext.delegationEndDateEpoch = val.delegationEndDateEpoch || null
...
function contextExists(contextArr, candidate) {
  var candStr = JSON.stringify(candidate);
  for (var i = 0; i < contextArr.length; i++) {
    if (JSON.stringify(contextArr[i]) === candStr) {
      return true;
    }
  }
  return false;
...
function reformApplicationContext(inputArr) {
  try {
    var groups = {};
...
for (var idx = 0; idx < inputArr.length; idx++) {
  var item = inputArr[idx];
  // Extract main properties
  var roleId = item.roleId;
  var roleName = item.roleName;
  var appId = item.appId;
  var appName = item.appName;
  var currentDelegatorUserAccountId = item.currentDelegatorUserAccountId || null;
  var originalDelegatorUserAccountId = item.originalDelegatorUserAccountId || null

  // Build context object (all other properties)
  var context = {};
  for (var key in item) {
    if (
      key !== "roleId" &&
      ...
      key !== "appId" &&
      key !== "appName" &&
      key !== "currentDelegatorUserAccountId" && 
      key !== "originalDelegatorUserAccountId" 
    ) {
      context[key] = item[key];
    }
  }

  var groupKey = roleId + "|" + appId;
  if (!groups[groupKey]) {
    groups[groupKey] = {
      roleId: roleId,
      roleName: roleName,
      appId: appId,
      appName: appName,
      currentDelegatorUserAccountId:currentDelegatorUserAccountId,
      originalDelegatorUserAccountId:originalDelegatorUserAccountId,
      roleContext: []
    };
  }
  // Add context if not duplicate
  if (!contextExists(groups[groupKey].roleContext, context)) {
    groups[groupKey].roleContext.push(context);
  }
}
...
var output = [];
for (var k in groups) {
  if (groups.hasOwnProperty(k)) output.push(groups[k]);
}
return output
 
    
  } catch (error) {
    logger.error("Error Occureed While reformApplicationContext "+error)
    return output
  }
...
function isMFARegistered(requiredMFACode,requestedUserAccountId) {
  try {
    ...
    let foundMFA = false
     if (requestedUserDetails && requestedUserDetails.authenticators !== null) {
    let UserMFAResponse = requestedUserDetails.authenticators
 }
  else{
    return false
  }
    ...
    try {
        ...
        if (response !== null && response.resultCount > 0) {
            ...
            for (let i = 0; i < response.resultCount; i++) {
                ...
                userPrereqTemplate.id = response.result[i] && response.result[i].preRequisiteId ? response.result[i].preRequisiteId._id : ""
                userPrereqTemplate.userPrereqId = response.result[i] ? response.result[i]._id : ""
                userPrereqTemplate.typeId = response.result[i] && response.result[i].preRequisiteTypeId ? response.result[i].preRequisiteTypeId._id : ""
                userPrereqTemplate.typeName = response.result[i] && response.result[i].preRequisiteTypeId ? response.result[i].preRequisiteTypeId.typeName : ""
                userPrereqTemplate.name = response.result[i] && response.result[i].preRequisiteId ? response.result[i].preRequisiteId.displayName : ""
                userPrereqTemplate.description = response.result[i] && response.result[i].preRequisiteId ? response.result[i].preRequisiteId.displayDescription : ""
                userPrereqTemplate.status = response.result[i] ? response.result[i].status : ""
...
function sendCancellationEmail(approverMails, userDetailsWithMail, enrollmentRequestId, cancellationReason) {  //Rishitha:02/10
    var requesterUserObj = mapUserDetailsAndRoles (userDetailsWithMail, enrollmentRequestId, cancellationReason)
    logger.error("response of userDetailsWithMail:" + JSON.stringify(userDetailsWithMail))
    for (let i = 0; i < approverMails.length; i++) {
        try {
            var body = {
...
/*function sendCancellationEmailToRequester(requestermail) {
    try {
        ...
        logger.info("custom-endpoint userprerequisite_v2B sendCancellationEmail unable to send cancellation notification email for requester: " + e);
    }
}*/
            ...
            if (requestId && requestId != null) {
                ...
                if (workflowRequest && workflowRequest != null) {
                    if (workflowRequest.decision.status.toLowerCase() != "completed") {
                        ...
                        logger.error("custom-endpoint userprerequisite_v2B cancelWorkflowRequest inProgressApproval: " + JSON.stringify(inProgressApproval));
                        ...
                        if (inProgressApproval && inProgressApproval != null) {
                            ...
                            logger.error("kyid custom endpoint userPrerequisite_v2B cancelWorkflowRequest cancelResponse:" + JSON.stringify(cancelResponse));
                            ...
                            if (cancelResponse.id != null) {
                                ...
                                if (approverMails && approverMails.length > 0) {
                                    sendCancellationEmail(approverMails, userDetailsWithMail, enrollmentRequestId, cancellationReason)
                                }
