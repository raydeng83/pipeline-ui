...
try {
    ...
    logger.debug("Executing KYID.2B1.Journey.IDProofing.PatchUserPrereqAppEnroll ")
    if (nodeState.get("context") === "appEnroll") {
        logger.debug("KYID.2B1.Journey.IDProofing.PatchUserPrereqAppEnroll  Inside App enroll ")
    ...
    try {
        ...
        if (prereqValues.length > 0) {
         ...
         try {
            ...
            var jsonObj = { operation: "replace", field: "/updateDateEpoch", value: auditData.updatedDateEpoch }
            jsonArray.push(jsonObj)
