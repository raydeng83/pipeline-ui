...
function requestCallbacks(jsonobj) {
    ...
    try {
        ...
        if(nodeState.get("verificationAttempt") == "1" || nodeState.get("verificationAttempt") == "0" || !nodeState.get("verificationAttempt")){
            callbacksBuilder.confirmationCallback(0, ["Review and retry","Choose a different method"], 1);
        }else{
            callbacksBuilder.confirmationCallback(0, ["Choose a different method"], 0);
        }
    }catch (error) {
        nodeLogger.error(transactionid + "::" + nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + nodeConfig.begin + "::" + "Error in requestCallbacks Function" + error.message);
    }
}
             ...
             if(selectedOutcome === 0){
                ...
                nodeState.putShared("userSelection","retry")
                 if(patchUseridenityResponse){
                     ...
                     logger.debug("something in patchUseridenityResponse1")
                 }
            }else{
...
function patchUserIdentity(attempt){
    try{
        ...
        nodeLogger.error(transactionid + "::" + nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + nodeConfig.begin + "::" + "Error in patchUserIdentity Function" + error.message);
    }
}
