...
if (missingInputs.length > 0) {
    ...
    jsonObj["custom_createDate"] = auditData.createdDate;
    jsonObj["custom_createdBy"] = auditData.createdBy;
    jsonObj["custom_createdByID"] = auditData.createdByID;
    jsonObj["custom_createDateEpoch"] = auditData.createdDateEpoch;
    jsonObj["custom_updatedDateISO"] = auditData.updatedDate;
    jsonObj["custom_updatedDateEpoch"] = auditData.updatedDateEpoch;
    jsonObj["custom_updatedBy"] = auditData.updatedBy;
    jsonObj["custom_updatedByID"] = auditData.updatedByID;
...
function fetchMailFromUserStore(mail) {
  try {
    ...
    logger.debug("userQueryResultid: " + JSON.stringify(userQueryResult));

    if (userQueryResult.result && userQueryResult.result.length > 0) {
      var _id = userQueryResult.result[0]._id || null;
      logger.debug("_id found" + userMail);
      return _id;
    } else {
      logger.debug("_id found" + null);
      return null;
    }
  } catch (error) {
