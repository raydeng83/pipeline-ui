...
function main(){
    ...
    try {
        ...
        // logger.debug("Role ID is --> "+ nodeState.get("roleIDinSession"));
        nodeLogger = require("KYID.2B1.Library.Loggers");
        txid = JSON.stringify(requestHeaders.get("X-ForgeRock-TransactionId")); 
        nodeLogger.log("debug", nodeConfig.functionName, "begin", txid);
        ...
        if(nodeState.get("isEndpointPresent") === "true"){
            if(isEndpointValidationSuccess(uuid,usrPageInfo,sysEndptResponseMapKeys)){
                ...
                //     logger.debug("Creating EXT IDN")
                //     createExtIDNResponse.push(addIdentityInExtIdentityObject(res)); 
                // }
                    
                    
            } else {
                logger.debug("Endpoint validation failed");
            }   
        }  
        ...
        if(prerequisite==="training" || prerequisite==="agreement"){
            ...
            logger.debug(" "+JSON.stringify(sysPageInfo));
            logger.debug("usrPageInfo is in "+JSON.stringify(usrPageInfo));
            validateResult = validateForm(sysPageInfo,usrPageInfo,pageMetadata); 
            logger.debug("validateResult is: "+JSON.stringify(validateResult));
            // if(validateResult[0].dob){
            //      
            // }else if(validateResult[1].dob){
            //      validateResultArray.push(validateResult[1]);
            // }
            ...
            if(validateResult.length>0){
                ...
                if(usrPageInfo!=null && usrPageInfo){
                        if(nodeState.get("existingPageData")!=null && nodeState.get("saveinput")===false){
