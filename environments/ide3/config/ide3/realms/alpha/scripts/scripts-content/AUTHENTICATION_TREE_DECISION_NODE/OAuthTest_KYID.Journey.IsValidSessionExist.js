...
// Node Config
 var nodeConfig = {
    ...
    script: "Script",
    scriptName: "OAuthTest_KYID.Journey.IsValidSessionExist",
...
function createRequestURL() {
    try{
        ...
        if(requestParameters && requestParameters.get("goto") && requestParameters.get("goto") != null) {
            ...
            nodeLogger.error("goto URL without Decoe : "+requestParameters.get("goto").get(0)); //Remove later
           ...
           // if(requestParameters.get("spEntityID") && requestParameters.get("spEntityID") != null && !requestParameters.get("rp-realm")) {
            ...
            if(count==0) {
                ...
                if(gotoURL.includes("whr")){
                    ...
                    logger.error("inside url without whr param got0URLArray" + gotoURLArray)
                    logger.error("inside url without whr param got0URLArray0" + gotoURLArray[0])
                    logger.error("inside url without whr param got0URLArray1" + gotoURLArray[1])
                    
                    if(gotoURL.includes("display")){
                        logger.error("line181")
