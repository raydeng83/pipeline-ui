...
var nodeConfig = {
    ...
    script: "Script",
    scriptName: "KYID.2B1.Journey.MFAAuthenticationAndRegistration.MFAAuthenticationWithoutRemoveReset",
    timestamp: dateTime,
...
function availableMFAOptions(){
    ...
    nodeLogger.debug(nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + MFAContextCode);
	var MFAContextCode = 3																	
    var newMFAOptions = []
    if (MFAContextCode === 3) {
...
function processCallbacks(mfa) {
    var mfaOptions=[];
    try {
        nodeLogger.debug(nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + "Processing callbacks");
        logger.debug("mfa in Processing callbacks: "+mfa)
        for (var i = 0; i < mfa.length; i++) {
             logger.debug("Printing on trimming 1:::::::::::::::::")
             logger.debug("Printing on trimming 2::::::::::::::::"+ mfa[i])
            if(mfa[i].includes("|")){
                logger.debug("Printing on trimming 3::::::::::::::::"+ mfa[i].substring(0, mfa[i].indexOf("|")))
                 mfaOptions.push(mfa[i].substring(0, mfa[i].indexOf("|")));
            } else {
                mfaOptions.push(mfa[i])
            }
        }
        logger.debug("mfaOptions in processCallbacks: "+mfaOptions)
        var clocale = getLocale();
        var localeMessages = {
            en: {
                verify: "Select"
	    ...
	    var promptMessage = clocale === "es" ? "Seleccione entre los siguientes métodos" : "Select from the following options";																													  

        if (callbacks.isEmpty()) {
            if(nodeState.get("blankmsg")){
                var debug = nodeState.get("blankmsg");
