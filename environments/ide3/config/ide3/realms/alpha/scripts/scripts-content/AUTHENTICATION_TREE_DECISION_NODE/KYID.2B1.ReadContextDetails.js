...
try {
    ...
    logger.debug("requestHeaders ID is:: " + requestHeaders)
    logger.debug("requestParameters ID is:: " + requestParameters)
    ...
    //logger.error("****Session Reference ID value => "+JSON.stringify(sessionRefId))
    //var sessionRefId = generateGUID();
    ...
    if (contextid != null) {
        if (readCookieValueFromSession("ReturnURL") != null && readCookieValueFromSession("ReturnURL")) {
            ...
            if (businessRecords && businessRecords.resultCount > 0) {
                ...
                if (businessRecord && (businessRecord.logoFileName || businessRecord.name)) {
                    ...
                    var contextdetails = returnURLformatJSON(kogAppName, appLogo, spUrl);
                    ...
                    nodeLogger.debug(nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + nodeConfig.missingAppURL + "::" + error);
                }
            } else {
        ...
        nodeLogger.debug(transactionid + "::" + nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + nodeConfig.begin + "context ID not present" + nodeConfig.errorId_InvalidContextId);
...
function getContextDetails(contextid) {
    try {
        ...
        if (contextdetails.result.length > 0) {
            ...
            // nodeLogger.error(transactionid + "::" + nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + nodeConfig.begin + "::" + "Enrollment Context for context ID" + "::" + contextid + "::" + "is valid" + "::" + contextdetails);
            // var contextdetailsResponse = formatJSON(contextdetails,appResponse)
            ...
            nodeLogger.debug(transactionid + "::" + nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + nodeConfig.begin + "::" + "Enrollment Context for context ID" + "::" + contextid + "::" + "is Invalid" + "::" + nodeConfig.errorId_InvalidContextId);
...
function returnURLformatJSON(appName,appLogo,applicationURL) {
    try {
