...
function roleProvisioning() {
  ...
  try {
    ...
    // API URL for
    if (systemEnv.getProperty("esv.assignrolestouser.api") && systemEnv.getProperty("esv.assignrolestouser.api") != null) {
      var getadditionalflagsinfoAPIURL = systemEnv.getProperty("esv.assignrolestouser.api" );
    } else {
    ...
    // Roles to Provision
    if (systemEnv.getProperty("esv.assignrolestouser.api.scope") && systemEnv.getProperty("esv.assignrolestouser.api.scope") != null) {
      var rolesProvisioningScope = systemEnv.getProperty( "esv.assignrolestouser.api.scope");
      //logger.debug("rolesProvisioningScope:: "+ rolesProvisioningScope)
    } else {
    ...
    if (missingInputs.length > 0) {
      ...
      if (roleNames && roleNames.length > 0) {
        ...
        roleNames.forEach((roleName) => {
          userAuths.push({
            ApplicationName: nodeState.get("kogParentApplicationName") ||
...
function checkPrerequisiteException(userId) {
  try {
    ...
    logger.error("rolesToProvision in checkPrerequisiteException :: => " + eligibleRoles );
