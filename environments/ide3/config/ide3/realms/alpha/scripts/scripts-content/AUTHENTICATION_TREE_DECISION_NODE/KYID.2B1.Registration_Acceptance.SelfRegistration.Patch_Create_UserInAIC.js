...
try {
    ...
    if(userExist == "true"){
        logger.debug("availableMFAMethods.length is  " + availableMFAMethods.length)
...
function patchUser (userId,telephoneNumber,primaryEmail,password) {
    try {
        ...
        if (primaryEmail!= null){
            ...
            logger.debug("Email is Updated"+ updateEmail);
        }
        if (telephoneNumber != null){
            ...
            logger.debug("Phone Number is Updated"+ upatePhoneNumber);
        ...
        // var updatePassword = openidm.patch("managed/alpha_user/" + userId, null, [{ "operation": "add", "field": "password", "value": password}]);
            ...
            logger.debug("KOG ID is not Present")
            return false;
        }
    } catch (error) {
