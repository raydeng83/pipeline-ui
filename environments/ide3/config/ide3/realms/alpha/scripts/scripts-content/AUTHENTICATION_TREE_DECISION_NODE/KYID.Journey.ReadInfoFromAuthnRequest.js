...
try {   
    ...
    if(nodeState.get("EntityID") && nodeState.get("EntityID")!=null){
        ...
        spUrl = "https://ide3.kog.ky.gov/home/";
    ...
    try { 
        ...
        logger.error("spUrl in KYID.Journey.ReadInfoFromAuthnRequest- "+spUrl)
        var appRecords = openidm.query("managed/alpha_kyid_application_metadata", { "_queryFilter" : '/forgerockAppId co "'+spUrl+'"'});
        logger.error("appRecords in KYID.Journey.ReadInfoFromAuthnRequest - "+JSON.stringify(appRecords))
    } catch(error) {
