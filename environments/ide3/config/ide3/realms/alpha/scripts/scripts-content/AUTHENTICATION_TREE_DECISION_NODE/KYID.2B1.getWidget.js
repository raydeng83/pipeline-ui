...
//                     if (widgettagResponse && widgettagResponse.result.length > 0) {
//                         logger.debug("tags found for widget: " + widgetId);
//                         for (var j = 0; j < widgettagResponse.result.length; j++) {
//                             var widgettagItem = widgettagResponse.result[j];
//                             var tagID = widgettagItem._refResourceId;
...
//                         }
//                     } else {
//                         logger.debug("No tags found for widget: " + widgetId);
//                     }
...
//               widgetApplicationID: null,
//               widgetdynamicContentEndpointId: widgetResponse.dynamicContentEndpointId,
//               widgettag: widgetTagsList || null,
//               contentTitle: widgetResponse.content && widgetResponse.content[0] && widgetResponse.content[0].title ? widgetResponse.content[0].title : {},
//               contentMyApps: widgetResponse.content && widgetResponse.content[0] && widgetResponse.content[0].myApps ? widgetResponse.content[0].myApps : {},
//               contentAppLibrary: widgetResponse.content && widgetResponse.content[0] && widgetResponse.content[0].appLibrary ? widgetResponse.content[0].appLibrary : {},
//               ellipsisActions: [
...
//             ]
//             });
//            logger.debug("Widget details added from roleId match: " + JSON.stringify(widgetResponse));
//           }
...
//         } else {
//           logger.debug("No widget found for roleId: " + roleId + ". Attempting fallback via applicationId.");
//           // Get applicationId from the role object
...
//           if (roleObj) {
//             //var applicationId = roleObj.applicationId._refResourceId;
//              var applicationId = roleObj.result[0]._refResourceId
//             logger.debug("Found applicationId (" + applicationId + ") from roleId: " + roleId);

//             // Query widget using applicationId
//             var AppWidgetResult = openidm.query("managed/alpha_kyid_dashboardapplicationwidget", {
//               "_queryFilter": "/applicationId/_refResourceId eq \"" + applicationId + "\""
//             });
...
//                         logger.debug("tags found for widget: " + widgetId);
//                         for (var k = 0; k < widgettagforAppResponse.result.length; k++) {
//                             var widgettagItem = widgettagforAppResponse.result[k];
//                             var tagID = widgettagItem._refResourceId;
...
//                 widgetApplicationID: applicationId,
//                 widgetdynamicContentEndpointId: AppWidget.dynamicContentEndpointId,
//                 widgettag: widgetTagsAppList || null,
//                 contentTitle: AppWidget.content && AppWidget.content[0] && AppWidget.content[0].title ? AppWidget.content[0].title : {},
//                contentMyApps: AppWidget.content && AppWidget.content[0] && AppWidget.content[0].myApps ? AppWidget.content[0].myApps : {},
//                contentAppLibrary: AppWidget.content && AppWidget.content[0] && AppWidget.content[0].appLibrary ? AppWidget.content[0].appLibrary : {},
//                 ellipsisActions: [
//               { action: "viewAppDetails", label: "View App Details" },
//               { action: "showLargeIcon", label: "Show Large Display" },
...
//               logger.debug("Widget details added from applicationId fallback: " + JSON.stringify(AppWidget));
//             } else {
//               logger.debug("No widget found for applicationId: " + applicationId);
//             }
//           } else {
...
//     }
//   } catch (e) {
//     logger.debug("Error in getWidgetforUserDetails: " + e.message);
//   }
...
// if (widgetresponseArray.length === 0) {
//   callbacksBuilder.textOutputCallback(0, "No results found.");
//   logger.debug("No widgets found for user.");
//   outcome = "false";
// } else {
//   //callbacksBuilder.textOutputCallback(0, JSON.stringify(widgetresponseArray));
//     var allAppsResponse = JSON.stringify(widgetresponseArray)
//     nodeState.putShared("allAppsResponse",allAppsResponse);
//   logger.debug("Widgets returned for user: " + JSON.stringify(widgetresponseArray));
//   outcome = "true";
// }
...
//       if (userRoleList.resultCount > 0) {
//            logger.debug("user has prov roles")
//         for (var i = 0; i < userRoleList.resultCount; i++) {
//           var roleId = userRoleList.result[i]._refResourceId;
  
//           // Query for widget associated with this role
//           var widgetResult = openidm.query("managed/alpha_kyid_dashboardapplicationwidget", {
//            "_queryFilter": "/roleId/_refResourceId eq \"" + roleId + "\""
//           });
...
//           if (widgetResult && widgetResult.result.length > 0) {
//             var widgetId = widgetResult.result[0]._id;
//               logger.debug("widget id for user" +widgetId)
//             var widgetResponse = openidm.read("managed/alpha_kyid_dashboardapplicationwidget/" + widgetId);
            
//             if (widgetResponse) {
//                 logger.debug("widget response" +widgetResponse)
//               matchedWidgets.push({
//                 widgetId: widgetResponse._id,
//                 widgetName: widgetResponse.name,
//                 widgetType: widgetResponse.type
