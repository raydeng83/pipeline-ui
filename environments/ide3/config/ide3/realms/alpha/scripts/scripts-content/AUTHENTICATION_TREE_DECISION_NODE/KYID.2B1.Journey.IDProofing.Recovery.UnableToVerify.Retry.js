...
var nodeConfig = {
    ...
    script: "Script",
    scriptName: "KYID.2B1.Journey.IDProofing.Recovery.UnableToVerify.Retry",
    timestamp: dateTime,
...
function requestCallbacks() {
    ...
    try {
        ...
        if(nodeState.get("verificationAttempt") == "0" || !nodeState.get("verificationAttempt")){
            nodeState.putShared("promtValue","1")
            var promt = ["Review and retry","Choose a different method"]
        }else{
            nodeState.putShared("promtValue","2")
...
function handleUserResponses() {
    ...
    try {
        ...
        nodeLogger.debug(transactionid + "::" + nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + nodeConfig.begin + "::" + "User selected option index: " + selectedOutcome);
            ...
            if(nodeState.get("verificationAttempt") == "0" || nodeState.get("verificationAttempt") == null){
                ...
                patchUseridenityResponse = patchUserIdentity("2")
                auditLog("RIDP025", "KYID-RD-002: Unable to Verify - User chose to Choose a different method");
                nodeState.putShared("userSelection","differentMethod")
            }

            if(nodeState.get("promtValue")==="1"){
                if (selectedOutcome === 0) {
                    logger.debug("true")
                    nodeState.putShared("verificationAttempt","1")
                ...
                logger.debug("differentMethod")
                nodeState.putShared("verificationAttempt","2")
                action.goTo("differentMethod");
            }
    } catch (error) {
...
function patchUserIdentity(attempt){
    try{
        ...
        if(userIdentity && userIdentity != null && userIdentity != ""){
            ...
            logger.debug(transactionid + "::" + nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + "User Identity is null or empty in Function patchMCIStatus");
