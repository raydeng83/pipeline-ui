        ...
        if(!isResponseValidResult){
            ...
            if (selectedOutcome === 0) {
                ...
                if(responseMCISearchApiCall.status === "fullMatch"){
                    if(responseMCISearchApiCall.MCIResponse.length>0){
                        ...
                        for (var i = 0; i < responseMCISearchApiCall.MCIResponse.length; i++) {
                            ...
                            if(searchUserInPingResponse && searchUserInPingResponse.accountStatus && searchUserInPingResponse.accountStatus.toLowerCase() =="terminated" && searchUserInPingResponse.riskLevel && searchUserInPingResponse.riskLevel.toLowerCase() == "high" ){
                                ...
                                break;
                            }else if(searchUserInPingResponse && searchUserInPingResponse._id && searchUserInPingResponse.mail && searchUserInPingResponse.accountStatus && (searchUserInPingResponse.lexID == verifiedLexId || searchUserInPingResponse.lexID == "") && searchUserInPingResponse.mail == mail && searchUserInPingResponse._id == _id){
                                if((searchUserInPingResponse.accountStatus.toLowerCase() == "active")){
                                    ...
                                    nodeState.putShared("KOGID",searchUserInPingResponse.kogID)
                        ...
                        if(isHighRisk){
                            ...
                            reason = "The KOGID's associated account(s) in Ping Identity is marked as High Risk.";
                        ...
                        // }
                           ...
                           // nodeState.putShared("mail",matchedWithLoggedUser[0])
                            ...
                            auditLog("KYID-LN-002", `User identity verification failed as part of ${flowName}`, false, transactionid, flowName, mail, null, null, null);
                            nodeState.putShared("verifiedLexId","")
                            nodeState.putShared("proofingMethod","-1")
                            nodeState.putShared("MCISYNC","false")
                            nodeState.putShared("errorMessage","KYID-LN-002")
                            ...
                            // nodeState.putShared("ListOfPrimaryEmails",JSON.stringify(jitMailID));     
                    ...
                    var filteredSearchAccountArray = nodeState.get("filteredSearchAccountArray") ? JSON.parse(nodeState.get("filteredSearchAccountArray")) : []
                    logger.debug("filteredSearchAccountArray is :: "+ nodeState.get("filteredSearchAccountArray"))
                    logger.debug("filteredSearchAccountArray is :: "+ filteredSearchAccountArray)                                                               
                    if(filteredSearchAccountArray && filteredSearchAccountArray.length == 1 && nodeState.get("accountStatus") && nodeState.get("accountStatus") === "active"){
                        // nodeState.putShared("mail",matchedWithLoggedUser[0])
                        // nodeState.putShared("ListOfPrimaryEmails",matchedWithLoggedUser)
                        // nodeState.putShared("userSelection",null)
                        reason = "The user personal information provided to LexisNexis is verified";
                        ...
                        auditLog("KYID-LN-002", `${flowName} - Inactive accounts associated with the Identity`, true, transactionid, flowName, mail, userInfo, lexisnexisResponse, reason, title);
                        auditLog("KYID-LN-002", `User identity verification failed as part of ${flowName}`, false, transactionid, flowName, mail, null, null, null);
                        action.goTo("noMatchPerviousSearch");
                    }else if (filteredSearchAccountArray && filteredSearchAccountArray.length == 1 && nodeState.get("accountStatus") && nodeState.get("accountStatus") === "notMatching"){
                        reason = "KYID or LexID does not match with the response provided LexisNexis LexID";
                        title = "User identity verification  transaction failed due to user details provided as part of the input does not match with the verified identity."
                        auditLog("KYID-LN-001", `${flowName} - Input NOT matching with the verified identity`, true, transactionid, flowName, mail, userInfo, lexisnexisResponse, reason, title);
                        auditLog("KYID-LN-001", `User identity verification failed as part of ${flowName}`, false, transactionid, flowName, mail, null, null, null);
                        ...
                        reason = "LexID does not match with the response provided LexisNexis LexID";
                        ...
                        auditLog("KYID-LN-004", `${flowName} - Not able to find the individual based on verified information, verification failed`, true, transactionid, flowName, mail, userInfo, lexisnexisResponse, reason, title);
                        auditLog("KYID-LN-004", `User identity verification failed as part of ${flowName}`, false, transactionid, flowName, mail, null, null, null);
