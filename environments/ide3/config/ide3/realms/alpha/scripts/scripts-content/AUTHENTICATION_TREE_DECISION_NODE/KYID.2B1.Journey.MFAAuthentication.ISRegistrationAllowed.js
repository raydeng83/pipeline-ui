...
var nodeConfig = {
    ...
    script: "Script",
    scriptName: "KYID.2B1.Journey.MFAAuthentication.ISRegistrationAllowed",
    timestamp: dateTime,
...
//         logger.debug("Set MFA Context: "+setMFAContext);
//     }
//     if (JSON.parse(setMFAContext).isMFARequired) {
//         logger.debug("Going to TRUE");
//         action.goTo(NodeOutcome.TRUE)
//     } else {
//         logger.debug("Going to FALSE");
...
try {
    ...
    var isMFARequired = null;
    //var requiredMFAMethodCode = null;
    logger.debug("Set MFA Context in json: "+JSON.parse(JSON.stringify(setMFAContext)));    

    if(nodeState.get("setMFAContext")){
        setMFAContext = JSON.parse(JSON.stringify(nodeState.get("setMFAContext")));
        logger.debug("Set MFA Context in json: "+JSON.parse(JSON.stringify(setMFAContext)));    
        if (setMFAContext.isRegistrationAllowed === true) {
            logger.debug(`${scriptName} : isRegistrationAllowed is True`);
            nodeState.putShared("isRegistrationAllowed","true");
            action.goTo(NodeOutcome.TRUE)
        } else {
            logger.debug(`${scriptName} : isRegistrationAllowed is False`);
