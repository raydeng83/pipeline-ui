...
function main() {
    ...
    try {
        ...
        var parseIfNeeded = function(data) {
        ...
        var isEqual = function(obj1, obj2) {
            var clean1 = sortObject(parseIfNeeded(obj1));
            var clean2 = sortObject(parseIfNeeded(obj2));
            
            logger.debug("Normalized clean1: " + JSON.stringify(clean1));
            logger.debug("Normalized clean2: " + JSON.stringify(clean2));
...
var sortObjectKeys = function(obj) {
    // If it's an array, sort each element and the array itself (if order doesn't matter)
    if (Array.isArray(obj)) {
        return obj.map(sortObjectKeys);
    }
    // If it's an object, sort keys alphabetically
    if (obj !== null && typeof obj === 'object') {
        var sorted = {};
        var keys = Object.keys(obj).sort();
        for (var i = 0; i < keys.length; i++) {
...
var isEqual = function(obj1, obj2) {
    ...
    // Normalize both by sorting keys recursively
    var sorted1 = sortObjectKeys(clean1);
    var sorted2 = sortObjectKeys(clean2);

    logger.debug("sorted1 is :: " + JSON.stringify(sorted1));
    logger.debug("sorted2 is :: " + JSON.stringify(sorted2));
