...
function auditLog(code, message){
    try{
         var auditLib = require("KYID.2B1.Library.AuditLogger")
        logger.error("InsideTryPTA");
                var headerName = "X-Real-IP";
                var headerValues = requestHeaders.get(headerName); 
                var ipAdress = String(headerValues.toArray()[0].split(",")[0]); 
                var userId=null;
                var eventDetails = {};
                logger.error("_id of the user is" +nodeState.get("_id"));
                eventDetails["IP"] = ipAdress;
         ...
         if(result.records>0) {
            ...
            try {
                ...
                if(ret1.code== 0){
                    ...
                    nodeLogger.error(nodeConfig.timestamp+"::"+nodeConfig.node+"::"+nodeConfig.nodeName+"::"+nodeConfig.script+"::"+nodeConfig.scriptName+"::"+ "code is -5"+"::Email::"+nodeState.get("mail"));
...
function queryLDAP(ConnectorName,UserNameAttribute,UserNameAttributeValue){
    ...
    try{
        ...
        nodeState.putShared("sAMAccountName", result.data.sAMAccountName);
            ...
            nodeLogger.error(nodeConfig.timestamp+"::"+nodeConfig.node+"::"+nodeConfig.nodeName+"::"+nodeConfig.script+"::"+nodeConfig.scriptName+"::"+"No Record Found"+"::Email::"+nodeState.get("mail"));
            result['records']=-1;
            result['data']="No Record Found"; 
        }
