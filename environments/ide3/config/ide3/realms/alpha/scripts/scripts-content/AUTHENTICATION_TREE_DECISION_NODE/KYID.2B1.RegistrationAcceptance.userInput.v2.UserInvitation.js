...
function getLangCode(langCode, languageMap){
    ...
    //logger.debug("langKey is :: "+ langKey)
    return langKey;
}
  ...
  function handleUserResponses() {
      ...
      // if (isNameEditable == null || isNameEditable == "true" || givenName != null || lastName != null){
            ...
            // logger.error("Given Name is ::"+ givenName)
            //var lastName = callbacks.getTextInputCallbacks().get(1).trim();
            // logger.error("last Name is ::"+ lastName)
           ...
           // dynamicFields.push(lastName);
            // logger.error("dynamicFields Length is: FirstName and LastName "+ dynamicFields.length)
        ...
        // }
         if (isEmailEditable == null || isEmailEditable == "true" || primaryEmail ==null ){
             ...
             // logger.error("dynamicFields Length is: Email "+ dynamicFields.length)
         ...
         nodeLogger.debug(" Print Outcome Selected :::: " + selectedOutcome);
          ...
          if (selectedOutcome === 0) {
              ...
              nodeLogger.debug(transactionid + "::" + nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + nodeConfig.begin +"::"+ "Collected Email ID :: " +collectedPrimaryEmail );
              if(checkEmail == true){
              ...
              //nodeState.putShared("lastName",null);
                ...
                //     logger.error("KYID.2B1.RegistrationAcceptance.userInput going back from here");
          ...
          var requestedUserAccountAttibutes = nodeState.get("requestedUserAccountAttibutes")
              for(var i=0; i<requestedUserAccountAttibutes.length;i++){
                  if(requestedUserAccountAttibutes[i].attributeName === "mail"){
                      primaryEmail = requestedUserAccountAttibutes[i].attributeValue
                  ...
                  if(requestedUserAccountAttibutes[i].attributeName === "givenName"){
                     nodeState.putShared("orig_givenName",requestedUserAccountAttibutes[i].attributeValue)
                     nodeState.putShared("orig_givenNameIsReadOnly",requestedUserAccountAttibutes[i].isReadOnly)
                     
                  }
                  if(requestedUserAccountAttibutes[i].attributeName === "givenName"){
                     nodeState.putShared("orig_sn",requestedUserAccountAttibutes[i].attributeValue)
                     nodeState.putShared("orig_snIsReadOnly",requestedUserAccountAttibutes[i].isReadOnly)
