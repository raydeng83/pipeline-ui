...
if(nodeState.get("orig_sn")){
    ...
    callbacksBuilder.textInputCallback("Legal last Name");
        ...
        // }
        // //logger.error("phoneArray is "+phoneArray)
...
function handleUserResponses() {
    try {
        ...
        if(nodeState.get("orig_custom_zipExtension")!=zipExtension){
                        changeLog.push({"Postal Extension": zipExtension})
...
if (callbacks.isEmpty()) {
    logger.debug("starting the KYID.2B1.Journey.Manage.Profile.Organ.Donor")
    requestCallbacks();
} else {
    handleUserResponses();
}