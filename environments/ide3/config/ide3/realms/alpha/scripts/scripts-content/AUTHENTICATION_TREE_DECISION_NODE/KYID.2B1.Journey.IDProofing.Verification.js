...
function emptyhandleResponse(){
    ...
    try{
        ...
        nodeState.putShared("orignal_givenName", givenName);
        ...
        if (nodeState.get("orig_custom_middleName")) {
            ...
            nodeState.putShared("orignal_custom_middleName", middleName); 
        ...
        if (nodeState.get("orig_sn")) {
            ...
            nodeState.putShared("orignal_sn", lastName)
        ...
        if(nodeState.get("journeyName")=="forgotPassword"){
            if (nodeState.get("orig_telephoneNumber")  || nodeState.get("userTelephoneNumber")) {
                var telephoneNumber = nodeState.get("orig_telephoneNumber") || nodeState.get("userTelephoneNumber");
            ...
            if (nodeState.get("mail")) {
        ...
        }else if(nodeState.get("journeyName")=="updateprofile" || nodeState.get("journeyName")=="organdonor" || nodeState.get("journeyName")==="MFARecovery" || nodeState.get("journeyName")=="RIDP_LoginMain" || nodeState.get("context") === "appEnroll" || (nodeState.get("journeyName") === "forgotemail") ){

                if(nodeState.get("journeyName")=="RIDP_LoginMain"){
                    nodeState.putShared("loginFlag","true");
                }
                var phoneArray = []
                ...
                if (nodeState.get("mail") || nodeState.get("EmailAddress")) {
                    ...
                    callbacksBuilder.textOutputCallback(0,`Email : ${mail}`);;
                } else if(!(nodeState.get("journeyName") === "forgotemail")){
                    callbacksBuilder.textInputCallback("Primary email");
                }
        }
        ...
        //FAQ topic for my account -- personal info page
        if(nodeState.get("journeyName")=="updateprofile"){
     var lib = require("KYID.Library.FAQPages");
    var process ="RIDP";
    var pageHeader= "1_RIDP_Collect_Personal_Info";
        ...
        var selectedOutcome = callbacks.getConfirmationCallbacks()[0];
            if(selectedOutcome === 1){
                ...
                if(nodeState.get("journeyName")=="forgotPassword" ){
                    ...
                    //nodeState.putShared("registerPhone", true);
                    ...
                    if(nodeState.get("phoneArray")!== null && nodeState.get("phoneArray")){
                        ...
                        if(phoneArray.length>0){
                            ...
                            var telephoneNumber = callbacks.getTextInputCallbacks().get(num);
                            logger.debug("telephoneNumber IN KYID.2B1.Journey.IDProofing.Verification is 1:: "+ telephoneNumber)
                            if(telephoneNumber && telephoneNumber!=null){
                              if(telephoneNumber.length>0){
                                  if(!(nodeState.get("journeyName") === "forgotemail")){
                                      nodeState.putShared("registerPhone", true);
                                  }   
                                }else{
                                   nodeState.putShared("registerPhone", false);
                                }
                            }else{
                                nodeState.putShared("registerPhone", false);
                            }
                ...
                logger.debug("telephoneNumber IN KYID.2B1.Journey.IDProofing.Verification is :: "+ telephoneNumber)
                
                if( nodeState.get("journeyName") && !(nodeState.get("journeyName").toLowerCase() === "createaccount")){
                     nodeState.putShared("userTelephoneNumber",telephoneNumber)
                     nodeState.putShared("telephoneNumber",telephoneNumber)
                }
                ...
                if (postalCode != null && postalCode) {
                    ...
                    // if((!isValidZip(postalCode) || postalCode.length > 40)){ //PA: commented 212573 on 23-Jan-26
                ...
                if (county != null && county) {
                    ...
                    nodeState.putShared("orig_custom_county", county);
                    if( !isCounty(county)){
                        logger.debug("Invalid County errorArray. " +  !isCounty)
                ...
                if(nodeState.get("journeyName")=="forgotPassword" || nodeState.get("journeyName")=="updateprofile" || nodeState.get("journeyName")=="organdonor"||nodeState.get("journeyName")=="MFARecovery" || nodeState.get("context") === "appEnroll" || nodeState.get("journeyName")=="RIDP_LoginMain" || nodeState.get("appEnroll") || (nodeState.get("journeyName") === "forgotemail") || nodeState.get("firsttimeloginjourney") ==="true"){
                    var flag = false;
                       ...
                       logger.debug("telephoneNumber is in :: "+ telephoneNumber)
                        if (telephoneNumber != null && telephoneNumber) {
                            ...
                            if(!isValidPhone(telephoneNumber)){
                             ...
                             userInfoJSON["telephoneNumber"] = telephoneNumber && ( typeof telephoneNumber !== undefined) ? telephoneNumber : null;
                    ...
                    logger.debug("telephoneNumber IN KYID.2B1.Journey.IDProofing.Verification is :: "+ typeof telephoneNumber)
                    if(telephoneNumber){
                         userInfoJSON["telephoneNumber"] = telephoneNumber && ( typeof telephoneNumber !== undefined) ? telephoneNumber : null;
                    }else{
                        userInfoJSON["telephoneNumber"] = null;
                    }
                   // userInfoJSON["telephoneNumber"] = telephoneNumber && ( typeof telephoneNumber !== undefined) ? telephoneNumber : "";
                ...
                if(nodeState.get("_id") || nodeState.get("UserId")){
                    var id = nodeState.get("_id") || nodeState.get("UserId")
                    userInfoJSON["_id"] = id;
                }
               ...
               logger.debug("userInfoJSON in KYID.2B1.Journey.IDProofing.Verification"+ JSON.stringify(userInfoJSON1))
                ...
                action.goTo(NodeOutcome.MISSING_MANDATORY);
                    ...
                    logger.debug("selectedoutcome is :: =>" + selectedOutcome); 
                     logger.debug("selectedoutcome is :: =>" + terms +" : "+ nodeState.get("journeyName")); 
                    

                    if(terms === 0 && selectedOutcome === 0 && (nodeState.get("journeyName")==="createAccount")){
                        logger.debug("selectedoutcome is 1 :: =>" + selectedOutcome); 
                        nodeState.putShared("method", "LexisNexis")
                        nodeState.putShared("action", "verification")
                        if(nodeState.get("registerPhone") == true){
                            logger.debug("telephoneNumber IN KYID.2B1.Journey.IDProofing.Verification is :: "+ telephoneNumber)
                            //auditLog("RIDP000", "Collected personal information, Going for phone registration");
                            action.goTo(NodeOutcome.phoneRegistration);
                            //action.goTo(NodeOutcome.NEXT);
                        }else{
                            //auditLog("RIDP000", "Collected personal information, Going for RIDP");
                            action.goTo(NodeOutcome.NEXT);
                        }
                        
                    }else if(terms === 0 && selectedOutcome === 0 && ((nodeState.get("journeyName") === "updateprofile") || (nodeState.get("journeyName") === "organdonor") || nodeState.get("journeyName")=="RIDP_LoginMain") || nodeState.get("context")==="appEnroll"){
                        /*if(changeLog.length>0){
                            nodeState.putShared("changeLog", changeLog)
                            action.goTo(NodeOutcome.changeLog); 
                        }else{
                        */
                         logger.debug("selectedoutcome is 2 :: =>" + selectedOutcome);
                        if(nodeState.get("registerPhone") == true){
                            logger.debug("telephoneNumber IN KYID.2B1.Journey.IDProofing.Verification is :: "+ telephoneNumber)
                           // auditLog("RIDP000", "Collected personal information, Going for phone registration");
                            action.goTo(NodeOutcome.phoneRegistration);
                    ...
                    else if(terms === 0 && selectedOutcome === 0 && ((nodeState.get("journeyName") === "forgotemail") || nodeState.get("journeyName")=="MFARecovery" || (nodeState.get("journeyName") === "forgotPassword"))){
                        nodeState.putShared("method", "LexisNexis")
                        nodeState.putShared("action", "proofing")
                        //action.goTo(NodeOutcome.NEXT);
                            logger.debug("selectedoutcome is 3 :: =>" + selectedOutcome);
                        if(nodeState.get("registerPhone") == true){
                            logger.debug("telephoneNumber IN KYID.2B1.Journey.IDProofing.Verification is :: "+ telephoneNumber);
                           // auditLog("RIDP000", "Collected personal information, Going for phone registration");
                            action.goTo(NodeOutcome.phoneRegistration);
