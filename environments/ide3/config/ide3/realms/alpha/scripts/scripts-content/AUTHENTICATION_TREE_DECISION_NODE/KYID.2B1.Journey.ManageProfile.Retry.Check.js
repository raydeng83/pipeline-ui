...
function main() {
    ...
    var outcome = {}
    var ridpFlag = systemEnv.getProperty("esv.ridp.manageprofile.version.flag") ? systemEnv.getProperty("esv.ridp.manageprofile.version.flag").toLowerCase() : "v2";
    try{
    ...
    if(ridpFlag && ridpFlag === "v2"){
         if(nodeState.get("orig_proofingMethod") && (nodeState.get("orig_proofingMethod") == "4" || nodeState.get("orig_proofingMethod") == "-1" || nodeState.get("orig_proofingMethod") == "2")){
                    if(systemEnv.getProperty("esv.manageprofile.ssp.retry.limit")){
                        var retryLimit = systemEnv.getProperty("esv.manageprofile.ssp.retry.limit")
