...
var nodeConfig = {
    ...
    script: "Script",
    scriptName: "KYID.2B1.Journey.IsKOGIDExistInPing",
    timestamp: dateTime,
...
try {
    var patchResult = openidm.patch("managed/alpha_user/" + id, null, patchArray);
...
if (missingInputs.length > 0) {
    ...
    try {
        logger.debug("InsideIsKOGexistinginping :: " + usrKOGID)
        var response = openidm.query("managed/alpha_user", { "_queryFilter": "/userName eq \"" + usrKOGID + "\"" }, ["userName", "_id", "mail","custom_kyidProfileSetup","custom_isJITDone","custom_userIdentity/*"]);
        logger.debug("Reposne of User in AIC with Kog ID " + response.result.length)
        if (response.result.length == 1) {
            var idmUser = response.result[0];
            logger.debug("the idmUser.custom_kyidProfileSetup is"+idmUser.custom_kyidProfileSetup)
            nodeState.putShared("_id", idmUser._id);
            nodeState.putShared("isKYIDProfileSetupCompleted",idmUser.custom_kyidProfileSetup) //added for checking JIT first time for externaluser 
            nodeState.putShared("isJITDone",idmUser.custom_isJITDone) //added for checking JIT first time for externaluser 
            //check user has idenity linked or not
            if(idmUser.custom_userIdentity && idmUser.custom_userIdentity._id){
                 nodeState.putShared("userIdentityPresent","true")
            }
            ...
            if (response.result.length >= 1) {
                ...
                nodeLogger.debug(nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + nodeConfig.usrNotExistIDM);
                action.goTo(nodeOutcome.NOT_EXIST);
            }
        }
