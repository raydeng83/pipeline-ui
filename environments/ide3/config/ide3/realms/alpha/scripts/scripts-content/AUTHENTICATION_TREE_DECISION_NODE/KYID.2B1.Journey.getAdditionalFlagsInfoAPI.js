...
try {
    ...
    nodeLogger.debug("Existing session properties in KYID.2B1.Journey.getAdditionalFlagsInfoAPI :: => " + existingSession);
    // Transaction ID
    var transactionID;
        ...
        if (kogAPITokenResponse.status === 200) {
            ...
            if (res.status === 200) {
                ...
                if (data.ResponseStatus === 0) {
                    ...
                    //nodeLogger.error(nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + ":: Alternate email removed/added sucessfully ::Email::" + alternateEmailAddress);
                ...
                //nodeLogger.error(nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + ":: HTTP response ::" + res.status + "::Email::" + alternateEmailAddress);
                action.goTo(nodeOutcome.FAIL);
            }
        } else {
            logger.error("kogAPITokenResponse is not 200 ");
            //nodeLogger.error(nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + ":: kogAPITokenResponse is not 200 :: Email ::" + alternateEmailAddress);
            action.goTo(nodeOutcome.FAIL);
        }
    }
} catch (error) {
