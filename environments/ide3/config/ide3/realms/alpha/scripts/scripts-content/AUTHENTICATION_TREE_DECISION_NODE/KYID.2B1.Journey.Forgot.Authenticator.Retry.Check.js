...
function main() {
    ...
    var ridpFlag = systemEnv.getProperty("esv.ridp.forgot.authenticator.version.flag") ? systemEnv.getProperty("esv.ridp.forgot.authenticator.version.flag").toLowerCase() : "v2";
    try {
        ...
        if (ridpFlag && ridpFlag === "v2") {
            if (nodeState.get("orig_proofingMethod") && (nodeState.get("orig_proofingMethod") == "4" || nodeState.get("orig_proofingMethod") == "-1" || nodeState.get("orig_proofingMethod") == "2")) {
                if (systemEnv.getProperty("esv.forgot.authenticator.ssp.retry.limit")) {
                    var retryLimit = Number(systemEnv.getProperty("esv.forgot.authenticator.ssp.retry.limit"))
                    if (verificationAttempt >= retryLimit) {
                        diffInDays = diffInDays();
                        if (systemEnv.getProperty("esv.forgot.authenticator.ssp.refresh.limit")) {
                            var refreshLimit = Number(systemEnv.getProperty("esv.forgot.authenticator.ssp.refresh.limit"))
