...
function fetchUserData(userId) {
    try {
        logger.debug("Reading User Data from profile for user " + userId);
        var userData = openidm.read("managed/alpha_user/" + userId);
        logger.debug("User Data from profile: " + userData);
        return userData;
    } catch (error) {
   ...
   var credId = nodeState.get("credentialId");
    ...
    function deactivateMfaMethod(mfaMethodName) {
       var usrKOGID = fetchuserKOGId();
        logger.debug("printing the usrKOGId" +usrKOGID);
        var query = {
        ...
        nodeLogger.debug("Printing usrKOGID: " + usrKOGID);
        nodeLogger.debug("Printing mfaMethodId: " + JSON.stringify(mfaMethodId));
        ...
        if (mfaMethodId.resultCount > 0) {
            for (var i = 0; i < mfaMethodId.result.length; i++) {
                ...
                deletedAny = true;
                nodeLogger.debug("MFA method with ID " + mfamethodId + " set to INACTIVE.");
            }
            return true; // Indicate that at least one method was deactivated
        } else {
            nodeLogger.debug("No matching MFA methods found for user " + user + " with method " + mfaMethodName);
            return false; // Indicate failure to deactivate
        }
    }

     if (mfamethod === "FRPUSH" || mfamethod === "FRTOTP") {
            logger.debug("************inside swk or otp*************");
            var requestURL;
            //var URL = systemEnv.getProperty("esv.kyid.tenant.fqdn");
         var URL = "https://sso.dev2.kyid.ky.gov";
            logger.debug("Removing PUSH for USer ----"+ user)
            if (mfamethod === "FRPUSH") {
                logger.debug("IN Side PUSH REMOVAl..")
                logger.debug("Removing PUSH for USer ----"+ user)
...
logger.debug("*********reading requestURL************" +requestURL)
        
            var bearerToken = nodeState.get("amAccessToken");
          logger.debug("reading access token" +bearerToken)
            var options = {
                method: "POST",
                headers: {
            ...
            nodeLogger.debug("API Response: " + JSON.stringify(apiResponse));

            if (status === 200) {
                nodeLogger.debug("Response status is 200 for mfamethod: " + mfamethod);
                deletedAny = true;
            } else {
                nodeLogger.debug("Failed API call for mfamethod: " + mfamethod + " with status: " + status);
            }
        }
