...
function getWidgetForUser(userId){
    ...
    const userApps = []
    const userRoles = []
    var appNameToId = {};  // app name → app _id mapping for access expiry check
   ...
   // const user = openidm.read(`${MO_ALPHA_USER}${userId}`,null,["*"])
       const user = openidm.read(`${MO_ALPHA_USER}${userId}`,null,["_id", "effectiveRoles"])
    ...
    if(user.effectiveRoles && user.effectiveRoles.length>0){
        user.effectiveRoles.forEach(roleRef=>{
            ...
            if(role.businessAppId  && role.businessAppId.name){
                ...
                // Capture app name → app _id mapping for access expiry lookup
                if (role.businessAppId._refResourceId) {
                    appNameToId[role.businessAppId.name] = role.businessAppId._refResourceId;
                }
            }
        })
    }
    /* Check access expiry: build set of apps with at least one non-expired access record */
    var nonExpiredApps = {};
    var currentEpoch = new Date().getTime();
    var allAccessRecords = openidm.query("managed/alpha_kyid_access", {
        "_queryFilter": "userIdentifier eq '" + userId + "' AND (recordState eq '0' OR recordState eq 'ACTIVE')"
    }, ["appIdentifier", "expiryDateEpoch"]).result;

    for (var a = 0; a < allAccessRecords.length; a++) {
        var rec = allAccessRecords[a];
        if (!rec.appIdentifier) continue;
        // null/undefined/0 expiryDateEpoch means no expiry (permanent access)
        if (!rec.expiryDateEpoch || rec.expiryDateEpoch === 0 || rec.expiryDateEpoch > currentEpoch) {
            nonExpiredApps[rec.appIdentifier] = true;
        }
    }
    logger.debug("Access expiry check: " + allAccessRecords.length + " records found");

    //Adding the logic to to show the widgets based on the usertype

   var queryFilter = "(/recordState eq \"0\" and myAppsSettings/associatedAppSystemName pr ";

        if (nodeState.get("typeofuser") === "Internal") {
            logger.debug("user is internal in myApps query")
            // Internal user allowedUserType = 0 OR 2
           queryFilter += "and (allowedUserType eq \"0\" or allowedUserType eq \"2\" ))";
        } else {
            logger.debug("user is external in myApps query")
            // External user allowedUserType = 1 OR 2 OR empty OR not present
             queryFilter += "and (allowedUserType eq \"1\" or allowedUserType eq \"2\" ))";
         }
        ...
        // ).result;
           var allWidgets = openidm.query(
            MO_KYID_DASHBOARDWIDGET,
            { "_queryFilter": queryFilter },
            ["_id", "name", "type", "description", "logoURL", "myAppsSettings", "appLibrarySettings", "content", "maintenanceModeSettingsisEnabled", "maintenanceModeSettingsmessage"]
        ).result;
    
        allWidgets.forEach(widget => {
   var isAppMatch = false;
    var isRoleMatch = false;
    var associatedRoleName = null;
    ...
    logger.debug("Processing widget for app: " + associatedAppName +" | roleList length: " + associatedRoleList.length);

    if (associatedRoleList.length === 0) {
        //Application widget
        isAppMatch = userApps.includes(associatedAppName);
        logger.debug("Application widget has no roles.This is application widget. isAppMatch=" + isAppMatch);

    } else if (associatedRoleList.length === 1) {
        //Role widget
        var roleNameInList = associatedRoleList[0];
        isRoleMatch = userRoles.includes(associatedAppName + "-" + roleNameInList);
        if (isRoleMatch) {
            associatedRoleName = roleNameInList;
        }
        logger.debug("Role widget has 1 role. isRoleMatch=" + isRoleMatch +" | matchedRole=" + associatedRoleName);

    } else {
        // More than 1 role --> treat as Application widget
        // isAppMatch = userApps.includes(associatedAppName);
        // logger.debug("Application widget detected more than 1 roles. isAppMatch=" + isAppMatch);
         // Multiple roles -- show ONLY if user has at least one matching role
for (var r = 0; r < associatedRoleList.length; r++) {
        var roleName = associatedRoleList[r];
        if (userRoles.includes(associatedAppName + "-" + roleName)) {
            isAppMatch = true;   // treat as app widget
            break;
        }
    }

    logger.debug("Multi-role widget (app-level with role check). rolesCount=" +associatedRoleList.length +" | isAppMatch=" + isAppMatch);
    }
        /* Check if the user has role level or app level access that matches with the widget */
        if (isRoleMatch || isAppMatch) {
            // Skip widget if no non-expired access record exists for this app
            var widgetAppId = appNameToId[associatedAppName];
            if (widgetAppId && !nonExpiredApps[widgetAppId]) {
                logger.debug("Skipping widget - no valid access for app: " + associatedAppName);
                return;  // skip this widget in forEach
            }
...
function handleUserResponses() {
    try {
        ...
        if (selectedOutcome === 0) {
            ...
            var inputId = parsedInput.roleId || parsedInput.applicationId || parsedInput.widgetId;
            //var inputType = parsedInput.roleId ? "roleId" : "applicationId";
            var actionName = parsedInput.action;

            if (inputId) {
                logger.debug("the inputId "+inputId);
                if(parsedInput.widgetId){
                    logger.debug("the inputId for viewAppDetails"+inputId);
                     nodeState.putShared("widgetselected",parsedInput.widgetId)
                }
                else if(parsedInput.roleId){
                   ...
                   // nodeState.putShared("roleIds", [parsedInput.roleId]);
                    ...
                    logger.debug("the inputId is for applicationID "+inputId);
...
// === MAIN EXECUTION ===
try {
    logger.debug("KYID.2B1.Journey.getAndDisplayWidgets.Combine script initiated")
    nodeState.putShared("myAppsJourney","true")
...
callbacksBuilder.textOutputCallback(0,JSON.stringify(errMsg))
        ...
        if (nextAction) {
            ...
            switch (nextAction) {
                case "viewAppDetails":
                    logger.debug("user selected view app details")
