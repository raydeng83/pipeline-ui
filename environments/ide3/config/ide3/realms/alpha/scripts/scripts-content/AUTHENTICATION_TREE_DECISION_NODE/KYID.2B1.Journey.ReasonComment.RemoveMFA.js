    ...
    try {
       ...
       var mfaidremoval = nodeState.get("mfaidremoval")
        if (selectedIndex === 0) {
            ...
            nodeLogger.debug("the jsonArray in cancel invitation is: " + JSON.stringify(jsonArray));

            try {
                openidm.patch("managed/alpha_kyid_mfa_methods/" + mfaidremoval, null, jsonArray);
                nodeLogger.debug("Audit fields patched successfully for contextId: " + mfaidremoval);
            } catch (patchErr) {
...
try {
    ...
    nodeState.putShared("journeyName","BusinessSupportUpdate") //MFA Reporting
