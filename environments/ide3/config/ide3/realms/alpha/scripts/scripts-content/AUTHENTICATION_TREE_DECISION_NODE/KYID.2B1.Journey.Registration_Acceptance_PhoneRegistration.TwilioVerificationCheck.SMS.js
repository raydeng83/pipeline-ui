...
function handleUserResponses() {
    try {
        ...
        var resendsmsretryCountforReporting = 1 //MFA Reporting
        ...
        if (selectedOutcome === 3) {
            ...
            action.goTo(NodeOutcome.MAX_LIMIT);
                ...
                resendsmsretryCount++
                resendsmsretryCountforReporting = resendsmsretryCountforReporting + 1 //MFA Reporting
                nodeState.putShared("resendsmsretryCount", resendsmsretryCount);
                nodeState.putShared("resendsmsretryCountforReporting", resendsmsretryCountforReporting); //MFA Reporting
                nodeState.putShared("errorMessage_ExpiredOTP", null)
                nodeState.putShared("validationErrorCode", null)
...
nodeState.putShared("resendsmsretryCount", 1);
                ...
                nodeState.putShared("resendsmsretryCount", resendsmsretryCount);
                nodeState.putShared("resendsmsretryCountforReporting", resendsmsretryCountforReporting); //MFA Reporting
                logger.debug("the nodeSTate resendsmsretryCount"+nodeState.get("resendsmsretryCountforReporting")) //MFA Reporting
               nodeState.putShared("errorMessage_ExpiredOTP", null)
                nodeState.putShared("validationErrorCode", null)
            ...
            if (blanksmsvoiceotpCount > 2) {
                ...
                if(timediff >=298800){
                    ...
                    auditLog("VER005", "OTP validation Expired"); //MFA Reporting
...
try {
    ...
    nodeState.putShared("errorMessage_BlankOTP", null)
    nodeState.putShared("errorMessage_ExpiredOTP", null)  
    auditLog("VER005", "Error Occured"); //MFA Reporting
    action.goTo(NodeOutcome.ERROR);
}
...
function auditLog(code, message){
    try{
         ...
         var headerValues = requestHeaders.get(headerName); 
          ...
          if(nodeState.get("resendsmsretryCount")){
               var resendsmsretryCount = parseInt(nodeState.get("resendsmsretryCount")) - 1
               logger.error("the resendsmsretryCount in eventdetails1:"+resendsmsretryCount)
               eventDetails["NumberofResendCodes"] = resendsmsretryCount
           } else {
