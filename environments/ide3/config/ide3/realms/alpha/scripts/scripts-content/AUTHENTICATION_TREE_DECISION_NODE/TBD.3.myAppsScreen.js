...
function getWidgetForUser(userId){
   ...
   var isAppMatch = false;
    ...
    logger.error("Processing widget for app: " + associatedAppName +" | roleList length: " + associatedRoleList.length);

    if (associatedRoleList.length === 0) {
        //Application widget
        isAppMatch = userApps.includes(associatedAppName);
        logger.error("Application widget has no roles.This is application widget. isAppMatch=" + isAppMatch);
        ...
        logger.error("Role widget has 1 role. isRoleMatch=" + isRoleMatch +" | matchedRole=" + associatedRoleName);

    } else {
        // More than 1 role --> treat as Application widget
        isAppMatch = userApps.includes(associatedAppName);
        logger.error("Application widget detected more than 1 roles. isAppMatch=" + isAppMatch);
    }
