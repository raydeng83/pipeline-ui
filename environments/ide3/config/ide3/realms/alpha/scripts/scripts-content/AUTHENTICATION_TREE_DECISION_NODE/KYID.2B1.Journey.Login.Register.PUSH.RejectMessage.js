...
// Main Execution
try {
    nodeLogger.debug(transactionid + "::" + nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + nodeConfig.begin +"::"+"ForegRock Push notification is rejected" +"::"+mail );
...
function requestCallbacks() {
    ...
    try {
        errMsg["code"] = "ERR-2FA-FRP-002";
        errMsg["message"] = libError.readErrorMessage("ERR-2FA-FRP-002"); 
    ...
    if(selectedOutcome === 0){
       // nodeLogger.debug(transactionid + "::" + nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + nodeConfig.begin +"::"+"ForegRock Push notification is regenerated successfully" +error.message+"::"+mail );
        ...
        if(nodeState.get("journeyNameReporting") === "SelfMFAManagement" 
           ...
           || nodeState.get("journeyNameReporting") === "MFARecovery"){ //MFA Reporting
            var retryPUSHAttemptReport = 0
            if(nodeState.get("retryPUSHAttemptReport")){
                 retryPUSHAttemptReport = nodeState.get("retryPUSHAttemptReport")
                 retryPUSHAttemptReport = retryPUSHAttemptReport + 1
            } else {
                 retryPUSHAttemptReport = 1
            }
            nodeState.putShared("retryPUSHAttemptReport",retryPUSHAttemptReport) //MFA Reporting
            ...
            nodeState.putShared("retryPUSHAttempt",retryPUSHAttempt) //MFA Reporting
        }
           ...
           var userId = null;
            ...
            if (journeyName) {
                ...
                if(nodeState.get("journeyNameReporting") === "SelfMFAManagement"
                   ...
                   || nodeState.get("journeyNameReporting") === "MFARecovery"){ //MFA Reporting
                    if(nodeState.get("retryPUSHAttemptReport")){
                           eventDetails["NumberofResendCodes"] = nodeState.get("retryPUSHAttemptReport")
                       } else {
