...
if(systemEnv.getProperty("esv.kyid.kogapi.token") && systemEnv.getProperty("esv.kyid.kogapi.token")!=null) {
    //kogTokenApi = systemEnv.getProperty("esv.kyid.kogapi.token"); 
    kogTokenApi="https://sso.ide.kyid.ky.gov/am/oauth2/alpha/access_token";
 }  else {
...
if(systemEnv.getProperty("esv.kyid.kogapi.userclaims") && systemEnv.getProperty("esv.kyid.kogapi.userclaims")!=null) {
    ...
    kogUsrClaimApi = "https://uat.kogsvc.ky.gov/ide/kogapi/V1/userclaims/GetUserClaimsFR";  
}  else {
...
if(missingInputs.length>0){
    ...
    try{
        ...
        if (kogAPITokenResponse.status === 200){
            ...
            var body = {
                        upn:upn,
                        // applicationname:"AuthRequest",
                        //applicationidentifier:"https://ide3.kog.ky.gov/home/",
            ...
            logger.error("body in claims: "+JSON.stringify(body))
                    ...
                    if(mfatypeValue != 0){
                        ...
                        try{
                            ...
                            for(var i=0;i<custom_MFAMethods.length;i++){
                                ...
                                nodeLogger.error("MFA method value is: "+method.MFAMethod+" | "+method.MFAStatus)
                            ...
                            //logger.error("****mfaOptions: "+mfaOptions)
                            var factorType = mfaOptions.filter(value => usrRegisteredFactors.includes(value));
                            //logger.error("******factorType.length: "+factorType.length)
                            if(factorType.length>0){
                                nodeLogger.error("MFA Registeration not allowed");
