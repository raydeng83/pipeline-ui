...
function main() {
  ...
  nodeLogger.log("debug", nodeConfig, "mid", txid, "username in session is - "+username);
      
    if (username) {
        userId = getIdFromUsername(username);
        nodeLogger.log("debug", nodeConfig, "mid", txid, "userID in session is -" + userId);
        ...
        if (callbacks.isEmpty()) {
            ...
            if(selectedOutcome === 0){
                ...
                if (entryHasValidKeys && entryHasValidName && entry.action.toLowerCase() === "view") {
                  ...
                  nodeLogger.log("debug", nodeConfig, "mid", txid, "Invalid entry ");
                  nodeState.putShared("retry",true);
                  action.goTo(nodeOutcome.RETRY);
                 }
             }   
