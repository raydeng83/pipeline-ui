...
function getListPrerequisite(_id, enteredFormName) {
  ...
  var obj = {};
    ...
    logger.debug("OBJ is in "+ JSON.stringify(obj))
    return prereqList;
}
...
function parseExpiredDate(date) {
  ...
  if (dateString.length !== 8) {
    logger.debug("Invalid date length:", dateString);
    return "N/A";
  }
...
function main() {
  ...
  ops = require("KYID.2B1.Library.IDMobjCRUDops");
    ...
    if (username!= null && username) {
        ...
        nodeLogger.log("debug", nodeConfig, "mid", txid, "verifiedRequests is - " + JSON.stringify(verifiedRequests));
    } else {
        nodeLogger.log("debug", nodeConfig, "mid", txid, "Invalid username"); 
        ...
        if (callbacks.isEmpty()) {
            ...
            if(selectedOutcome === 0){
                ...
                //logger.debug("enteredForm is "+ enteredForm)
                if(enteredForm!== null && enteredForm && Array.isArray(enteredForm) ){
                    nodeLogger.log("debug", nodeConfig, "mid", txid, "enteredForm is - "+JSON.stringify(enteredForm));
                    //entry = Array.isArray(enteredForm) && enteredForm.length === 1 ? enteredForm[0] : nodeLogger.log("debug", nodeConfig, "mid", txid, "enteredForm is not a valid Array");
                    entry = enteredForm[0];
                    nodeLogger.log("debug", nodeConfig, "mid", txid, "entry is - " +entry["Name"]);
                    //logger.debug("entry is - " +entry["Name"]);
                    ...
                    nodeLogger.log("debug", nodeConfig, "mid", txid, "hasValidKeys is - " + entryHasValidKeys+" nameExists is - " + entryHasValidName);
                    ...
                    if(enteredFormName!==null && enteredFormName){
                        ...
                        //logger.debug("contextId issss "+ contextID[0].contextid)
                        ...
                        if(nodeState.get("policy")!== null && nodeState.get("policy")){
                            ...
                            nodeLogger.log("debug", nodeConfig, "mid", txid, "Invalid data entered"); 
                            nodeState.putShared("retry",true);
                        ...
                        if(prereqListArray!==null && prereqListArray){
                          ...
                          nodeState.putShared("preqtypeArray",preqtypeArray)
                            ...
                            nodeLogger.log("debug", nodeConfig, "mid", txid, "Invalid form Name entered"); 
                            nodeState.putShared("retry",true);
                            errMsg = nodeLogger.readErrorMessage("KYID101"); 
                            nodeState.putShared("readErrMsgFromCode",JSON.stringify(errMsg));
                            nodeLogger.log("debug", nodeConfig, "mid", txid, JSON.stringify(errMsg));
                            ...
                            nodeLogger.log("debug", nodeConfig, "mid", txid, "Invalid form entered");
                            nodeState.putShared("retry",true);
                ...
                if (entryHasValidKeys && entryHasValidName && entry.action.toLowerCase() === "view" ) {
                    ...
                    nodeLogger.log("debug", nodeConfig, "mid", txid, "You cannot view this Prerequisite"); 
                    nodeState.putShared("retry",true);
