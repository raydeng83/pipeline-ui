...
logger.debug("Searching User in AD")
...
function entitlementRequest (userId,EntitlementId) {
    try {
logger.debug("User ID is" + userId+ "Entitlement ID is "+ EntitlementId);  
var body = {
  "common": {
    "userId": userId,
...
logger.debug("Entitlement Request Response Is ::" + Response)
...
logger.debug("Entitlement ID is ::"+ Response.result[0].id);
...
logger.debug("Group DN is :: " + entitlementName);
var entitlementId = getEntitlementId(entitlementName)
if (entitlementId == -1){
    logger.debug("Unexptected Error Occurred ")
}
else{
logger.debug("entitlementInfo is Successful")
// var entitlementId= entitlementInfo.result[0]._id;
logger.debug("Entitlement ID is::" +entitlementId);
}
//
var requestResponse = entitlementRequest(userId,entitlementId);
logger.debug("Response of Function entitlementRequest is ::"+ requestResponse);
logger.debug("Reqest Id is "+requestResponse.id)
if (requestResponse == -1){
    ...
    logger.debug("Entitlement Grant Request ID is :: " +requestId);
    outcome = "true"
}
