...
function createRequestURL(reqIDCookies) {
    try {
        ...
        if (requestParameters && requestParameters.get("goto") && requestParameters.get("goto") != null) {
            ...
            // logger.debug("decodeReqIdStr"+decodeReqIdStr)
            // var ReqID=decodeReqIdStr.split("=")[1]
            // logger.debug("Final_RequestId:"+ReqID)
            ...
            if (missingRequestParamsOrHeaders.length > 0) {
                ...
                //      if(EntityID.localeCompare("https://dev2.kyid.ky.gov/callback") == 0) {
                //         nodeState.putShared("EntityID","https://dev2.kog.ky.gov/home/");
                //         logger.debug("The entityID stored in nodeState is : "+"https://dev2.kog.ky.gov/home/")
                //     } else {
                //         nodeState.putShared("EntityID",EntityID);
                //         logger.debug("The entityID stored in nodeState is : "+EntityID)
                //     }

                //     if (systemEnv.getProperty("esv.kyid.portal.entityid") && systemEnv.getProperty("esv.kyid.kog.home")) {
                //     var portalentityID = systemEnv.getProperty("esv.kyid.portal.entityid");
                //     var koghomeurl = systemEnv.getProperty("esv.kyid.kog.home");

                //     if (EntityID.localeCompare(portalentityID) === 0) {
                //         nodeState.putShared("EntityID", koghomeurl);
                //         logger.debug("The entityID stored in nodeState is: " + koghomeurl);
                //     } else {
                //         nodeState.putShared("EntityID", EntityID);
                //         logger.debug("The entityID stored in nodeState is: " + EntityID);
                //     }
...
function main() {
    ...
    try {
        ...
        if (requestParameters && requestParameters.get("goto") && requestParameters.get("goto") != null) {
            ...
            logger.debug("gotoURL in SSO request - " + gotoLink)
            reqID = getReqIDFromGoto(gotoLink)
            logger.debug("reqID in SSO request - " + reqID)
            if (reqID != null) {
                reqIDCookies = readReqIDCookieFromSession(reqID)
                nodeState.putShared("reqIDCookies", reqIDCookies);
                logger.debug("reqIDCookies values are - " + JSON.stringify(reqIDCookies))
            }
