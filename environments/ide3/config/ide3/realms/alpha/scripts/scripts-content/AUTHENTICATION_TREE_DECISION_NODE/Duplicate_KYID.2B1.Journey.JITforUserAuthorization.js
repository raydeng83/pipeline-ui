        ...
        if (data.ResponseStatus === 0 && data.UserAuthorizations) {
           // nodeState.putShared("usrcreatedId","c6657e2d-c325-41d9-9822-56ce4088a8e8")
                        ...
                        if (roleQuery.result.length === 1) {
                            ...
                            if (userRoleIds.indexOf(roleId) === -1) {
                                if (assignedRoleIds.indexOf(roleId) === -1) {
                                    ...
                                    assignedAppRolePairs.push({ appId: appId, roleId: roleId, userId: id });
                                    //updateAccessMO(id, appId,roleId)
                                    logger.error("Mapped NEW Role: " + auth.RoleName + " under App: " + auth.ApplicationName);
                                } else {
            ...
            if (rolesToAssign.length > 0) {
                ...
                // createAccessMO for each app-role pair
                     assignedAppRolePairs.forEach(function (pair) {
                     try {
                          createAccessMO(pair.appId, pair.roleId, pair.userId);
                        } catch (err) {
                         logger.error("Access MO creation failed for appId: " + pair.appId + " and roleId: " + pair.roleId);
                        }
                    });
...
function createAccessMO(appId,roleId,id){
...
try{
    ...
    if(response){
    logger.error("Access MO Entry Created"+response);
    return
    }
    else{
    logger.error("Access Entry Failed");
    return
    }
}                 
catch (error) {
