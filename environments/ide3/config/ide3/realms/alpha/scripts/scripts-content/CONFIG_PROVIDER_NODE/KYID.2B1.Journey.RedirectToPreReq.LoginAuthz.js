...
try {
    ...
    var contextID = requestParameters.get("contextID").get(0)
if(contextID.startsWith("\"")) {
    contextID = contextID.substring(1,contextID.length-1);
}
    //var prerequrl = "https://" + url + "/am/XUI/?realm=alpha&authIndexType=service&authIndexValue=kyid_2B1_PrerequisitesEnrolment" + "&contextID=" + contextID;
    //var prerequrl = "https://selfservice.dev.kyid.ky.gov:5173/applicationEnrollment/"+ contextID;
    var prerequrl = "/applicationEnrollment/"+ contextID;
    logger.error("prerequrl is "+prerequrl)
    config = {
        "successUrl": prerequrl
    }
}
catch (e) {
