...
var nodeConfig = {
    ...
    node: "Node",
    nodeName: "Route to LoginEN",
    script: "Script",
    scriptName: "KYID.2B1.Journey.BackToLoginEN",
    timestamp: dateTime,
...
try {
    ...
    // logger.debug("Request URL Value is ---" +readCookieValueFromSession("RequestURL"))
    var RequestURL= readCookieValueFromSession("RequestURL")
    logger.debug("Request URL from Session is ... " +RequestURL )
    if(nodeState.get("TransactionSessionOver") === "True" && RequestURL.includes("composite_advice")){
       var ReturnURL=readCookieValueFromSession("ReturnURL")
        ...
        nodeLogger.debug(nodeConfig.timestamp+"::"+nodeConfig.node+"::"+nodeConfig.nodeName+"::"+nodeConfig.script+"::"+nodeConfig.scriptName+"URL has TransactionID and Transaction session has maxed out"+ link); 
        action.goTo("redirect");
    }
    else{
    RequestURL = RequestURL.replace(/\\/g, '')
    // logger.debug("Request URL Value is Cleaned One ---" +RequestURL)
    ...
    if (RequestURL === undefined || !RequestURL || RequestURL === null ) {
        ...
        var link= vlink+"&clocale=en&locale=en";
