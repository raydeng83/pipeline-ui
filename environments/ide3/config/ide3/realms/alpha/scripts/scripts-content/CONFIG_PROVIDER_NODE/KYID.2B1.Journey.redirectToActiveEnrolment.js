...
try {
    ...
    var appIDReq = nodeState.get("appIDReq").toString();
if(appIDReq.startsWith("\"")) {
    appIDReq = appIDReq.substring(1,appIDReq.length-1);
}
    //var activeenrolmenturl = "https://" + url + "/am/XUI/?realm=alpha&authIndexType=service&authIndexValue=kyid_2B1_getActiveEnrollment" + "&appIDReq=" + appIDReq;
   // var activeenrolmenturl = "https://selfservice.dev.kyid.ky.gov:5173/authorization/active-enrollment" + "?appId=" + appIDReq;
    var activeenrolmenturl = "/authorization/active-enrollment" + "?appId=" + appIDReq;
    logger.error("active enrolment url"+activeenrolmenturl)
    config = {
        "successUrl": activeenrolmenturl
    }
}
catch (e) {
