...
function preAuthentication() {
	 logger.error("************INSIDE PREAuthentication FUNCTION*************"+reqId);
    ...
    if (authnRequest) {
        ...
        response.addHeader("Set-Cookie", "issuer=" + issuer + "; Path=/; HttpOnly; sameSite=Lax; domain="+tenantCookieDomain); // set issuer in cookie

        if (issuer.localeCompare("kyidsp") == 0 && authnRequest.getExtensions()) {
            ...
            response.addHeader("Set-Cookie", "tmp-displayUrl=" + decodeURIComponent(displayUrl) + "; Path=/; HttpOnly; sameSite=Lax; domain="+tenantCookieDomain); // set temporary displayUrl in cookie
            response.addHeader("Set-Cookie", "tmp-wtrealmUrl=" + decodeURIComponent(wtrealmUrl) + "; Path=/; HttpOnly; sameSite=Lax; domain="+tenantCookieDomain); // set temporary wtrealmUrl in cookie
            response.addHeader("Set-Cookie", "tmp-isWsfedApp=" + isWsfedApp + "; Path=/; HttpOnly; sameSite=Lax; domain="+tenantCookieDomain); // set temporary isWsfedApp in cookie
        }

        var reqID = authnRequest.getID();
        response.addHeader("Set-Cookie", "reqID=" + reqID + "; Path=/; HttpOnly; sameSite=Lax; domain="+tenantCookieDomain); // set reqID in cookie
    }
...
function preSendResponse() {
    logger.error("************INSIDE PRESEND RESPONSE FUNCTION*************"+reqId);
    /* This method assumes that the authentication is only SP Initiated and not IDP initiated*/
    var displayUrl = null;
    var wtrealmUrl = null;
    var isWsfedApp = false;
    ...
    if(request.getHeader("Cookie") && request.getHeader("Cookie")!=null){
        ...
        for(i=0;i<rawCookieParams.length;i++){
          ...
          if(cookie.localeCompare("tmp-displayUrl")==0){
              displayUrl = cookieName[1];
            } 

            if(cookie.localeCompare("tmp-wtrealmUrl")==0){
                wtrealmUrl = cookieName[1];
              } 

              if(cookie.localeCompare("tmp-isWsfedApp")==0){
                isWsfedApp = Boolean(cookieName[1]);
              } 

              if(cookie.localeCompare("issuer")==0){
                issuer = cookieName[1];
              } 
    ...
    try {
        ...
        if (issuer.localeCompare(systemEnv.getProperty("esv.logout.app.url")) == 0) {
            ...
            // setAppContextInSessionProps(realm,request,authnRequest,response,reqId,session,relayState);
            setAppContextInSessionProps(realm,request,displayUrl,wtrealmUrl,isWsfedApp,issuer,finalUrl, response,reqId,session,relayState)
            var lang = readLangFromSessionCookie(request);
...
function setAppContextInSessionProps(realm,request,displayUrl,wtrealmUrl,isWsfedApp,issuer,finalUrl, response,reqId,session,relayState){
    ...
    // var finalUrl = null;
  	...
  	logger.error("************issuer value in setAppContextInSessionProps method*************"+issuer);
    ...
    if (issuer.localeCompare(systemEnv.getProperty("esv.logout.app.url")) == 0 && !session) {
        ...
        displayUrl = issuer;
    } else if (issuer.localeCompare("kyidsp") == 0 && isWsfedApp) {
		// logger.error("************authnRequest.getExtensions() value in setAppContextInSessionProps method*************"+authnRequest.getExtensions().getAny());
        //wsfed application coming from on-prem app
        // var authnRequestExtensionList=authnRequest.getExtensions().getAny();
        //logger.error("SAML PreAuthentication AUTHNREQUEST-EXTLIST: "+authnRequestExtensionList);
        // var rawDisplayUrl=authnRequestExtensionList.get(0).split("saml2:AttributeValue")[1];
        // displayUrl=rawDisplayUrl.substring(1,rawDisplayUrl.length()-2);
        response.addHeader("Set-Cookie", "ReturnURL=" + decodeURIComponent(displayUrl) + "; Path=/; HttpOnly");
        logger.error("************displayUrl in a ReturnURL cookie in setAppContextInSessionProps method*************"+displayUrl);
        //logger.error("Set displayUrl in a ReturnURL cookie: " + displayUrl);
        ...
        // }
        response.addHeader("Set-Cookie", "rp-realm=" + decodeURIComponent(wtrealmUrl) + "; Path=/; HttpOnly");
        logger.error("************wtrealmUrl in a rp-realm cookie in setAppContextInSessionProps method*************"+wtrealmUrl);
        logger.error("************WSFed PreAuthentication DISPLAY URL in setAppContextInSessionProps method*************"+displayUrl);
        //logger.error("Set wtrealmUrl in a rp-realm cookie: " + wtrealmUrl);
        //logger.error("WSFed PreAuthentication DISPLAY URL: "+displayUrl);
        // isWsfedApp=true;
    } else if(issuer.localeCompare("kyidsp") == 0 ) {
            ...
            response.addHeader("Set-Cookie", "ReturnURL=" + decodeURIComponent(displayUrl) + "; Path=/; HttpOnly");
            response.addHeader("Set-Cookie", "rp-realm=" + decodeURIComponent(displayUrl) + "; Path=/; HttpOnly");
            logger.error("************Default DISPLAY URL in setAppContextInSessionProps method*************"+displayUrl);
    } else if(relayState !== null && relayState.startsWith("https")) {
        //SAML Application with a full proper relayState URL
        displayUrl = relayState;
        response.addHeader("Set-Cookie", "ReturnURL=" + decodeURIComponent(displayUrl) + "; Path=/; HttpOnly");
    ...
    response.addHeader("Set-Cookie", "locale=" + lang + "; Path=/; HttpOnly; sameSite=Lax; domain="+tenantCookieDomain);
    response.addHeader("Set-Cookie", "clocale=" + lang + "; Path=/; HttpOnly; sameSite=Lax; domain="+tenantCookieDomain);
    logger.error("Language value in a locale cookie in setAppContextInSessionProps method: " + lang);
