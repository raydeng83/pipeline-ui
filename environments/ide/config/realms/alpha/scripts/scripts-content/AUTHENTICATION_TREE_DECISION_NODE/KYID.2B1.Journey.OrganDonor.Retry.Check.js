...
function main() {
    ...
    try{
        ...
        var ridpFlag = response.result[0].ridp_organ_donor_version ? response.result[0].ridp_organ_donor_version : "v2";
        logger.error("KYID.2B1.Journey.ManageProfile.RIDP.Version.Flag response from query :: " + ridpFlag)
        if(ridpFlag && ridpFlag === "v2"){
         if(nodeState.get("orig_proofingMethod") && (nodeState.get("orig_proofingMethod") == "4" || nodeState.get("orig_proofingMethod") == "-1" || nodeState.get("orig_proofingMethod") == "2")){
                if(response.result[0].ridp_organ_donor_retry_limit){
                    var retryLimit = response.result[0].ridp_organ_donor_retry_limit
                    if(verificationAttempt >= retryLimit){
                        diffInDays = diffInDays();
                        if(response.result[0].ridp_organ_donor_refresh_limit){
                            var refreshLimit = response.result[0].ridp_organ_donor_refresh_limit
