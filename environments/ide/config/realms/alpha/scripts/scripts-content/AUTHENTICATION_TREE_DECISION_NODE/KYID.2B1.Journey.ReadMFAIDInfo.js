...
function fetchMFAIDFromMO(id) {
    try {
        ...
        if (mfaQueryResult.result && mfaQueryResult.result.length > 0) {
            ...
            logger.error("fetchMFAIDFromMO: No MFA record found for id=" + id);
            return null;
        }
    } catch (error) {
...
function fetchEndUserDetails(KOGID) {
    try {
        ...
        if (userQueryResult.result && userQueryResult.result.length > 0) {
            ...
            logger.error("fetchEndUserDetails: Found user for KOGID=" + KOGID);
            return user;
        } else {
            logger.error("fetchEndUserDetails: No user found for KOGID=" + KOGID);
            return null;
        }
    } catch (error) {
...
function main() {
    ...
    try {
        ...
        if (requestParameters.get("mfaid")) {
            ...
            logger.error("MFAIDInfo: " + JSON.stringify(MFAIDInfo));
            logger.error("Param Value: " + mfaId);
            ...
            logger.error("No mfaid parameter found in request");
