...
function validateUser(email) {
    ...
    try {
        ...
        /*logger.error("Validate email exist or not - "+email)
        var response = openidm.query("managed/alpha_user/", { "_queryFilter": '/mail eq "'+email+'"'}, [""]);
        logger.error("User Response is :: " + JSON.stringify(response))
        var usrRecords = response.result
        if(response.resultCount > 0){
            for(var i=0;i<usrRecords.length;i++){
                createNewAccount = false
                logger.error("User record => "+JSON.stringify(usrRecords[i]))
                if(usrRecords[i].accountStatus=="Terminated"){
                    createNewAccount = true
                }
            }   
            logger.error("createAccount? => "+createNewAccount)
            if(createNewAccount){
                return false
            } else {
                return true
