...
var nodeConfig = {
    ...
    script: "Script",
    scriptName: "KYID.Journey.MFAAuthentication.TwilioVerificationCheck",
    timestamp: dateTime,
...
function getDisplayMessage() {
    ...
    if (clocale === "es") {
        ...
        var displayMessage = "Se envió un código a " + boldoutput.bold() + ". Ingrese el código a continuación para verificar su identidad." + "<br><br>Puede que su operadora telefónica le cobre el envío de mensajes de SMS."
    }
...
function handleUserResponses() {
    try {
        ...
        var sendOTPTimestamp= nodeState.get("sendOTPTimestamp")
        logger.error("sendOTPTimestamp in VerificationCheck"+ sendOTPTimestamp)
        var currentdateTime = new Date().toISOString();
        var timeDifference = new Date(currentdateTime) - new Date(sendOTPTimestamp)
        logger.error("timeDifference:"+timeDifference)
        ...
        if (selectedOutcome === 2) {
            ...
            action.goTo(NodeOutcome.EMPTY_OTP);
                ...
                if(!verifyOTP(password) && timeDifference>=300000){
                nodeState.putShared("errorMessage", null)
                nodeState.putShared("resendcodeMessage", null)
                nodeState.putShared("errorMessage_BlankOTP", null)
                logger.error("Inside_ExpiredOTP")
