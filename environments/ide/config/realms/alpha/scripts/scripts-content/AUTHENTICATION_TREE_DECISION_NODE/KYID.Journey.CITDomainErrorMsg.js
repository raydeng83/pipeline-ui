...
// Node Config
 var nodeConfig = {
    ...
    node: "Node",
    nodeName: "CIT Domain Error",
    script: "Script",
    scriptName: "KYID.Journey.CITDomainErrorMsg",
    timestamp: dateTime,
    errorMsg_EN: "esv.error.iscitdomainpart.en",
    errorMsg_ES: "esv.error.iscitdomainpart.es", 
    ...
    if(!isMandatoryConfigParamsPresent()){
        action.goTo(nodeOutcome.ERROR);
         ...
         try{
             ...
             isCITDomainPart = libFunc.setErrorMessage(getLocale(),errLangMsgJSON);
             nodeState.putShared("isCITDomainPart", isCITDomainPart);
             nodeLogger.debug(nodeConfig.timestamp+"::"+nodeConfig.node+"::"+nodeConfig.nodeName+"::"+nodeConfig.script
                     +"::"+nodeConfig.scriptName+"::"+nodeConfig.errorMsg+"::"+isCITDomainPart);
