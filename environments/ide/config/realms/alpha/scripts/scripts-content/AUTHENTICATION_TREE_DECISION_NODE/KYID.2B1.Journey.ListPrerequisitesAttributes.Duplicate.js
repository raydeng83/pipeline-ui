...
function calculateExpiryDate(type, value) {
    ...
    if (type === 'dayOfMonth') {
        ...
        if (typeof value === 'string') {
            ...
            logger.error("Invalid value type for oneTimeDueDate");
        }
...
logger.error("response is in getPrerequisitePageAttrs "+ JSON.stringify(response));
    return response;
}
...
function getPageDetails(uuid, prereqID, compPreqPagesArray, type) {
    ...
    logger.error("jsonObjPage is in "+ JSON.stringify(jsonObjPage));
    return jsonObjPage;
}
...
function getSubmittedPrereqPagesDetails(uuid, type, pageNumber,saveinput) {
    ...
    logger.error("Rlogs::compPreqPagesArray value is - " + compPreqPagesArray);
...
function getRequestStatus(requestId) {
    try {
        ...
        if (record.code == -1) {
            ...
            logger.error("response of request is ====" + record)
