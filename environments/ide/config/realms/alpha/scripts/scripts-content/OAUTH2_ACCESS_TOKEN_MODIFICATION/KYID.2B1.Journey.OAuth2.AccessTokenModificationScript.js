...
if(logonAppId !== null && authTime !== null && isLoginPrereqMandatory(logonAppId,authTime)){
   accessToken.setField("roleCategoryName", "KYID-Login-Prerequisite"); 
    ...
    appName = "KYID Business Support Portal"
...
if(session && session.getProperty("sessionRefId") && session.getProperty("sessionRefId")!=null){
    var sessionRefId = session.getProperty("sessionRefId")
    logger.error("KYID.2B1.Journey.OAuth2.AccessTokenModificationScript sessionRefId :: " + sessionRefId)
    accessToken.setField("sessionRefId", session.getProperty('sessionRefId'));
}
