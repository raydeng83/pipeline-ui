...
(function () {
     ...
     */
    logger.error("##### KYID.2B1.Journey.OIDC.IdTokenModificationScript :: Begining Execution");

    var utils = getUtils();
    var userIdentityAttributes = getUserIdentityAttributes();
    if (!userIdentityAttributes) {
        userIdentityAttributes = { givenName: "", sn: "", name: "" };
    }
    logger.error("##### KYID.2B1.Journey.OIDC.IdTokenModificationScript :: User Identity Attributes " + JSON.stringify(userIdentityAttributes));
    //logger.error("userType is in ther")
    ...
    utils.setClaimResolvers({
        ...
        */
         ...
         */
        name: utils.getUserIdentityClaims('name'),
        family_name: utils.getUserIdentityClaims('sn'),
        given_name: utils.getUserIdentityClaims('givenName'),
        ...
        user_type: utils.getUserProfileClaimResolver('fr-attr-str1'),
    ...
    function getUtils() {
        ...
        function getUserProfileClaimResolver(attributeName) {
            ...
            function resolveClaim(claim) {
                ...
                if (identity) {
                    logger.error(attributeName + " userProfileValue is in " + identity.getAttribute(attributeName))
        ...
        function getUserInternalRoles() {

            function resolveInternalRoleClaim() {

                try {

                    var userCustomAttributes = identity.getAttribute("fr-idm-custom-attrs");
                    ...
                    message = 'OIDC Claims script exception. Unable to get user roles. ' + e;
                }

            }
            return resolveInternalRoleClaim;

        }

        function getUserIdentityClaims(attributeName) {
            function resolveIdentityClaim() {
                return userIdentityAttributes[attributeName];
            }
            return resolveIdentityClaim;
        }
        ...
        return {
            ...
            getUserInternalRoles: getUserInternalRoles,
            getUserIdentityClaims: getUserIdentityClaims
        };
    }

    function getUserIdentityAttributes() {
        var hostUrl = requestProperties.get('requestUri').getHost();
        logger.error("##### KYID.2B1.Journey.OIDC.IdTokenModificationScript :: AM Host URL : " + hostUrl);
        ...
        var username = identity.getAttribute("mail").toArray()[0];
        var scope = "fr:idm:*";
        if (!hostUrl || hostUrl.trim() === "") {
            logger.error("KYID.2B1.Journey.OIDC.IdTokenModificationScript :: Tenant FQDN is null. Unable to proceed");
            return null;
        }
        ...
        logger.error("##### KYID.2B1.Journey.OIDC.IdTokenModificationScript :: Token URL : " + tokenUrl);
        //logger.error("##### KYID.2B1.Journey.OIDC.IdTokenModificationScript :: Request Body : " + requestBody);
        var tokenResponse = httpClient.send(tokenRequest).get();
        logger.error("##### KYID.2B1.Journey.OIDC.IdTokenModificationScript :: Bearer Token tokenResponse : " + tokenResponse);
        var tokenApiResponse = tokenResponse.getEntity().getString();
        logger.error("##### KYID.2B1.Journey.OIDC.IdTokenModificationScript :: Bearer Token tokenApiResponse : " + tokenApiResponse);
        var jsonObject = JSON.parse(tokenApiResponse)
        var bearerToken = jsonObject.access_token;
        logger.error("##### KYID.2B1.Journey.OIDC.IdTokenModificationScript :: Bearer Token : " + bearerToken);
        //var bearerToken = "";
        if (bearerToken) {
            var request = new org.forgerock.http.protocol.Request();
            request.setUri("https://" + hostUrl + "/openidm/managed/alpha_user?_queryFilter=mail+eq+'" + encodeURIComponent(username) + "'&_fields=userName,custom_userIdentity/sn,custom_userIdentity/givenName");
            request.setMethod("GET");
            request.getHeaders().add("Authorization", "Bearer " + bearerToken);
            var response = httpClient.send(request).get();
            var apiResponse = response.getEntity().getString();
            logger.error("##### KYID.2B1.Journey.OIDC.IdTokenModificationScript :: API Response : " + apiResponse);
            ...
            if (userAttributeJson && userAttributeJson.result[0] && userAttributeJson.result[0].custom_userIdentity) {
                var responseIdentityAttribute = {};
                responseIdentityAttribute.givenName = userAttributeJson.result[0].custom_userIdentity.givenName;
                responseIdentityAttribute.sn = userAttributeJson.result[0].custom_userIdentity.sn;
                responseIdentityAttribute.name = userAttributeJson.result[0].custom_userIdentity.givenName+" "+userAttributeJson.result[0].custom_userIdentity.sn;
                return responseIdentityAttribute;
            } else {
                return null;
            }
        }
