...
function saveUserPrerequisiteValues(requestObj) {
    ...
    if (saveInput) {
        ...
        openidm.action(
            ...
            "POST", {
                comment: "Saving request credentials for user."
            }, {
                _action: "update"
            }
        );
        logger.info("KOG API call comment added for request " + requestId + ": " + "Saving request credentials for user.");
        ...
        if (responseKOGCredentialsAPI.apiStatus == 200) {
            if (responseKOGCredentialsAPI.ResponseStatus == 0) {
                try {
                    openidm.action(
                        "iga/governance/requests/" + requestId,
                        "POST", {
                            comment: "Request credentials saved successfully."
                        }, {
                            _action: "update"
                        }
                    );
                    logger.info("KOG API call comment added for request " + requestId + ": " + "Request credentials saved successfully.");
            ...
            openidm.action(
                "iga/governance/requests/" + requestId,
                "POST", {
                    comment: "Request credentials saving failed."
                }, {
                    _action: "update"
                }
            );
            logger.info("KOG API call comment added for request " + requestId + ": " + "Request credentials saving failed.");
        ...
        openidm.action(
            "iga/governance/requests/" + requestId,
            "POST", {
                comment: "Saving user onboarding information."
            }, {
                _action: "update"
            }
        );
        logger.info("KOG API call comment added for request " + requestId + ": " + "Saving user onboarding information.");
        ...
        if (responseUserOnboardingAPI.apiStatus == 200) {
            if (responseUserOnboardingAPI.ResponseStatus == 0) {
                ...
                openidm.action(
                    "iga/governance/requests/" + requestId,
                    "POST", {
                        comment: "User onboarding information saved successfully."
                    }, {
                        _action: "update"
                    }
                );
                logger.info("KOG API call result comment added for request " + requestId + ": " + "User onboarding information saved successfully.");
            ...
            openidm.action(
                "iga/governance/requests/" + requestId,
                "POST", {
                    comment: "User onboarding information saving failed."
                }, {
                    _action: "update"
                }
            );
            logger.info("KOG API call result comment added for request " + requestId + ": " + "User onboarding information saving failed.");
    ...
    if (isPendingPrereq == false) {
        ...
        if (responseAccess.status = 'success') {
            openidm.action(
                "iga/governance/requests/" + requestId,
                "POST", {
                    comment: "Access provisioning success."
                }, {
                    _action: "update"
                }
            );
            logger.info("KOG API call comment added for request " + requestId + ": " + "Access provisioning success.");
        } else {
            openidm.action(
                "iga/governance/requests/" + requestId,
                "POST", {
                    comment: "Access provisioning failed."
                }, {
                    _action: "update"
                }
            );
            logger.info("KOG API call comment added for request " + requestId + ": " + "Access provisioning failed.");
        }
