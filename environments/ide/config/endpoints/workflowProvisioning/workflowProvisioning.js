//https://<PingHostURL>/openidm/endpoint/workflowProvisioning
...
logger.error("Inside provisioning endpoint");
...
function saveUserPrerequisiteValues(requestObj, requestId) {
            ...
            if (responseKOGCredentialsAPI.ResponseStatus == 0) {
                try {
                    ...
                    if (requestObj.request.custom.page.values && requestObj.request.custom.page.values != null) {
                        ...
                        logger.error("kyid-workflow fieldValuesJson: " + JSON.stringify(fieldValuesJson));
                        ...
                        //var result = openidm.update("managed/alpha_kyid_enrollment_user_prerequisites/" + userPrereqObj._id, null, userPrereqObj);

                        logger.error("kyid-workflow patch result: " + result);
                    }
    ...
    logger.error("kyid-workflow shouldPatchUserPrerequisite:" + shouldPatchUserPrerequisite);

    if (shouldPatchUserPrerequisite) {
        logger.error("kyid-workflow userpreqid:" + requestObj) //.custom.userPrerequisiteId);
    ...
    logger.error("kyid-workflow patch user prerequisite completed successfully");
