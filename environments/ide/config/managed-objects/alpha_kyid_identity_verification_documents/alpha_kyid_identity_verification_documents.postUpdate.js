...
if (endpointExecution && endpointExecution === "true") {
    ...
    if (enableAuditLogHistory && enableAuditLogHistory === "true") {
        ...
        //logger.error("Response from Audit History endpoint for UPDATE event on alpha_kyid_identity_verification_documents object:::: => " + JSON.stringify(response));
        } catch (error) {
            logger.error("An error occurred while updating the UPDATE event audit History events for alpha_kyid_identity_verification_documents object");
        }
    }   
}