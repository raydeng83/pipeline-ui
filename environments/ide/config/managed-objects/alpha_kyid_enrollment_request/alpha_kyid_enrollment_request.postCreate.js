...
if (endpointExecution && endpointExecution === "true") {
    ...
    if (enableAuditLogHistory && enableAuditLogHistory === "true") {
        try {
            ...
            //logger.error("Response from Audit History endpoint for create event on alpha_kyid_enrollment_request object:::: => " + JSON.stringify(response));
        } catch (error) {
            logger.error("An error occurred while updating the CREATE event audit History events for alpha_kyid_enrollment_request object");
        }
    }
}