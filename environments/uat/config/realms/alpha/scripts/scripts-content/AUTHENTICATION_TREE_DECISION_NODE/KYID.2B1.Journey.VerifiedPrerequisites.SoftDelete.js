...
function queryPrerequisiteRequest(userId, roleID, value) {
  logger.error("Inside queryPrerequisiteRequestContexID");
  ...
  try {
    ...
    logger.error("Successfully queried record in alpha_kyid_request managed object :: " +JSON.stringify(recordRequest));
    recordRequestJSONObj = JSON.parse(JSON.stringify(recordRequest.result));
    logger.error("recordRequestJSONObj" + JSON.stringify(recordRequest.result));
    contextID= recordRequest.result[0].contextid;
      logger.error("contextID is in "+ contextID)
  ...
  try {
        if(Array.isArray(type)){
                type.forEach(value=>{
                        ...
                        logger.error("recordRequest is in "+ JSON.stringify(recordRequest))
      ...
      logger.error("prereqRemove is "+ prereqRemove)
