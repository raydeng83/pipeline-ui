...
function getListOfPrereqTypes(txid,nodeLogger,ops,arrPreReqTypeIDs){
  ...
  try { 
        for(var j=0;j<arrPreReqTypeIDs.length;j++){
          ...
          if(recordPrereqTypes.result[0].typeofprerequisite.toLowerCase() === "mfa"){
              nodeState.putShared("MFAPrerequisite",JSON.stringify(recordPrereqTypes.result[0]));
          }
  ...
  try { 
      logger.debug("prereqname is "+prereqname);
        ...
        logger.debug("recordPrereqTypes.result[0].typeofprerequisite - "+recordPrereqTypes.result[0].typeofprerequisite)
          ...
          else if(recordPrereqTypes.result[0].typeofprerequisite.toLowerCase() === "mfa"){
              nodeState.putShared("typeofprerequisite","mfa");
          }
          else {
    ...
    try{
        ...
        if(result === false){
            var patch = [
             ...
             var resourcePath = ("managed/alpha_user/"+uuid).toString();

             var addRoleResult = openidm.patch("managed/alpha_user/"+uuid,null, patch);
          ...
          if(policyID!=null && policyID){
             arrPreReqTypeIDs = getPolicyDetails(txid,nodeLogger,ops,policyID);
                 if(arrPreReqTypeIDs.length>0){
                    ...
                    addRoleResponse = addRole(roleID, uuid);
                    logger.debug("addRoleResponse is in "+ JSON.stringify(addRoleResponse));
                 }
           }else{
               addRoleResponse = addRole(roleID, uuid); 
               logger.debug("addRoleResponse is in "+ JSON.stringify(addRoleResponse))
...
function patchPrerequisiteRequestStatus(ops,txid,nodeLogger,id,userID,type,status){
    ...
    var content = {};
    var contentArray = [];
    var nullifyMFAPrereq = false;
   nullifyMFAPrereq= nodeState.get("nullifyMFAPrereq");
    try{
        recordRequest = openidm.query("managed/alpha_kyid_request", { "_queryFilter" : 'contextid eq "'+id+'"' 
        + 'and type eq "'+type+'"' 
        + 'and requester eq "'+userID+'"'}, ["contextid", "type", "requester", "status"]);
        recordRequestJSONObj = JSON.parse(JSON.stringify(recordRequest.result[0]));
        if(nullifyMFAPrereq === true){
        ...
        else if(!(recordRequestJSONObj["status"]==="COMPLETED" || recordRequestJSONObj["status"]==="SKIPPED") ){
            content["operation"] = "replace";
            content["field"] = "status";
            content["value"] = status ;
            contentArray.push(content);
...
function alphaAppResponse(appId,ops){
    try {
        ...
        logger.error("Error Occurred while getting alpha application-"+error);
    }
    
}
function queryBusinessApplication(businessAppId,ops){
    try {
         var response = ops.crudOps("read", "alpha_kyid_businessapplication", null, null, null, businessAppId);
        if(response){
            return response;
        }
    } catch (error) {
        logger.error("Error Occurred while queryBusinessApplication-"+error);
    }
    
}

function getUserMFAMethod(){
    try {
         var response = openidm.query("managed/alpha_kyid_mfa_methods", { "_queryFilter": '/KOGId eq "' + nodeState.get("KOGID") + '"'+ ' and MFAStatus eq "' + "ACTIVE" + '"'});
        if(response){
            return response;
        }
    } catch (error) {
        logger.error("Error Occurred while getUserMFAMethod-"+error);
    }
    
}


function isMFAEnrolled(roleId,ops) {
    try {
        var foundMFA = false;
        ...
        // var appResponse = ops.crudOps("read", "alpha_application", null, null, null, appId);
        var appResponse=alphaAppResponse(appId,ops);
        logger.debug("App Response --- "+appResponse );
        var businessAppId = appResponse.businessApplication._refResourceId;
         logger.debug("businessAppId is  --- "+businessAppId );
        
        // var businessAppResponse = ops.crudOps("read", "alpha_kyid_businessapplication", null, null, null, businessAppId);
         var businessAppResponse = queryBusinessApplication(businessAppId,ops);
        logger.debug("businessAppResponse is  --- "+businessAppResponse );
        var businessAppMFA = businessAppResponse.requiredMFAMethod;
        logger.debug("businessAppMFA Code is  --- "+businessAppMFA );
        nodeState.putShared("requiredMFACode",businessAppMFA);
                
        // var UserMFAResponse = openidm.query("managed/alpha_kyid_mfa_methods", { "_queryFilter": '/KOGId eq ' + nodeState.get("KOGID") + '"'+ ' and MFAStatus eq ' + "ACTIVE" + '"'});
        var UserMFAResponse=getUserMFAMethod();
        logger.debug("UserMFAResponse is -- "+ UserMFAResponse);

        for (i=0; i<UserMFAResponse.resultCount;i++){
            if(businessAppMFA === "5" && (UserMFAResponse.result[i].MFAMethod ==="TOTP" || UserMFAResponse.result[i].MFAMethod ==="PUSH" || UserMFAResponse.result[i].MFAMethod ==="SYMANTEC")){
                foundMFA = true;
                break;
            }
            else if(businessAppMFA === "4" &&(UserMFAResponse.result[i].MFAMethod ==="SMSVOICE"|| UserMFAResponse.result[i].MFAMethod ==="TOTP" || UserMFAResponse.result[i].MFAMethod ==="PUSH" || UserMFAResponse.result[i].MFAMethod ==="SYMANTEC")){
                foundMFA = true;
                break;
                
            }
            else if(businessAppMFA === "3" &&(UserMFAResponse.result[i].MFAMethod ==="EMAIL" || UserMFAResponse.result[i].MFAMethod ==="SECONDARY_EMAIL" || UserMFAResponse.result[i].MFAMethod ==="SMSVOICE"|| UserMFAResponse.result[i].MFAMethod ==="TOTP" || UserMFAResponse.result[i].MFAMethod ==="PUSH" || UserMFAResponse.result[i].MFAMethod ==="SYMANTEC")){
                foundMFA = true;
                break;
            }
        }
        ...
        // }
        logger.debug("Is MFA Found "+ foundMFA);
        return foundMFA

                
        
    } catch (error) {
        logger.error("Error Occurred While executing isMFAEnrolled Function -- "+error);
...
function completeMessage(){
    ...
    logger.debug("testing123 is "+ JSON.stringify(prereqcompeletedJSONObj))
...
function main(){
    ...
    var isMAFRegistered = false;
    var MFAPrerequisite  = null;
    ...
    try {
        ...
        if(contextID!=null && contextID){
           response = getEnrolmentPrerequisiteTypes(txid,nodeConfig,ops,contextID);
            logger.debug("response after getEnrolmentPrerequisiteTypes "+ JSON.stringify(response));
            if(Object.keys(response).length==0){
                logger.debug("Role added exiting")
                completeMessage();
                if(nodeState.get("prereqcompeletedJSONObj") && nodeState.get("prereqcompeletedJSONObj")!==null){
                    action.goTo(nodeOutcome.ROLEADDED);
                }
            }else{
                responseJSONArray = response["prereqtypes"];
            
           MFAPrerequisite = JSON.parse(nodeState.get("MFAPrerequisite"));
        if(MFAPrerequisite){
            isMAFRegistered = isMFAEnrolled(nodeState.get("roleIDinSession"),ops);
        }
           ...
           MFAPrerequisite = JSON.parse(nodeState.get("MFAPrerequisite"));
            ...
            if(MFAPrerequisite !== null){
            
            status = statusExistInRequest(txid,nodeConfig,ops,contextID,MFAPrerequisite.prereqtypename,uuid);  
            logger.debug("Status is -- "+ status);
            logger.debug("MFAPrerequisite.prereqtypename  value is "+ MFAPrerequisite.prereqtypename);
            logger.debug("MFAPrerequisite.sequence  value is "+ MFAPrerequisite.sequence);
                        
           
           if(status !== null && isMAFRegistered ===true && MFAPrerequisite.prereqtypename !== null){
                   logger.debug("Status is not null")
                   patchPrerequisiteRequestStatus(ops,txid,nodeConfig,contextID,uuid,MFAPrerequisite.prereqtypename,"COMPLETED");
                   
               }
           else if(status === "COMPLETED" && isMAFRegistered === false && MFAPrerequisite.prereqtypename !== null){
               logger.debug("Patching to TODO....");
               nodeState.putShared("nullifyMFAPrereq",true);  // If MFA is Removed for the user than nullifing the status
               patchPrerequisiteRequestStatus(ops,txid,nodeConfig,contextID,uuid,MFAPrerequisite.prereqtypename,"TODO");
               logger.debug("Patched successfully");
           }
           else if(status == null && isMAFRegistered ===true && MFAPrerequisite.prereqtypename !== null)
                {
                  logger.debug("Status is  null");
                   populateRequestObject(txid,nodeConfig,ops,contextID,MFAPrerequisite.prereqtypename,uuid,"COMPLETED",new Date().toISOString(),"","",MFAPrerequisite.sequence);
           ...
           for(var i=0; i<prereqList.length ;i++){
               ...
               for (var k = 0; k < data.length; k++) {
                ...
                if(status===null || status==="null" ){
                    logger.debug("InSide Status == null--")
                    ...
                    if(i==0){  
                        ...
                        if(i==0){
                         ...
                         logger.debug("Inside i > 0  ----"+i+"status is --"+status)
                         if(status==="NOT_STARTED"){
                             ...
                             logger.debug("Inside TODO Case -- ");
                             existingstatus = statusExistInRequest(txid,nodeConfig,ops,contextID,arrPreReqTypes[i-1],uuid);
                             logger.debug("Existing prereq status Inside TODO - "+existingstatus)
                             if(existingstatus==="TODO"){
                            logger.debug("Inside TODO Case --existingstatus ");
                                var dataObj = {
                                    "status": "NOT_STARTED"
                                };
                                prerequisiterecord.patchRequest(contextID,uuid,arrPreReqTypes[i],dataObj);
                             ...
                             logger.debug("Inside Else Condtion ----")
                            responseJSONObj["status"] = status; 
                         }  
            ...
            action.goTo(nodeOutcome.DONE); 
               ...
               if(selectedOutcome === 0){
                   ...
                   logger.debug("typeofprerequisite value from NodeState is --- "+nodeState.get("typeofprerequisite") )
                      if(nodeState.get("typeofprerequisite") === "mfa"){
                          if(isMAFRegistered === true){
                              logger.debug("Inside MFA is True")
                              patchPrerequisiteRequestStatus(ops,txid,nodeConfig,contextID,uuid,MFAPrerequisite.prereqtypename,"COMPLETED");
                              action.goTo(nodeOutcome.NEXT); 
                          }
                          else{
                            logger.debug("Navigating to Enroll MFA ");
                              nodeState.putShared("journeyName","AppEnrollPrerequisite");
