...
var nodeConfig = {
    ...
    script: "Script",
    scriptName: "KYID.Journey.IsKOGIDExistInPingAIC",
    timestamp: dateTime,
...
function removeUser(userID) {
    try {
        if (userID) {
            logger.error("Inside Remove User Function")
            var response = openidm.delete('managed/alpha_user/' + userID, null)
            nodeLogger.error(nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + "Duplicate Email ID Senario :: USER Removed from AIC");
            return true
        }
...
function InactivateUser(userID, userEmail) {
    try {
        if (userID) {
            logger.error("Inside Inactivate User Function")
           ...
           var InactivateUser = openidm.patch("managed/alpha_user/" + userID, null, [{ "operation": "add", "field": "accountStatus", "value": "inactive" }]);
            nodeLogger.error(nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + "Duplicate Email ID Senario :: USER INACTIVATED in AIC");
            return true
        }
    ...
    try {
        ...
        if (response.result.length == 1) {
            ...
            if (emailResponse.result.length >= 1) {
              ...
              //  logger.error("Case Issue: emailUserKogID="+emailUserKogID+" ::usrKOGID="+usrKOGID)
                if(emailUserKogID !== usrKOGID){
          //          logger.error("Case Issue: Not Matched");
            ...
            nodeLogger.error(nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + "User Not Found in AIC with KOG ID");
