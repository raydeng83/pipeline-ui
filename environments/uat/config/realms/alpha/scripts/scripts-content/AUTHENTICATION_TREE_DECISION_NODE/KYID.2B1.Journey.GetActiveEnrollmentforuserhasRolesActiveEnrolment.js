...
script: "Script",
scriptName: "KYID.2B1.Journey.GetActiveEnrollmentforuserhasRolesActiveEnrolment",
timestamp: dateTime,
...
/*        var allowedStatuses = ["TODO", "NOT_STARTED", "IN_PROGRESS", "PENDINGAPPROVAL"];
        ...
        var firstValidRoleId = null;
        
        if (!kyidResponse || kyidResponse.result.length === 0) {
            logger.error("No matching alpha_kyid_request found for requester.");
            return []; // early return, avoids continuing on empty
        }
        ...
        for (var i = 0; i < kyidResponse.result.length; i++) {
            ...
            //null check for roleId
            if (!roleId) {
                logger.error("Skipping item because approleid is missing for requestId: " + requestId);
                continue; // Skip this iteration
            }

            if (!firstValidRoleId) { 
                firstValidRoleId = roleId;
            }
            
            logger.error("roleId from request" +roleId)
            // Get Role Name
            params = {
                key: "_id",
                ops: "eq",
            ...
            if (contextIdTracker[contextId]) {
                logger.error("Skipping duplicate contextId: " + contextId);
                continue;
            }
            
             //null check for roleId
            if (!roleNameResponse || !roleNameResponse.result || roleNameResponse.result.length === 0) {
                logger.error("No role found for roleId: " + roleId);
                continue; // Skip if role not found
            }
            ...
            if (!appResults || appResults.length === 0) {
                logger.error("No applications found for roleId: " + roleId);
                continue; // Skip if no apps found
            }
...
*/
       ...
       // return responseArray;
        ...
        //Added for helpdesk contact info at footer
        if (firstValidRoleId) {
            var lib = require("KYID.2B1.Library.GenericUtils");
            var appResponse = JSON.parse(lib.getBusinessAppInfo(firstValidRoleId));
            var finalOutput = {
                application: appResponse.application
            };
            logger.debug("Final application output: " + JSON.stringify(finalOutput));

            // Inject into responseArray
...
function requestCallbacks() {
    ...
    try {
        ...
        callbacksBuilder.confirmationCallback(0, ["Skip"], 0);
    } catch (error) {
...
try {
    ...
    if (callbacks.isEmpty()) {
        ...
        requestCallbacks(responseArray);
