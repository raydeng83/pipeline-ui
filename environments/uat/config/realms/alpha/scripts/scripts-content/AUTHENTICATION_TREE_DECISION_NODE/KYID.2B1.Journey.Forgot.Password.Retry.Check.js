...
var nodeConfig = {
    ...
    script: "Script",
    scriptName: "KYID.2B1.Journey.Forgot.Password.Retry.Check",
    timestamp: dateTime,
...
function main() {
    ...
    var response = null;
    var version = null;
    //var ridpFlag = systemEnv.getProperty("esv.ridp.forgot.password.version.flag") ? systemEnv.getProperty("esv.ridp.forgot.password.version.flag").toLowerCase() : "v2";
    try {
        ...
        var ridpFlag = response.result[0].ridp_forgot_password_version ?  response.result[0].ridp_forgot_password_version : "v2";
        logger.error("KYID.2B1.Journey.Forgot.Password.Retry.Check response from query :: " + ridpFlag)
        
        var retryLimit = Number(response.result[0].ridp_forgot_password_retry_limit)
        logger.error("KYID.2B1.Journey.Forgot.Password.Retry.Check retryLimit from query :: " + retryLimit)
        var refreshLimit = response.result[0].ridp_forgot_password_refresh_limit
        logger.error("KYID.2B1.Journey.Forgot.Password.Retry.Check refreshLimit from query :: " + refreshLimit)
