...
if (callbacks.isEmpty()) {
    ...
    if(requestParameters.get("contextId")){
       contextid = requestParameters.get("contextId")[0]; 
        logger.error("Context found for user"+contextid);
    } 
    
    var jsonobj = {};
    var defaultAppName = null;
    logger.error("Context ID type is :: " + typeof contextid)
    var spUrl = null;
    var isDefault = false;
    var kogAppName = null;
    var appLogo = null;
    ...
    //Check if the context id is same as default context id
     var defaultContextID = systemEnv.getProperty("esv.kyid.default.contextid")
        if(contextid != null && defaultContextID !=null && (contextid === defaultContextID)){   
            logger.error("Context found for user. Not equals to Default ContextID");
             contextid = null;
             logger.error("Context ID is matching with default context id");
            } else {
            logger.error("Context ID is not matching with default context id");
            }
    
    if(contextid!=null){
        logger.error("Context ID value :: " + contextid)
        ...
        callbacksBuilder.confirmationCallback(0, ["Next"], 0);
         ...
         try {
            ...
            // }
                ...
                function getBusinessAppRecord(query,spUrl) {
                    var matchingApp = null;
                                if(spUrl){
                                    var records = openidm.query("managed/alpha_kyid_businessapplication", query, ["*"]);
                                    if (records && records.resultCount > 0) {
                                        if (records.resultCount > 1) {
                                            nodeLogger.debug(nodeConfig.timestamp + "::" + nodeConfig.node + "::" + nodeConfig.nodeName + "::" + nodeConfig.script + "::" + nodeConfig.scriptName + "::" + " More than one business application record found for query: " + JSON.stringify(query));
                                            records.result.forEach(apps => {
                                                nodeLogger.debug("App Records are :: => "+ JSON.stringify(apps));
                                                var frappId = apps.forgerockAppId || apps.applicationURL
                                                if(frappId){
                                                   frappId = getDomainAndFirstSegment(frappId)
                                                }
                                                
                                                // if(apps.forgerockAppId == spUrl || apps.applicationURL == spUrl){
                                                //     matchingApp = apps;
                                                // } 
                                                 if(frappId == spUrl){
                                                    matchingApp = apps;
                                            ...
                                            if(matchingApp){
                                                 nodeLogger.error("Exact Match Found Maching App URL: "+matchingApp)
                                                return matchingApp;
                                            } else {
                                                nodeLogger.error("Exact Match Not Found ")
                                                return records.result[0];
                                            }
                                        } else {
                                            return records.result[0];
            ...
            function getDefaultBusinessAppRecord() {
                ...
                var record = getBusinessAppRecord(query,null);
                return record;
            }
            ...
            try {
                ...
                if (spUrl) {
                    ...
                    var businessRecord = getBusinessAppRecord(query,spUrl);
                ...
                // 3. Fallback to default portal
                    ...
                    var query = {"_queryFilter": '/name eq "' + defaultAppName + '"'};
                    var businessRecord = getBusinessAppRecord(query,null);
                    if (!businessRecord) {
                        businessRecord = getDefaultBusinessAppRecord();
                    }
                }
                ...
                if (businessRecord) {
                    ...
                    appLogo = businessRecord.logoURL || businessRecord.logoFileName || null;
                    ...
                    nodeLogger.error("appLogo: " + appLogo);
