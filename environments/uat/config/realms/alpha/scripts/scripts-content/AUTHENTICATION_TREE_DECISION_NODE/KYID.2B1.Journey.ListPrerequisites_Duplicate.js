...
function getRoleAprrover(id, ops){

var roleId= null
var params = {};
var paramFields= [];
var approvers = null;
var jsonApproversData = null;
var jsonApproversParsedData = null;
...
var backupApprovalLevel2 = null;
    ...
    try{
        ...
        logger.error("id is in "+ id)
        params["key"] = "_id";
        params["ops"] = "eq";
        params["value"] = roleId;
        paramFields.push("_id");
        paramFields.push("name");
        paramFields.push("approverLevel1");
        paramFields.push("approverLevel2");
        paramFields.push("backupApprovalLevel1");
        paramFields.push("backupApprovalLevel2");
        //recordRole = openidm.query("managed/alpha_role", { "_queryFilter" : '/_id eq "'+id+'"'}, ["_id", "name", "_policy"]);
        approvers = ops.crudOps("query", "alpha_role", null, params, paramFields , null);
        logger.debug("Successfully retrieved alpha_role managed object attributes :: "+JSON.stringify(approvers.result[0]));
        var jsonApproversData =  JSON.stringify(approvers.result[0]);
        var jsonApproversParsedData = JSON.parse(jsonApproversData);
        var approverLevel1 = jsonApproversParsedData["approverLevel1"]; 
        var approverLevel2 = jsonApproversParsedData["approverLevel2"]; 
        var backupApprovalLevel1 = jsonApproversParsedData["backupApprovalLevel1"]; 
        var backupApprovalLevel2 = jsonApproversParsedData["backupApprovalLevel2"];
        logger.debug("approverLevel1 "+ approverLevel1);
        logger.debug("approverLevel2 "+ approverLevel2);
        logger.debug("backupApprovalLevel1 "+ backupApprovalLevel1);
        logger.debug("backupApprovalLevel2 "+ backupApprovalLevel2);
       if(approverLevel1!== undefined || approverLevel2!== undefined || backupApprovalLevel1!== undefined || backupApprovalLevel2!== undefined){
           nodeState.putShared("isworkflow",true)
       }else{
           nodeState.putShared("isworkflow",false)
       }
    }catch(error){
        logger.error("Error in getRoleAprrover function is ::"+ error)
    }  
}
...
logger.debug("arrPreReqTypeIDs is in it "+ arrPreReqTypeIDs)
  try { 
      if(arrPreReqTypeIDs!= null){
         ...
         if(nodeState.get("numberofRequest")!==null ){
             logger.debug("numberofRequest is in"+ nodeState.get("numberofRequest"))
             logger.debug("listOfPrereqTypes length in"+ listOfPrereqTypes.length)
             for(var m=listOfPrereqTypes.length; m<nodeState.get("numberofRequest"); m++){
                var workflow = {
                    "name": {
                        "en": "kyid_prerequisite_workflow",
                        "es": "kyid_prerequisite_workflow"
                    },
                    "description": {
                        "en": "This is a workflow that agents needs to complete",
                        "es": "Este es un flujo de trabajo que los agentes deben completar"
                    },
                    "sequence":m+1
                }
                  listOfPrereqTypes.push(workflow) 
             }
         }else{
             logger.debug("Inisde else of getListOfPrereqTypes")
              var workflow = {
                    "name": {
                        "en": "kyid_prerequisite_workflow",
                        "es": "kyid_prerequisite_workflow"
                    ...
                    "sequence": listOfPrereqTypes.length+1
                } 
             listOfPrereqTypes.push(workflow)
         }
         ...
         logger.debug("listOfPrereqTypes is in "+ JSON.stringify(listOfPrereqTypes))
...
function getRoleDetails(txid,nodeLogger,ops,id){
  ...
  //Local Variables  
   ...
   try { 
        ...
        logger.debug("P0licy is in  "+jsonRoleParsedData["_policy"]);
           if(jsonRoleParsedData["_policy"]!= null && jsonRoleParsedData["_policy"]){
        ...
        if(roleID!=null && roleID){
          ...
          if(policyID!=null && policyID){
             ...
             if(arrPreReqTypeIDs.length>=0){
              ...
              arrPreReqType = getListOfPrereqTypes(txid,nodeLogger,ops);
               response["prereqtypes"] = arrPreReqType;
          }
        }
   ...
   try { 
       ...
       if(status==="COMPLETED"){
           ...
           if(recordRequest["resultCount"]===totPrereqCount){
                ...
                logger.error("All prereqs are not completed");
            }
        }
...
*/
function statusExistInRequest(txid,nodeLogger,ops,id,type,requesterID, sequence){
  ...
  //Local Variables  
   ...
   var paramFields = [];
   var recordRequestJSONObj = {};
    logger.debug("type in statusExistInRequest "+ type)
   try { 
       ...
       if(nodeState.get("numberofRequest")!==null){
           if(type === "kyid_prerequisite_workflow"){
               if(nodeState.get("numberofRequest") === sequence){
                   recordRequest = openidm.query("managed/alpha_kyid_request", {"_queryFilter": '(contextid eq "' + id + '" AND type eq "' + type + '" AND requester eq "' + requesterID + '" AND !(status eq "COMPLETED") AND !(status eq "CANCELLED")' + ')'}, ["contextid", "type", "requester", "status","requestId"]);
               }
               else{
                    recordRequest = ""
                   //recordRequest = openidm.query("managed/alpha_kyid_request", { "_queryFilter": '(contextid eq "' + id + '" AND type eq "' + type + '" AND requester eq "' + requesterID + '" AND !(status eq "COMPLETED") AND !(status eq "CANCELLED") AND !(requestId eq ""))'}, ["contextid", "type", "requester", "status","requestId"]);
               }
               
           }else{
               recordRequest = openidm.query("managed/alpha_kyid_request", { "_queryFilter" : 'contextid eq "'+id+'"'+ 'and type eq "'+type+'"'+ 'and requester eq "'+requesterID+'"'}, ["contextid", "type", "requester", "status","requestId"]);
           }  
       }else{
            logger.debug("inside else numberofRequest is null")
           recordRequest = openidm.query("managed/alpha_kyid_request", { "_queryFilter" : 'contextid eq "'+id+'"'+ 'and type eq "'+type+'"'+ 'and requester eq "'+requesterID+'"'}, ["contextid", "type", "requester", "status","requestId"]);
       }
        
        logger.debug("Successfully queried record in alpha_kyid_request managed object in statusExistInRequest :: "+JSON.stringify(recordRequest));
    ...
    logger.error("status is in "+ status)
...
function findPrereqtypeForNavigation(txid,nodeLogger,prereqtypes,arrPreReqTypesName){
   ...
   //Local Variables 
    ...
    logger.error("RLogs::prereqtype value for navigation is - "+type);
    return type;
}
...
function main(){
    ...
    try {
        ...
        var FlagResponse = nodeState.get("flagJSONData");
        ...
        if(contextID!=null && contextID){
           ...
           getRoleAprrover(contextID, ops);
           responseJSONArray = response["prereqtypes"];
        
            
           logger.debug("getEnrolmentPrerequisiteTypes response: "+ JSON.stringify(response));
           nodeLogger.log("error", nodeConfig, "mid", txid, "getEnrolmentPrerequisiteTypes function response is- "+JSON.stringify(response));
           prereqList = JSON.parse(JSON.stringify(response.prereqtypes));
           arrPreReqTypes = nodeState.get("arrPreReqTypes");
           //arrPreReqTypes.push("kyid_prerequisite_workflow")

           logger.debug("arrPreReqTypes is in "+ arrPreReqTypes)
           logger.debug("prereqList is in "+ JSON.stringify(prereqList))
           nodeLogger.log("error", nodeConfig, "mid", txid, "arrPreReqTypes length is- "+arrPreReqTypes.length);
        
           for(var i=0; i<prereqList.length ;i++){
                prereqListJSONObj = {};
                responseJSONObj = {};
                prereqListJSONObj = JSON.parse(JSON.stringify(prereqList[i]));
                status = statusExistInRequest(txid,nodeConfig,ops,contextID,arrPreReqTypes[i],uuid, prereqListJSONObj["sequence"] );
                nodeLogger.log("error", nodeConfig, "mid", txid, "statusExistInRequest is- "+status);
               //if(!(arrPreReqTypes[i] === "kyid_prerequisite_workflow" && !(status ==="COMPLETED" || status ==="CANCELLED")) || (arrPreReqTypes[i] !== "kyid_prerequisite_workflow")){
               ...
               for (var k = 0; k < data.length; k++) {
                ...
                if(status===null){
                    if(nodeState.get("numberofRequest")==null){
                        ...
                        if(i==0){
                            if(!nodeState.get("isverifiedprerequisites")== true){
                                logger.debug("Inside populateRequestObject")
               ...
               logger.debug("responseJSONObj status - "+JSON.stringify(responseJSONObj.status));
               if(responseJSONObj.status!==undefined){
          ...
          //}
            ...
            action.goTo(nodeOutcome.DONE); 
                ...
                // } 
                    logger.debug("Entering is callback is empty");
                    ...
                    callbacksBuilder.confirmationCallback(0, ["Completed"], 0);
                     ...
                     logger.debug("Entering is callback is empty else part ");
                    ...
                    logger.debug("Entering is callback is empty else after part status is "+ status);
                    logger.debg("Type of Prereq is - "+typeofprerequisite)
                    if(typeofprerequisite === "workflow"){
                        logger.debug("Entering typeofprerequisite is workflow sttaus "+ status);
                        if(status === "REJECTED"){
                            ...
                            if(nodeState.get("_idWorkflow")!==null){
                               requestId=  getRequestId(contextID, "kyid_prerequisite_workflow", uuid, nodeState.get("_idWorkflow"))
                            }else{
                                if (getRequestId(contextID, "kyid_prerequisite_workflow", uuid) != null && getRequestId(contextID, type, uuid) != "" && getRequestId(contextID, type, uuid)) {
                            ...
                            prerequisiterecord.patchRequest(contextID,uuid,"kyid_prerequisite_workflow",dataObj);
                            nodeState.putShared("prereqcompeletedJSONObj",JSON.stringify(prereqcompeletedJSONObj));
                            action.goTo(nodeOutcome.DONE);
                            }else{
                            callbacksBuilder.textOutputCallback(0,JSON.stringify(response));
                            //callbacksBuilder.confirmationCallback(0, ["Start","Cancel","View workflow status"], 0);
                            callbacksBuilder.confirmationCallback(0, ["Cancel","View workflow status"], 0);  
                            }

                        } else if(nodeState.get("isworkflow")==true){
                            // callbacksBuilder.textOutputCallback(0,JSON.stringify(response));
                            // callbacksBuilder.confirmationCallback(0, ["Start","Cancel"], 0);
                           // nodeState.putShared("prereqtype","kyid_prerequisite_workflow")
                            logger.debug("Inside approvers ignoring start button")
                            action.goTo(nodeOutcome.NEXT);
                        } else if(nodeState.get("isworkflow")==false){
                                logger.debug("Inside no approvers")
                                action.goTo(nodeOutcome.NEXT);
                        }
                        // else {
                        //     callbacksBuilder.textOutputCallback(0,JSON.stringify(response));
               ...
               logger.debug("skiped to else workflow");
                      ...
                      if(status === "REJECTED"){
                              action.goTo(nodeOutcome.DONE); 
                               ...
                               if(arrPreReqTypes.length>0){
                                   ...
                                   logger.debug("skiped to else workflow and selectedOutcome is "+ selectedOutcome);
                                   patchPrerequisiteCancelRequestStatus(ops,txid,nodeLogger,contextID,uuid,arrPreReqTypes,"CANCELLED"); 
                                   isAllPrereqStatusCancelled = statusExistAllPrerequisiteInRequest(txid,nodeLogger,ops,contextID,uuid,arrPreReqTypes.length,"CANCELLED");
                                   logger.debug("isAllPrereqStatusCancelled return value - "+isAllPrereqStatusCancelled); 
                                   if(isAllPrereqStatusCancelled){
