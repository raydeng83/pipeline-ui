/**
* Name: KYID.2B1.Library.FormValidations
...
* Import Library:  
*                Example: 
*                        var portalData = require("KYID.2B1.Library.FormValidations");
...
function validateTextFieldValues(userInfo, sysInfo, validationArray, attrName){
   logger.error("**Inside validateTextFieldValues function**")   
    ...
    // if(parsedUserInfo["fieldFormat"]!== undefined && parsedUserInfo["fieldFormat"]){
    //     fieldFormat = parsedUserInfo["fieldFormat"]
    // }
   ...
   if (sysInfo != null && parsedUserInfo["isReadOnly"]) {
    ...
    if (parsedUserInfo["isReadOnly"] === true) {      
      ...
      return true;
        ...
        return isTextInputValueValid(parsedUserInfo["value"],charLimit,regex,attrName,fieldFormat);
    }
}
...
function validateDateFieldValues(userInfo,validationArray,attrName){
   ...
   // var dateRegex = /^\d{4}-\d{2}-\d{2}$/;
    ...
    //logger.error("Regex validation is: "+ !dateRegex.test(datevalue))
...
function isTextInputValueValid(value,limit,regex, attrName, fieldFormat){
    var validatePhoneLib = require("KYID.2B1.Library.GenericUtils");
    logger.error("The attrName in function is:"+ attrName);
    if (isRequired(value, attrName)) {
        var isLimitSet = (limit != null);
        var isPhoneFormat = (fieldFormat === "phoneNumber");
    
        if (isLimitSet && !isPhoneFormat) {
            if (!isMaxLengthReached(value, limit, attrName)) {
                if (regex != null) {
                    if (regex.test(value)) {
                        return true;
                    } else {
                        var msg = getValidationMessage(attrName, "regex");
                        addError(attrName, "en", msg.enMSg);
                        addError(attrName, "es", msg.esMSg);
                        return false;
                    }
                }
                // If regex is null, assume valid
                return true;
            }
            // Max length reached
            return false;
        ...
        // When limit is null or fieldFormat is "phoneNumber"
        if (!isPhoneFormat) {
            if (regex != null) {
                if (regex.test(value)) {
                    return true;
            ...
            // Special handling for phoneNumber format
