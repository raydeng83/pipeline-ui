...
function preAuthentication() {
   ...
   //var returnUrlCookieDomain=".sso.ide.kyid.ky.gov"; // Remove this and uncomment above before promotion
    ...
    if(issuer.localeCompare(systemEnv.getProperty("esv.logout.app.url")) == 0 && !session) {
       ...
       logger.error(adapterLoggerPrefix+"::"+"displayUrl PRESENT IN PREAuthentication"+"::"+displayUrl);
        ...
        logger.error(adapterLoggerPrefix+"::"+"displayUrl PRESENT IN PREAuthentication 0"+"::"+displayUrl);
        ...
        logger.error(adapterLoggerPrefix+"::"+"wtrealmUrl PRESENT IN PREAuthentication 1"+"::"+wtrealmUrl);
        // response.addHeader("Set-Cookie", "rp-realm=" + decodeURIComponent(wtrealmUrl) + "; Domain=.ide.kyid.ky.gov; Path=/; HttpOnly");
		...
		logger.error(adapterLoggerPrefix+"::"+"displayUrl PRESENT IN PREAuthentication 2"+"::"+displayUrl);
        response.addHeader("Set-Cookie", "ReturnURL=" + decodeURIComponent(displayUrl) + "; Domain="+returnUrlCookieDomain+"; Path=/; HttpOnly");
        ...
        logger.error(adapterLoggerPrefix+"::"+"displayUrl PRESENT IN PREAuthentication 3"+"::"+displayUrl);
        response.addHeader("Set-Cookie", "ReturnURL=" + decodeURIComponent(displayUrl) + "; Domain="+returnUrlCookieDomain+"; Path=/; HttpOnly");
        ...
        response.addHeader("Set-Cookie", "rp-realm=" + "; Domain="+returnUrlCookieDomain.replace(/\.?sso\./, ".")+"; Path=/; expires=Thu, 01 Jan 1970 00:00:00 UTC; HttpOnly");
        ...
        }                                                                                  /**** Tableau Issue Fix 24/01 :: 183821, 183826, 183801 ****/
        logger.error(adapterLoggerPrefix+"::"+"displayUrl PRESENT IN PREAuthentication 4"+"::"+displayUrl);
        response.addHeader("Set-Cookie", "ReturnURL=" + decodeURIComponent(displayUrl) + "; Domain="+returnUrlCookieDomain+"; Path=/; HttpOnly");
        response.addHeader("Set-Cookie", "rp-realm=" + "; Path=/; expires=Thu, 01 Jan 1970 00:00:00 UTC; HttpOnly");
        response.addHeader("Set-Cookie", "rp-realm=" + "; Domain="+returnUrlCookieDomain+"; Path=/; expires=Thu, 01 Jan 1970 00:00:00 UTC; HttpOnly");
        response.addHeader("Set-Cookie", "rp-realm=" + "; Domain="+returnUrlCookieDomain.replace(/\.?sso\./, ".")+"; Path=/; expires=Thu, 01 Jan 1970 00:00:00 UTC; HttpOnly");
        ...
        logger.error(adapterLoggerPrefix+"::"+"displayUrl PRESENT IN PREAuthentication 5"+"::"+displayUrl);
        response.addHeader("Set-Cookie", "ReturnURL=" + decodeURIComponent(displayUrl) + "; Domain="+returnUrlCookieDomain+"; Path=/; HttpOnly");
        response.addHeader("Set-Cookie", "rp-realm=" + "; Path=/; expires=Thu, 01 Jan 1970 00:00:00 UTC; HttpOnly");
        response.addHeader("Set-Cookie", "rp-realm=" + "; Domain="+returnUrlCookieDomain+"; Path=/; expires=Thu, 01 Jan 1970 00:00:00 UTC; HttpOnly");
        response.addHeader("Set-Cookie", "rp-realm=" + "; Domain="+returnUrlCookieDomain.replace(/\.?sso\./, ".")+"; Path=/; expires=Thu, 01 Jan 1970 00:00:00 UTC; HttpOnly");
...
function preSignResponse() {
  logger.error(adapterLoggerPrefix+"::ENTERING preSignResponse FUNCTION");
   ...
   try {
    if(session.getProperty("idpauthlevel")=="3" || 
   session.getProperty("idpauthlevel")=="4" ||
   session.getProperty("idpauthlevel")=="5"){
   mfaCompleted = true
   logger.error(adapterLoggerPrefix+"::mfaCompleted = [" + mfaCompleted + "] type = " + typeof mfaCompleted);
    }
   } catch (e) {
    logger.error(adapterLoggerPrefix+"::Error reading 'idpauthlevel' property::" + e);
   }
