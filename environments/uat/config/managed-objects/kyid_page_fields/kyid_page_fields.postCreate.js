...
if (endpointExecution && endpointExecution === "true") {
    ...
    if (enableAuditLogHistory && enableAuditLogHistory === "true") {
        try {
            ...
            //logger.error("Response from Audit History endpoint for create event on kyid_page_fields object:::: => " + JSON.stringify(response));
        } catch (error) {
            logger.error("An error occurred while updating the CREATE event audit History events for kyid_page_fields object");
        }
    }
}