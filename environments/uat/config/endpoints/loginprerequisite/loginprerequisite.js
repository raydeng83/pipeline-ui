...
(function() {
    ...
    const ACTION_REMIND_ME_LATER = "12"
    ...
    try {
        ...
        // const userId = "98357eba-738d-49c2-90b3-73c99aa847cb"
        let result
        switch (request.method) {
            ...
            case REQUEST_POST:
                ...
                }else if(action === ACTION_REMIND_ME_LATER){
                  input.payload = requestContent.payload
                  input.payload["userId"] = userId
                  result = markRemindMeLater(input)
                  response = generateResponse(RESPONSE_CODE_SUCCESS, input.transactionId, SUCCESS_MESSAGE, {
                        result: result
                  })
                }
...
function markRemindMeLater(input){
    const functionName = "markRemindMeLater"
    const EXCEPTION_UNEXPECTED_ERROR = {
        code: "KYID-EUE",
        content: ""
    }
    ...
    const invalidRequestException = {
        ...
        "logger": `${input._ENDPOINT_NAME}/markRemindMeLater`,
        "timestamp": ""
    }
  ...
  try {
    if(input && input.payload && input.payload.userId && input.payload.userPrereqId){
      let userPrereqPatch =[
        {
          "operation": "replace",
          "field": "remindMeLater",
      ...
      var userPrerePatchResponse = patchMORecord(input.payload.userPrereqId, "alpha_kyid_enrollment_user_prerequisites", userPrereqPatch)
      if(userPrerePatchResponse){
        return {
          "remindMeLater":"success"
        }
      }
...
function getToDoLoginPrerequisites(input) {
    ...
    try {
        ...
        if (input.payload) {
            ...
            if (queryFilter) {
                let queryFilter2 = queryFilter
                queryFilter2 += "AND !(remindMeLater eq true)"
                logger.error("the searchInvitationQuery: " + JSON.stringify(queryFilter2))
                logDebug(input.transactionId, input.endPoint, "getToDoLoginPrerequisites", `Search filter: ${queryFilter2}`)
                const searchResponse = openidm.query(`${input._MO_OBJECT_NAME}`, {
                        "_queryFilter": queryFilter2
                    },
                    returnProperties
                )
                if (searchResponse) {
                    if (input.payload.view && input.payload.view.toLowerCase() === "todologinprereq") {
                        return {
                            prerequisites: formatToDoResponse(searchResponse)
                        }
                    } else {
...
function formatToDoResponse(searchResponse) {
    try {
        ...
        let remindMeLaterPrereqTypes = ["4","3","2","5","7"]
        if (searchResponse && searchResponse.resultCount > 0) {

            searchResponse.result.forEach(value => {
                TODOLoginPrereqJSON = {}
                let showInTodo = true
                if(value.remindMeLater){
                 showInTodo = false
                }
                TODOLoginPrereqJSON["showRemindMeLater"] = remindMeLaterPrereqTypes.includes(value.preRequisiteTypeId.typeName)
                TODOLoginPrereqJSON["showInTodo"] = showInTodo
                ...
                // TODOLoginPrereqJSON["typeName"] = value.preRequisiteTypeId.typeName || "" // NR: Added this fix to fix defect 215288
                if(value.preRequisiteTypeId.typeName && value.preRequisiteTypeId.typeName.toLowerCase()== "custom"){
                  TODOLoginPrereqJSON["typeName"] = "6"
                }
                else{
                    ...
                    if (businessAppInfo) {
                        ...
                        if (prereqResponseArray.length > 0) {
                            ...
                            if (PrereqForEvaluation !== null) {
                                let apiCallFrom = "0"
                                prerequisites = evaluatePrereq(input, userId, Number(lastAttemptedLoginPrereq), PrereqForEvaluation, apiCallFrom) //input,userId,lastAttemptLoginPrereqTime,prereqList
                                if (prerequisites) {
                                    logger.error("sLoginPrereqRequired Function --> prerequisites are "+ JSON.stringify(prerequisites))
                        ...
                        if (loginPrereqEvaluationResult.length > 0) {
                            ...
                            return {
                                "mandatoryPrerequisite": mandatoryPrerequisite,
                                "optionalPrerequisite": optionalPrerequisite
                            }
                        } else {
                            return {
                                "mandatoryPrerequisite": false,
                                "optionalPrerequisite": false
                            }
                        }
                ...
                if (userInfo) {
                    if (businessAppInfo) {
                        ...
                        if (loginPrereqEvaluationResult.length > 0 && authTime && authTime !== null) {
                            ...
                            // // var userPatchResponse = patchMORecord(userId,"alpha_user",userPatchRequest)

                            // // let userPrerePatchResponse = patchMORecord(userId,"alpha_kyid_enrollment_prerequisite",userPatchRequest)
                            ...
                            //     // loginPrereqEvaluationResult.forEach(userPrereVal=>{
                            //     //   var userPrereqPatch = [{
                            //     //     "operation": "replace",
                            //     //     "field": "lastVisitTime",
                            ...
                            //     //     // "value": authTime
                            //     //     }]
                            //     //   var userPrerePatchResponse = patchMORecord(userPrereVal.userPrereqId,"alpha_kyid_enrollment_user_prerequisites",userPrereqPatch)
...
function createPrereqJSON(prereq) {
    try {
        const remindMeLaterPrereqTypes = ["4","3","2","5","7"]
        ...
        if (prereq) {
            if (prereq.mandatoryPrerequisites && prereq.mandatoryPrerequisites.length > 0) {
                ...
                prereq.mandatoryPrerequisites.forEach(value => {
                    ...
                    if(value.preRequisiteTypeId.typeName && value.preRequisiteTypeId.typeName.toLowerCase() === "custom"){
                      loginPrereqJSON["typeName"] = "6"
                    }
                    else if(value.preRequisiteTypeId.typeName && value.preRequisiteTypeId.typeName.toLowerCase() === "mfa"){
                      loginPrereqJSON["typeName"] = "4"
                    }
                    else{
            ...
            if (prereq.optionalPrerequisites && prereq.optionalPrerequisites.length > 0) {
                ...
                prereq.optionalPrerequisites.forEach(record => {
                    ...
                    if(record.preRequisiteTypeId.typeName && record.preRequisiteTypeId.typeName.toLowerCase() === "custom"){
                      loginPrereqJSON["typeName"] = "6"
                    }
                    else if(record.preRequisiteTypeId.typeName && record.preRequisiteTypeId.typeName.toLowerCase() === "mfa"){
                      loginPrereqJSON["typeName"] = "4"
                    }
                    else{
                    ...
                    loginPrereqJSON["displayOrder"] = displayOrder
                    loginPrereqJSON["showRemindMeLater"] = remindMeLaterPrereqTypes.includes(record.preRequisiteTypeId.typeName)
                    prerequisites.push(loginPrereqJSON)
...
function evaluatePrereq(input, userId, lastAttemptLoginPrereqTime, prereqList, apiCallFrom) {
    const EXCEPTION_UNEXPECTED_ERROR = {
        code: "KYID-EUE",
        content: ""
    }
    ...
    try {
        if (userId) {
            if (prereqList) {
                if (apiCallFrom && apiCallFrom === "0") {
                    apiCallFrom = apiCallFrom
                } else {
                    apiCallFrom = null
                }
                if (prereqList.prereqForEvaluation && prereqList.expiredPrerequisites) {
                    ...
                    logger.error("=prereqList.prereqForEvaluation -> " + JSON.stringify(prereqForEvaluation))
                    let authTime = input.payload.authTime || null
                    const parts = authTime.split("-")
                    authTime = parts[1]
                    let firstTimeLogin = 0
                    if (parts) {
                        firstTimeLogin = parts[0]
                    }
                    ...
                    var lastVistDateEpoch = null
                    var lastVistDateTimeStamp = null
                    var lastVistDate = null
                    var nextVistDate = null
                    ...
                    if (prereqForEvaluation.length > 0) {
                        prereqForEvaluation.forEach(value => {
                            if (value.lastVisitTime) {
                                lastAttemptLoginPrereqTime = Number(value.lastVisitTime)
                                lastAttemptLoginPrereqTime = new Date(lastAttemptLoginPrereqTime).toISOString()
                                logger.error("lastAttemptLoginPrereqTime ISO Format--> " + lastAttemptLoginPrereqTime)
                                lastAttemptLoginPrereqDate = lastAttemptLoginPrereqTime.split('T')[0]
                                logger.error("lastAttemptLoginPrereqDate is -->" + lastAttemptLoginPrereqDate)
                            ...
                            if (value.expiryDate) {
                                ...
                                logger.error("userPrereqexpiryDate Expiry is " + userPrereqexpiryDate)
                            }
                            if (value.createDate) {
                            ...
                            if (prereqRecord.LoginDueCompletion && prereqRecord.LoginDueCompletion !== null) {
                                if (prereqRecord.LoginDueCompletion.dueDate !== null && prereqRecord.LoginDueCompletion.dueDate) {
                                    ...
                                    // }
                              if (getFormatedDaysWithAdditional(userPrereqcreateDate, Number(prereqRecord.LoginDueCompletion.inDays), "add")) {
                                          dueDate = getFormatedDaysWithAdditional(userPrereqcreateDate, Number(prereqRecord.LoginDueCompletion.inDays), "add")
                                      }
                                ...
                                if (lastAttemptLoginPrereqDate) {
                                    ...
                                    if (frequencyInDays !== null && frequencyInDays !== "0") {
                                        ...
                                        logger.error("lastAttemptLoginPrereqWithFreqDate is --> " + lastAttemptLoginPrereqWithFreqDate + "UserPrereqId is ->" + value._id)
                                        logger.error("Current Date  is --> " + currentDate + "UserPrereqId is ->" + value._id)
                                        logger.error("lastAttemptLoginPrereqDate  is --> " + lastAttemptLoginPrereqDate + "UserPrereqId is ->" + value._id)
                                    } else {
                                ...
                                if(value.lastVisitTime){
                                  lastVistDateEpoch = Number(value.lastVisitTime)
                                  lastVistDateTimeStamp = new Date(lastVistDateEpoch).toISOString()
                                  var lastVistDate = lastVistDateTimeStamp.split('T')[0]
                                  if(frequencyInDays !== null && frequencyInDays !== "0"){
                                   nextVistDate = getFormatedDaysWithAdditional(lastVistDate, Number(frequencyInDays), "add")
                                   if(nextVistDate){
                                     value["nextVistDateVirtual"] = nextVistDate
                                   }
                                  }else{
                                    nextVistDate = getFormatedDaysWithAdditional(lastVistDate, Number("1"), "add")
                                  }
                                  
                                  
                                }else{
                                  value["nextVistDateVirtual"] = currentDate
                                }
                                value["lastVisitTimeVirtual"] = Date.now().toString()
                                value["lastVisitLogonTimeVirtaul"] = authTime.toString()
                                value["nextVisitTimeVirtual"] = new Date(lastAttemptLoginPrereqWithFreqDate + "T00:00:00Z").getTime().toString()
                                ...
                                if (skip === "0") {
                                    ...
                                    if ((userPrereqexpiryDate !== null && dueDate !== null) && (currentDate > userPrereqexpiryDate || userPrereqexpiryDate > dueDate)) {
                                        logger.error("Due Date is Exceeded")
                                        mandatoryPrerequisites.push(value)

                                    }
                                  
                                    else if (value.skipTime && firstTimeLogin === "1" && value.skipTime === value.lastVisitLogonTime) {
                                        logger.error("Skipping the Login Prereq")
                                    }  // Adding this to Handle Remind Me Later
                                    // else if (value.remindMeLater && value.remindMeLaterDate && value.remindMeLater == true && value.remindMeLaterDate == currentDate) {
                                    //     logger.error("Skipping the Login Prereq Due to Remind Me Later")
                                    // }
                                    else if (value.lastVisitLogonTime && authTime === value.lastVisitLogonTime) {
                                        logger.error("Inside if Current Logon Time == Last Visit Logon Time" + "userPrereqId " + value._id)
                                        // if(!(value.remindMeLater && value.remindMeLaterDate && value.remindMeLater == true && value.remindMeLaterDate == currentDate)){
                                        ...
                                        logger.error("lastAttemptLoginPrereqWithFreqDate is --> " + lastAttemptLoginPrereqWithFreqDate + "UserPrereqId is ->" + value._id)
                                        logger.error("Current Date  is --> " + currentDate + "UserPrereqId is ->" + value._id)
                                        logger.error("In currentDate >= lastAttemptLoginPrereqWithFreqDate Condtion" + "UserPrereqId is ->" + value._id)
                                        if (numberOfDaysBeforeDueDate !== null && numberOfDaysBeforeDueDateInDays !== "0") {
                                            logger.error("numberOfDaysBeforeDueDate !== null && numberOfDaysBeforeDueDateInDays !==0 " + "UserPrereqId is ->" + value._id)
                                            if ((currentDate > numberOfDaysBeforeDueDate) && numberOfDaysBeforeDueDate !== "0") {
                                                ...
                                                if (login === "0") {
                                                    logger.error("In currentDate >= lastAttemptLoginPrereqWithFreqDate --> login === 0 " + "UserPrereqId is ->" + value._id)
                                                    if (new Date().toISOString() >= lastAttemptLoginPrereqTime) {
                                                        logger.error("In currentDate >= lastAttemptLoginPrereqWithFreqDate --> new Date().toISOString() > lastAttemptLoginPrereqTime" + "UserPrereqId is ->" + value._id)
                                                        // if(!(value.remindMeLater && value.remindMeLaterDate && value.remindMeLater == true && value.remindMeLaterDate == currentDate)){
                                                          optionalPrerequisites.push(value)
                                                        // }else{
                                                        //   logger.error("Skipping the Login Prereq Due to Remind Me Later")
                                                        ...
                                                        logger.error("In Else -- currentDate >= lastAttemptLoginPrereqWithFreqDate --> new Date().toISOString() > lastAttemptLoginPrereqTime -- Not Display Prereq" + "UserPrereqId is ->" + value._id)
                                                    }
                                                } else {
                                                    logger.error("In currentDate >= lastAttemptLoginPrereqWithFreqDate --> login === 1" + "UserPrereqId is ->" + value._id)
                                                    // logger.error("In login = 1 condtion")
                                                    if (lastAttemptLoginPrereqDate == lastAttemptLoginPrereqWithFreqDate) {
                                                        logger.error("In currentDate >= lastAttemptLoginPrereqWithFreqDate --> login === 1 Skipping Prereq in Login Condtion 1" + "UserPrereqId is ->" + value._id)
                                                        logger.error("Don't Display Login Prerequisites" + prereqForEvaluation._id)
                                                    } else {
                                                        logger.error("In currentDate >= lastAttemptLoginPrereqWithFreqDate --> login === 1 Login Condtion 1" + "UserPrereqId is ->" + value._id)
                                                        // if(!(value.remindMeLater && value.remindMeLaterDate && value.remindMeLater == true && value.remindMeLaterDate == currentDate)){
                                                          optionalPrerequisites.push(value)
                                                        // }else{
                                                        //   logger.error("Skipping the Login Prereq Due to Remind Me Later")
                                                ...
                                                logger.error("Don't Display Login Prerequisites" + "UserPrereqId is ->" + value._id)
                                            }
                                            ...
                                            if (login === "0") {
                                                ...
                                                if (new Date().toISOString() >= lastAttemptLoginPrereqTime) {
                                                    ...
                                                    logger.error("In login new Date().toISOString() > lastAttemptLoginPrereqTime" + "UserPrereqId is ->" + value._id)
                                                    // if(!(value.remindMeLater && value.remindMeLaterDate && value.remindMeLater == true && value.remindMeLaterDate == currentDate)){
                                                      optionalPrerequisites.push(value)
                                                    // }else{
                                                    //   logger.error("Skipping the Login Prereq Due to Remind Me Later")
                                                    ...
                                                    logger.error("In else- Date().toISOString() > lastAttemptLoginPrereqTimecondtion" + "UserPrereqId is ->" + value._id)
                                                }
                                                ...
                                                if (lastAttemptLoginPrereqDate == lastAttemptLoginPrereqWithFreqDate) {
                                                    ...
                                                    logger.error("Don't Display Login Prerequisites" + prereqForEvaluation._id + "UserPrereqId is ->" + value._id)
                                                } else {
                                        ...
                                        logger.error("in Else Condition of currentDate >= lastAttemptLoginPrereqWithFreqDate " + "UserPrereqId is ->" + value._id)
                                    }
                    ...
                    let optionalPrerequisitesUpadted = []
                    ...
                    if (mandatoryPrerequisites.length == 0 && optionalPrerequisites.length > 0) {
                        ...
                        optionalPrerequisites.forEach(optValue => {
                          logger.error("nextVistDate is --> "+ optValue.nextVistDateVirtual +"Current Date is --> "+currentDate + "PrereqId --> "+ optValue._id)
                            if (firstTimeLogin === "1" && optValue.lastVisitLogonTime && optValue.lastVisitLogonTime === authTime && apiCallFrom === "0") {
                                var userPrereqPatch = [{
                                    "operation": "replace",
                                    "field": "skipTime",
                                    "value": authTime
                                ...
                                var userPrerePatchResponse = patchMORecord(optValue._id, "alpha_kyid_enrollment_user_prerequisites", userPrereqPatch)


                            } else if (apiCallFrom === "0") {
                                optionalPrerequisitesUpadted.push(optValue)
                            }


                            if ((!optValue.lastVisitLogonTime || optValue.lastVisitLogonTime != authTime) && apiCallFrom !== "0") {
                              logger.error("Inside Condtion 1")
                                var userPrereqPatch = [{
                                    "operation": "replace",
                                    "field": "lastVisitTime",
                                    "value": optValue.lastVisitTimeVirtual
                                }, {
                                    "operation": "replace",
                                    "field": "lastVisitLogonTime",
                                    "value": optValue.lastVisitLogonTimeVirtaul
                                }, {
                                    "operation": "replace",
                                    "field": "nextVisitTime",
                                    "value": optValue.nextVisitTimeVirtual
                                }]
                                if(optValue.nextVistDateVirtual && optValue.nextVistDateVirtual !== null && currentDate > optValue.nextVistDateVirtual && optValue.remindMeLater && optValue.remindMeLater == true){
                                  logger.error("Inside Condtion 2")
                                  userPrereqPatch.push({"operation": "replace","field": "remindMeLater","value": false})
                                  var userPrerePatchResponse = patchMORecord(optValue._id, "alpha_kyid_enrollment_user_prerequisites", userPrereqPatch) 
                                  optionalPrerequisitesUpadted.push(optValue)
                                }                                
                                else if(!(optValue.remindMeLater && optValue.remindMeLater == true )){
                                  logger.error("Inside Condtion 3")
                                  var userPrerePatchResponse = patchMORecord(optValue._id, "alpha_kyid_enrollment_user_prerequisites", userPrereqPatch) 
                                  optionalPrerequisitesUpadted.push(optValue)
                                }                                
                              ...
                              logger.error("Inside Condtion 4")
                               if(optValue.nextVistDateVirtual && optValue.nextVistDateVirtual !== null && currentDate  > optValue.nextVistDateVirtual && optValue.remindMeLater && optValue.remindMeLater == true){
                                 logger.error("Inside Condtion 5")
                                 var userPrerePatchResponse = patchMORecord(optValue._id, "alpha_kyid_enrollment_user_prerequisites", [{"operation": "replace","field": "remindMeLater","value": false}])
                                }
                              
                              // if(!(optValue.remindMeLater && optValue.remindMeLater == true && userPrerePatchResponse == null)){
                              if(!(optValue.remindMeLater && optValue.remindMeLater == true )){
                                logger.error("Inside Condtion 6")
                                 optionalPrerequisitesUpadted.push(optValue)
                              }
                    ...
                    if (mandatoryPrerequisites.length > 0) {
                        optionalPrerequisitesUpadted = optionalPrerequisites
                    }

                    return {
                        "mandatoryPrerequisites": mandatoryPrerequisites,
                        "optionalPrerequisites": optionalPrerequisitesUpadted,
                        "updateLastPrereTime": updateLastPrereTime
                    }
                } else {
...
function getPrereqForEvaluation(input, userId, prereqArray, userInfo) {
    ...
    try {
        // logger.error("Inside getPrereqForEvaluation Function User Info Is -->  "+JSON.stringify(userInfo) )
        logger.error("Inside getPrereqForEvaluation Function User Identity ID is  "+userInfo.custom_userIdentity._refResourceId)
        var lastVerificationDate = null
        var lastVerificationDateEpoch = null
        var userIdentityRecord = null
        if(userInfo && userInfo.custom_userIdentity._refResourceId){
          userIdentityRecord =  getMORecord(userInfo.custom_userIdentity._refResourceId, ["*"], "alpha_kyid_user_identity")
          if(userIdentityRecord && userIdentityRecord.lastVerificationDate){
            lastVerificationDate = userIdentityRecord.lastVerificationDate
            logger.error("Inside getPrereqForEvaluation Function lastVerificationDate Date is "+lastVerificationDate)
            logger.error("Inside getPrereqForEvaluation Function lastVerificationDate Date is in Epoch "+new Date(lastVerificationDate).getTime())
            lastVerificationDateEpoch = new Date(lastVerificationDate).getTime()
            logger.error("Inside getPrereqForEvaluation Function Type of lastVerificationDate Date is in Epoch "+typeof lastVerificationDateEpoch)
          }
        }
        if (prereqArray.length > 0) {
            ...
            prereqArray.forEach(prereqRecord => {
                ...
                if (prereqRecord.prereqTypeId && prereqRecord.prereqTypeId.typeName && ((prereqRecord.prereqTypeId.typeName === "2" && userInfo.custom_organdonor === true) || (prereqRecord.prereqTypeId.typeName === "5" && userInfo.custom_selfEnrollMFA === true) || 
                   (prereqRecord.prereqTypeId.typeName.toLowerCase() === "custom" && prereqRecord.prerequisiteSourceName && prereqRecord.prerequisiteSourceName.toLowerCase() === "agreement prerequisite"  && invokeAgreementEndpt(userInfo.userName,prereqRecord.name) === true) ||
                   (prereqRecord.prereqTypeId.typeName.toLowerCase() === "custom" && prereqRecord.prerequisiteSourceName && prereqRecord.prerequisiteSourceName.toLowerCase() === "training prerequisite"  && invokeTrainingEndpt(userInfo.userName,prereqRecord.name) === true) ||
                   (prereqRecord.prereqTypeId.typeName === "7" && (lastVerificationDateEpoch > triggerStartDateEpoch && lastVerificationDateEpoch < triggerEndDateEpoch ))
                    )) {
                        ...
                        getUserPrerequsite.forEach(val => {
                            ...
                            const jsonArray = [{
                                ...
                                },
                                {
                                    "operation": "replace",
                                    "field": "updateDateEpoch",
                                    "value": Date.now() //**Pending: Compute Logic based human readable format later from alpha_user
                                },
                                {
                                    "operation": "replace",
                                    "field": "updateDate",
                                    "value": new Date().toISOString() //**Pending: Compute Logic based human readable format later from alpha_user
                                }
                    ...
                    // let compDate = new Date().toISOString()
                    // let compDateEpoch = Date.now()
                    // createUserPrereq(prereqRecord, userId,"2",compDate, compDateEpoch)
                    // createUserPrereqResponse = createUserPrereq(prereqRecord,userId,true)
                  }
                    ...
                    if (getUserPrerequsite) {
                        ...
                        getUserPrerequsite.forEach(value => {
                            // if(value.remindMeLater && value.remindMeLater == true){ //Skipping Prereq marked as Remind Me later
                            //   logger.error("Skipping Prereq marked as Remind Me later" + "Prereq Id --" + value._id + "::Status::" + value.status)
                            // }else{
            ...
            return {
                "expiredPrerequisites": expiredPrerequisites,
                "prereqForEvaluation": prereqForEvaluation
            }
      ...
      logger.error("Error Occurred while getPrereqForEvaluation"+ error)
        throw error
    }
...
function checkLoginPrereqCondtions(userInfo, applicationId, input, businessRoles) {
    ...
    try {
        ...
        let sessionCreateTimeEpoch = input.payload.authTime
        logger.error("sessionCreateTimeEpoch is --> " + sessionCreateTimeEpoch + "Tye is " + typeof sessionCreateTimeEpoch)
        const parts = sessionCreateTimeEpoch.split("-")
        sessionCreateTimeEpoch = parts[1]
        // let firstLogin = parts[0] || "0"
        let firstLogin = "0"
        ...
        if (userInfo) {
            ...
            if (userInfo.custom_approvalUnit3Code) {
                userApprovalUnit3 = userInfo.custom_approvalUnit3Code
            }
            if (userInfo.custom_approvalUnit4Code) {
                userApprovalUnit4 = userInfo.custom_approvalUnit4Code
            }
            if (userInfo.custom_approvalUnit5Code) {
                userApprovalUnit5 = userInfo.custom_approvalUnit5Code
            }
            if (userInfo.custom_kyidAccountType) {
                userAccountType = userInfo.custom_kyidAccountType
            }
            logger.error("userAccountType is -> " + userAccountType)
            logger.error("userApprovalUnit1 is -> " + userApprovalUnit1)
            logger.error("userApprovalUnit2 is -> " + userApprovalUnit2)
            logger.error("userApprovalUnit3 is -> " + userApprovalUnit3)
            logger.error("userApprovalUnit4 is -> " + userApprovalUnit4)
            logger.error("userApprovalUnit5 is -> " + userApprovalUnit5)
            if (userInfo.custom_lastAttemptedLoginPrereq) {
                lastAttemptedLoginPrereq = userInfo.custom_lastAttemptedLoginPrereq
            ...
            if (userInfo.effectiveRoles && userInfo.effectiveRoles.length > 0) {
                logger.error("Effective Roles are--> "+ userInfo.effectiveRoles )
            ...
            if (userRoles.length > 0 && businessRoles && businessRoles.length > 0) {
                logger.error("User Roles are --> "+userRoles)
                logger.error("businessRoles Roles are --> "+businessRoles)
                commonRoles = userRoles.filter(item => businessRoles.includes(item))
            }
                            ...
                            prereqResponse.forEach(prereqRecord => {
                                ...
                                if (prereqRecord.LoginDisplayCriteria && prereqRecord.LoginDueCompletion && prereqRecord.LoginTriggerCondition && prereqRecord.LoginTriggerCondition.triggerStartDate && prereqRecord.LoginTriggerCondition.triggerEndDate) {
                                    ...
                                    if (currentDate >= prereqRecord.LoginTriggerCondition.triggerStartDate && currentDate < prereqRecord.LoginTriggerCondition.triggerEndDate) {
                                      ...
                                      if(!consideredPrereq.includes(prereqRecord._id)){
                                        consideredPrereq.push(prereqRecord._id)
...
function validatePrereqConfiguration(input, data) {
    ...
    try {
        ...
        // Validate "days" fields
        const daysFields = [{
                key: "LoginDisplayCriteria.frequencyInDays",
                value: displayCriteria.frequencyInDays
            },
            {
                key: "LoginDueCompletion.gracePeriodinDays",
                value: dueCompletion.gracePeriodinDays
            },
            {
                key: "LoginDueCompletion.inDays",
                value: dueCompletion.inDays
            }
        ];
        ...
        // Validate "date" fields
        const dateFields = [{
                key: "LoginTriggerCondition.triggerStartDate",
                value: triggerCondition.triggerStartDate
            },
            {
                key: "LoginTriggerCondition.triggerEndDate",
                value: triggerCondition.triggerEndDate
            },
            {
                key: "LoginDueCompletion.dueDate",
                value: dueCompletion.dueDate
            }
        ];
        ...
        const LoginDisplayCriteria = [{
                key: "LoginDisplayCriteria.login",
                value: displayCriteria.login
            },
            {
                key: "LoginDisplayCriteria.skipOption",
                value: displayCriteria.skipOption
            }
        ];
...
function createUserPrereq(prereqResponse,userId,completed) {
    try {
        let dueDate = null
        let dueDatewithGrace = null
        ...
        if (prereqResponse && userId) {
            if(completed){
              ...
              requestBody.completionDate = new Date().toISOString()
              requestBody.completionDateEpoch = Date.now()
            }
            // if(compDate){
            //   requestBody.status = compDate
            // }
            // if(compDateEpoch){
            //   requestBody.status = compDateEpoch
            // }
            logger.error("createUserPrereq -> createUserPrereq -> " + prereqResponse)
            if (prereqResponse._id) {
                requestBody.preRequisiteId = {
                    "_ref": "managed/alpha_kyid_enrollment_prerequisite/" + prereqResponse._id,
                    "_refProperties": {}
                }
            }
            if (prereqResponse.prereqTypeId) {
                requestBody.preRequisiteTypeId = {
                    "_ref": "managed/alpha_kyid_enrollment_prerequisite_type/" + prereqResponse.prereqTypeId._refResourceId,
                    "_refProperties": {}
                }
            }
            if (prereqResponse.LoginDueCompletion) {
                if (prereqResponse.LoginDueCompletion.dueDate && prereqResponse.LoginDueCompletion.dueDate !== null && prereqResponse.LoginDueCompletion.dueDate !== "0" && prereqResponse.LoginDueCompletion.dueDate !== "") {
                    ...
                    // }
                  if (getFormatedDaysWithAdditional(currentDate, Number(prereqResponse.LoginDueCompletion.inDays), "add")) {
                        dueDate = getFormatedDaysWithAdditional(currentDate, Number(prereqResponse.LoginDueCompletion.inDays), "add")
                    }
                ...
                if (dueDate && dueDate !== null) {
                    ...
                    if (dueDatewithGrace > dueDate) {
                        dueDatewithGrace = getFormatedDaysWithAdditional(dueDate, Number(prereqResponse.LoginDueCompletion.gracePeriodinDays), "add")
...
function searchMO(MO_NAME, queryFilter, requiredProperties) {
    try {
        ...
        const searchResponse = openidm.query(MO_NAME, {
...
// Invoke Agreement API 
function invokeAgreementEndpt(KOGID, agreementName) {
    ...
    let url = identityServer.getProperty("esv.useragreementssignedcheck.api")
    ...
    try {
        ...
        //     //requestBody.url = identityServer.getProperty(serviceEndptRequestObj.endpoint.url)
            ...
            if(agreementName) {
                payload.AgreementNames.push(agreementName)
            ...
            if (payload.AgreementNames == null) {
                ...
                return "Agreement_Details_Not_Configured"
            }
            ...
            while (shouldRetryForAgreement && retryCountForAgreement < 3) {
                try {
                    ...
                    logger.error("Exception in "  + " is - " + error)
                ...
                if(responseAgreementEndptAPI.response.ResponseStatus == 0) {
                    ...
                    if (responseAgreementEndptAPI.response.AgreementsSignedStatus != null) {
                        ...
                        if (pendingCompletion.length > 0) {
                            logger.error("Agreement is Pending : Length of pendingCompletion Array" + pendingCompletion.length)
                            return false
...
// Invoke Training API 
function invokeTrainingEndpt(KOGID,trainingName) {
    ...
    let url = identityServer.getProperty("esv.usertrainingscompletioncheck.api")
    var payload = {
        KOGID: KOGID,
        TrainingModuleNames: []
    }
    ...
    try {
            //requestBody.url = identityServer.getProperty(serviceEndptRequestObj.endpoint.url)
            if(trainingName){
              payload.TrainingModuleNames.push(trainingName)
              requestBody.payload=payload
            }
