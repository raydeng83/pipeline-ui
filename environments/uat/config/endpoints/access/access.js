...
(function () {
    ...
    try {
        ...
        logger.error("Validate endpoint authorization response => "+res)
        if(res && res != null && res.status === 200){
    ...
    logger.error("===REQUEST in access=== " + JSON.stringify(request));
...
function validateRequest(paramJSON) {
    ...
    try {
        if (paramJSON.requestedUserAccountId && paramJSON.requestedUserAccountId !== null) {
            ...
            if (userResponse.accountStatus.toLowerCase() !== "active") {
...
function addRolesToUser(roles, kogUserAuthPayload, user, requesterUser, kyidContextIdForKOGApi) {
    ...
    try {
      ...
      //////Siddharth - 11/6/2025 - To send Mail on app enrollment failure.
        sendEmailfn(user, roles, "kyidPrerequisiteEnrolmentFailure", requesterUser);
      ///////////////
...
function sendEmailfn(user, roles, templateId, requesterUser) {
    ...
    if(user && user.custom_languagePreference && user.custom_languagePreference!=null){
      if(languageMap && languageMap!=null){
        locale = getLangCode(user.custom_languagePreference,languageMap);
      }
    }
  
    // if(user && user.languagePreference && user.languagePreference === "1" ){
    //   locale = "en"
    // }
    // else if(user && user.languagePreference && user.languagePreference === "2"){
    //   locale = "es"
    // }
    try {
    ...
    // Check if any role has delegationEndDate (determines whether to show the column)
    var hasDelegationEndDate = false;
    roles.forEach(function (role) {
        if (role.delegationEndDate) hasDelegationEndDate = true;
    });

    // Build roleNamesHtml - full table with one row per role
    var applicationNames = []
    var roleNames = [];
    var emailTimeStamp = isoToEastern();
    var rowCount = roles.length;

    // Build roleNamesHtml - only <tr> rows (table/thead are in the email template)
    var roleNamesHtml = "";

    logger.error("sendEmailfn - Number of roles to process: " + roles.length);
    roles.forEach(function (role, index) {
        logger.error("sendEmailfn - Processing role: " + role.name + ", App: " + (role.businessAppId ? role.businessAppId.name : "NO APP"));
        roleNamesHtml += "<tr>";
        roleNamesHtml += "<td>" + role.businessAppId.name + "</td>";
        roleNamesHtml += "<td>" + role.name + "</td>";
        if (hasDelegationEndDate) {
            roleNamesHtml += "<td>" + (role.delegationEndDate ? formatDateDisplay(role.delegationEndDate) : "") + "</td>";
        }
        if (index === 0) {
            roleNamesHtml += "<td rowspan='" + rowCount + "' style='vertical-align:middle'>KYID-System</td>";
    ...
    sendMail(mail, givenName, sn, roleNamesHtml, phoneContact, emailContact, applicationNamesStr, templateId, requesterUser, locale, hasDelegationEndDate)
...
function sendMail(mail, givenName, sn, roleNamesHtml, phoneContact, emailContact, applicationNames, templateId, requesterUser, locale, hasDelegationEndDate) {
    try {
        var params = new Object();
        // var easternTimeStamp = convertTime.isoToEastern();
        logger.error("InsideFnEmailTemplateCall")
       logger.error("InsideFnEmailTemplateCall#### " + request)
...
// Date:11/07/2025 - Narendra : Added this function to search Manage Object based on Query
function searchMO(MO_NAME, queryFilter, requiredProperties) {
    try {
        ...
        logger.error("Error Occurred While in searchMO" +error)
        throw error
    }
...
function getCompletedPrereq(userId, prereqId, prereqTypeId) {
    try {
        logger.error("Endpoint Name: access Inside getCompletedPrereq ")
        if (prereqId) {
            let reqestBody = {
                "payload": {
                    "userId": userId,
            ...
            if (response && response !== null && response.resultCount > 0) {
                logger.error("Endpoint Name: access getCompletedPrereq -- response  is" + JSON.stringify(response))
        ...
        logger.error("Endpoint Name: access Error Occurred while getCompletedPrereq " + error)
        return null
...
function patchUserPrerequisites(id,status,completionDateEpoch,completionDate,expiryDateEpoch,expiryDate) {
...
function getPendingEnrollment(request){
  try {
      let pendingEnrollment =[]
        if(request && request.roleContext){
          
          if(request.roleIds && request.roleIds.length>0 && request.requestedUserId){
            const roleFilter = request.roleContext.map(id => `roleContext/[roleId eq '${id.roleId}']`).join(' AND ');
            let queryFilter = {"_queryFilter":`status eq 'IN_PROGRESS' AND requestedUserId eq '${request.requestedUserId}' AND (recordState eq 'ACTIVE' OR recordState eq '0') AND (${roleFilter})`};
            logger.error("Query Filter is "+ JSON.stringify(queryFilter))
            let response = searchMO("alpha_kyid_enrollment_request", queryFilter, ["*"])
            logger.error("getPendingEnrollment Response is --> "+ response)
            if(response){
              response.forEach(val=>{
                if(val.roleContext &&val.roleContext.length>0 && compareJsonArrays(val.roleContext, request.roleContext) === true){
                  logger.error("getPendingEnrollment Inside Compare True Condtion")
                  let userPrereqQuery = {"_queryFilter":`enrollmentRequestId eq '${val._id}' AND !(status eq 'COMPLETED' OR status eq '2' OR status eq 'ALREADY_COMPLETED' OR status eq '2') `}
                  let userPrereqResponse = searchMO("alpha_kyid_enrollment_user_prerequisites", userPrereqQuery, ["preRequisiteTypeId/*","preRequisiteId/*","*"])
                  if(userPrereqResponse && userPrereqResponse.length>0){
                    let userPrereqCompletedIds = []
                    userPrereqResponse.forEach(record=>{
                      if((record.preRequisiteTypeId && record.preRequisiteId) && (record.recordState === "ACTIVE" || record.recordState === "0")){
                        let completedUserPrereq = getCompletedPrereq(record.requestedUserAccountId, record.preRequisiteId._id, record.preRequisiteTypeId._id)
                        if(completedUserPrereq){
                          logger.error("endpoint/access - getPendingEnrollment - getCompletedPrereq "+ completedUserPrereq)
                          ...
                          let allowReuseIfDaysOld = record.preRequisiteId.enrollmentActionSettings.allowReuseIfDaysOld || "0"
                          // let prereqReuse = isPrereqReusable(allowReuseIfDaysOld, completionDateEpoch)
                          let prereqReuse = true
                          logger.error("endpoint/access - prereqReuse - "+ prereqReuse)
                          if (prereqReuse === true) {
                            let patchResponse = patchUserPrerequisites(record._id, "ALREADY_COMPLETED",completionDateEpoch,completionDate,expiryDateEpoch,expiryDate)
                            if (patchResponse) {
                    ...
                    if(userPrereqCompletedIds.length ===userPrereqResponse.length ){
                      pendingEnrollment.push(val._id)
                    }
                  }
                  else{
                    pendingEnrollment.push(val._id)
    ...
    logger.error("Error Occurred While Getting Pending Enrollments "+ error)
    return []
  }
}

function getPendingEnrollmentByContext(roleContext,userId){
  try {
      let pendingEnrollment =[]
        if(roleContext && userId){
          if(roleContext.roleId){
            let queryFilter = {"_queryFilter": `roleContext/[roleId eq '${roleContext.roleId}'] AND status eq 'IN_PROGRESS' AND requestedUserId eq '${userId}' AND (recordState eq 'ACTIVE' OR recordState eq '0')`};
            let response = searchMO("alpha_kyid_enrollment_request", queryFilter, ["*"])
            if(response){
            logger.error("endpoint/access - getPendingEnrollmentByContext alpha_kyid_enrollment_request - "+response)
              response.forEach(val=>{
                if(val.roleContext && val.roleContext.length>0 && isJsonInArray(val.roleContext,roleContext) === true){
                  // pendingEnrollment.push(val._id)
                  let userPrereqQuery = {"_queryFilter":`enrollmentRequestId eq '${val._id}' AND !(status eq 'COMPLETED' OR status eq '2' OR status eq 'ALREADY_COMPLETED' OR status eq '2') `}
                  let userPrereqResponse = searchMO("alpha_kyid_enrollment_user_prerequisites", userPrereqQuery, ["preRequisiteTypeId/*","preRequisiteId/*","*"])
                  if(userPrereqResponse && userPrereqResponse.length>0){
                    ...
                    userPrereqResponse.forEach(record=>{
                      if((record.preRequisiteTypeId && record.preRequisiteId) && (record.recordState === "ACTIVE" || record.recordState === "0")){
                        let completedUserPrereq = getCompletedPrereq(record.requestedUserAccountId, record.preRequisiteId._id, record.preRequisiteTypeId._id)
                        if(completedUserPrereq){
                          logger.error("endpoint/access - getPendingEnrollmentByContext - getCompletedPrereq "+ completedUserPrereq)
                          let expiryDateEpoch = completedUserPrereq.expiryDateEpoch || null
                          let expiryDate = completedUserPrereq.expiryDate || null
                          let completionDateEpoch = completedUserPrereq.completionDateEpoch || null
                          let completionDate = completedUserPrereq.completionDate || null
...
function deepEqualArray(a, b) {
  ...
  if (Array.isArray(a)) {
    ...
    for (let i = 0; i < a.length; i++) {
      if (!deepEqualArray(a[i], b[i])) return false;
    }
    return true;
  }
  ...
  for (let key of keysA) {
    if (!keysB.includes(key) || !deepEqualArray(a[key], b[key])) return false;
  }
  return true;
}

// Compares two arrays of JSON objects
...
function isJsonInArray(array, jsonToMatch) {
  for (let obj of array) {
    if (deepEqualObject(obj, jsonToMatch)) return true;
  }
  return false;
}
...
function provisionAccess(paramJSON) {
  logger.error(LP + " Role Provisioining Function Start"+new Date().toISOString());
    const LP = "[PROVISION_ACCESS]";
    var enrollmentId = (paramJSON && paramJSON.enrollmentRequestId) || "N/A";
    var userId = (paramJSON && paramJSON.requestedUserAccountId) || "N/A";
    logger.error(LP + " START paramJSON=" + JSON.stringify(paramJSON || {}));

    try {
        let pendingEnrollmentRequestIds = [] // Date: 11/19/2025 - Narendra Bug Fix - Defect 206523 - Checking any IN_PROGRESS enrollment request with same role context exists -
        let objects = validateRequest(paramJSON);
        if (objects.userResponse && objects.requestResponse) {
            logger.error(LP + " VALIDATED enrollmentId=" + enrollmentId + " resolvedUserId=" + objects.userResponse._id);
            ...
            pendingEnrollmentRequestIds = getPendingEnrollment(requestResponse) // Date: 11/19/2025 - Narendra Bug Fix - Defect 206523 - Checking any IN_PROGRESS enrollment request with same role context exists 
            ...
            logger.error(LP + " ROLECONTEXT enrollmentId=" + enrollmentId + " count=" + (requestResponse.roleContext ? requestResponse.roleContext.length : 0) + " data=" + JSON.stringify(requestResponse.roleContext));
            ...
            requestResponse.roleContext.forEach(function (roleContext) {
                ...
                if (roleContext && roleContext !== null) {
                  ...
                  // Date: 11/19/2025 - Narendra Bug Fix - Defect 206523 - Checking any IN_PROGRESS enrollment request with same role context exists - Start

                   let pendingEnrollments = getPendingEnrollmentByContext(roleContext,requestResponse.requestedUserId)
                    if(pendingEnrollments.length>0){
                      pendingEnrollments.forEach(val=>{
                        pendingEnrollmentRequestIds.push(val)
                      })
                    }
                   // Date: 11/19/2025 - Narendra Bug Fix - Defect 206523 - Checking any IN_PROGRESS enrollment request with same role context exists - End
                    ...
                    roleContext.delegationEndDate ? entry.expiryDateEpoch = new Date(roleContext.delegationEndDate).getTime() : null
                    logger.error("[DEBUG-DELEGATION-ENDDATE] ACCESS endpoint - Setting entry.expiryDate = " + entry.expiryDate + ", entry.expiryDateEpoch = " + entry.expiryDateEpoch + " (from roleContext.delegationEndDate = " + roleContext.delegationEndDate + ")");
                ...
                tempRole.delegationEndDate = roleContext.delegationEndDate || null;
                ...
                if (tempRole.businessAppId && enrolledApps.find(item => item.kogAppId === tempRole.businessAppId.kogAppId) === undefined) {
                    ...
                    if (requestResponse && requestResponse.enrollmentContext && requestResponse.enrollmentContext.hasOwnProperty('returnURL') && requestResponse.enrollmentContext.returnURL !== null && requestResponse.enrollmentContext.returnURL !== undefined && requestResponse.enrollmentContext.returnURL.toString().trim() !== "") {
                        //dharjani: No Need to Encode URI Component here
                    ...
                    else if ((contextBasedApp)&& (contextReturnURL || contextBasedApp.applicationURL ) ) {
                        softSignOutURLForRedirection = contextReturnURL || contextBasedApp.applicationURL;
                    }                    
                    else if (tempRole.businessAppId && tempRole.businessAppId.softLogoutURL) {
                        softSignOutURLForRedirection = tempRole.businessAppId.applicationURL
                            ? `${tempRole.businessAppId.softLogoutURL}?ReturnURL=${encodeURIComponent(tempRole.businessAppId.applicationURL)}`
                    ...
                    else if (tempRole.businessAppId && tempRole.businessAppId.applicationURL) {
                        softSignOutURLForRedirection = tempRole.businessAppId.applicationURL
                    }
                  ...
                  // NR: 05/02/2025- Added this fix to get the distinct app URL in Response
                      if(enrolledApps.length>0){
                      enrolledApps.forEach(app => {
                          if (app.softSignOutURL && softSignOutURLForRedirection && (app.softSignOutURL != softSignOutURLForRedirection)) {
                                  enrolledApps.push({
                ...
                if (tempRole.accessPolicy && tempRole.accessPolicy._ref) {
                    ...
                    policy.dependentRole.forEach(function (depRole) {
                        ...
                        if(tempDepRole){
                          let depRoleContext = roleContext
                          depRoleContext.roleId = tempDepRole._id
                          depRoleContext.roleName = tempDepRole.name
                          depRoleContext.applicationId = tempDepRole.businessAppId._id
                          depRoleContext.appName = tempDepRole.businessAppId.name

                          // Date: 11/19/2025 - Narendra Bug Fix - Defect 206523 - Checking any IN_PROGRESS enrollment request with same role context exists - Start
                          pendingEnrollments = getPendingEnrollmentByContext(depRoleContext,requestResponse.requestedUserId)
            ...
            // Deduplication check BEFORE other operations
            logger.error(LP + " DEDUP_START enrollmentId=" + enrollmentId + " accessCount=" + accessArray.length + " roleCount=" + roleArray.length);

            var filteredAccessArray = [];
            var filteredRoleArray = [];
            var filteredKogUserAuthPayload = [];
            var skippedEntries = [];

            accessArray.forEach(function (entry, index) {
                // Build query filter to check if same access already exists
                ...
                // Add delegator conditions for deduplication
                // This allows different delegators to create separate access records for the same role
                if (entry.originalDelegatorIdentifier) {
                    queryFilter += ` and /originalDelegatorIdentifier eq "${entry.originalDelegatorIdentifier}"`;
                } else {
                    queryFilter += ` and !/originalDelegatorIdentifier pr`;
                }
                if (entry.currentDelegatorIdentifier) {
                    queryFilter += ` and /currentDelegatorIdentifier eq "${entry.currentDelegatorIdentifier}"`;
                } else {
                    queryFilter += ` and !/currentDelegatorIdentifier pr`;
                }
                ...
                logger.error(LP + " DEDUP_QUERY enrollmentId=" + enrollmentId + " idx=" + index + " queryFilter=" + queryFilter);
                ...
                if (existingAccess && existingAccess.result && existingAccess.result.length > 0) {
                    logger.error(LP + " DEDUP_SKIP enrollmentId=" + enrollmentId + " idx=" + index + " existingAccessId=" + existingAccess.result[0]._id + " userId=" + entry.userIdentifier + " appId=" + entry.appIdentifier + " roleId=" + entry.roleIdentifier + " originalDelegator=" + entry.originalDelegatorIdentifier + " currentDelegator=" + entry.currentDelegatorIdentifier);
                    skippedEntries.push(entry);
                } else {
                    logger.error(LP + " DEDUP_KEEP enrollmentId=" + enrollmentId + " idx=" + index + " roleId=" + entry.roleIdentifier);
                    filteredAccessArray.push(entry);
                    // Keep corresponding role in roleArray
                    if (roleArray[index]) {
                        filteredRoleArray.push(roleArray[index]);
                    }
                    // Keep corresponding kogUserAuthPayload
                    if (kogUserAuthPayload && kogUserAuthPayload[index]) {
                        filteredKogUserAuthPayload.push(kogUserAuthPayload[index]);
                    }
                }
            });

            logger.error(LP + " DEDUP_RESULT enrollmentId=" + enrollmentId + " toCreate=" + filteredAccessArray.length + " skipped=" + skippedEntries.length);

            if (filteredAccessArray.length === 0) {
                logger.error(LP + " SKIP_ALL enrollmentId=" + enrollmentId + " reason=allExist");
            } else {
                logger.error(LP + " KOG_CALL enrollmentId=" + enrollmentId + " filteredRoleArray=" + JSON.stringify(filteredRoleArray));
                logger.error(LP + " KOG_CALL enrollmentId=" + enrollmentId + " filteredKogUserAuthPayload=" + JSON.stringify(filteredKogUserAuthPayload));
                logger.error(LP + " KOG_CALL enrollmentId=" + enrollmentId + " userResponse=" + JSON.stringify(userResponse));
                logger.error(LP + " KOG_CALL enrollmentId=" + enrollmentId + " requesterUser=" + requesterUser);
                logger.error(LP + " Role Provisioining Start"+new Date().toISOString());
                addRolesToUser(filteredRoleArray, filteredKogUserAuthPayload, userResponse, requesterUser, kyidContextIdForKOGApi);
                logger.error(LP + " Role Provisioining End"+new Date().toISOString());
                try {
                    filteredAccessArray.forEach(function (entry, idx) {
                        logger.error(LP + " ACCESS_CREATING enrollmentId=" + enrollmentId + " idx=" + idx + " entry=" + JSON.stringify(entry));
                        var resultAccessMO = openidm.create("managed/alpha_kyid_access", null, entry);
                        logger.error(LP + " ACCESS_CREATED enrollmentId=" + enrollmentId + " idx=" + idx + " result=" + JSON.stringify(resultAccessMO));
                    });
                } catch (error) {
                    logger.error(LP + " ACCESS_CREATE_ERROR enrollmentId=" + enrollmentId + " error=" + JSON.stringify(error));
                }
            }

          var dateTimeISO = new Date().toISOString();
        let currentEpoch = Date.now();
            logger.error(LP + " ENROLLMENT_UPDATE enrollmentId=" + enrollmentId + " newStatus=COMPLETED");
            ...
            logger.error(LP + " ENROLLMENT_PATCH_RESULT enrollmentId=" + enrollmentId + " patchResponse=" + JSON.stringify(patchResponse));

          // Date: 11/19/2025 - Narendra Bug Fix - Defect 206523 -  Start
            if(pendingEnrollmentRequestIds.length>0){
              logger.error(LP + " PENDING_UPDATE enrollmentId=" + enrollmentId + " count=" + pendingEnrollmentRequestIds.length + " ids=" + pendingEnrollmentRequestIds.join(","));
              pendingEnrollmentRequestIds.forEach(val=>{
                 patchResponse = openidm.patch("managed/alpha_kyid_enrollment_request/" + val, null, [{ "operation": "replace", "field": "status", "value": "COMPLETED" }, {
                "operation": "replace",
                "field": "updateDateEpoch",
                "value": currentEpoch
            }, {
                ...
                "value": dateTimeISO
                 ...
                 logger.error(LP + " PENDING_PATCH_RESULT enrollmentId=" + enrollmentId + " pendingId=" + val + " patchResponse=" + JSON.stringify(patchResponse));
              })
            }
          // Date: 11/19/2025 - Narendra Bug Fix - Defect 206523 -  End

            logger.error(LP + " SUCCESS enrollmentId=" + enrollmentId + " created=" + filteredAccessArray.length + " skipped=" + skippedEntries.length + " pendingUpdated=" + pendingEnrollmentRequestIds.length);
        } else {
            logger.error(LP + " VALIDATE_FAILED enrollmentId=" + enrollmentId + " userResponse=" + !!objects.userResponse + " requestResponse=" + !!objects.requestResponse);
            throw { code: 500, message: "User or enrollment request not found" };
        }
    } catch (error) {
        logger.error(LP + " ERROR enrollmentId=" + enrollmentId + " userId=" + userId + " error=" + JSON.stringify(error));
        throw { code: 500, message: getException(error) };
    }
  logger.error(LP + " Role Provisioining Function End"+new Date().toISOString());
    return {
        "status": "success",
...
// function getPendingEnrollmentRequest(roleContext) {
//   try {
//     if(roleContext && roleContext.length>0){
//       roleContext.forEach(val=>{
//         if(val.appId && val.roleId){
//           let query = `requestedUserAccountId eq '${userId}' AND preRequisiteId/_refResourceId eq '${prereqId}' AND (status eq '2' OR status eq 'COMPLETED' ) AND (recordState eq '0' OR recordState eq 'ACTIVE')`
...
// function getCompletedPrereq(userId,prereqId) {
...
//     userQueryResult.result.forEach(val=>{
...
//   }
// } catch (error) {
//   logger.error("Error Occurred Wile Fetching Completed Prerequiste")
//   return null
// }
// }
...
function deprovisionAccess(paramJSON) {
    ...
    try {
        ...
        if (objects.userResponse && objects.requestResponse) {
            ...
            // Send email notification with ALL roles being removed (including dependent roles)
        }
    } catch (error) {
        throw { code: 500, message: error.message };
    }
